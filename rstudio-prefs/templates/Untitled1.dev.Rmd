...
analyticDF_time2event %>% {cat("> ",deparse(substitute(.))," |> rownames_to_column() |> rownames_to_column() |> as_tibble()","\n", sep=""); print(as_tibble(tail(rownames_to_column(.))))}
...
assign( paste0(DataSetName,".NA",".rmAllNA"), get(paste0(DataSetName,".NA")) |> select_if(function(vec) !all(is.na(vec))) )
cat("dim(",DataSetName,") = ",deparse(dim(get(DataSetName))),"\n", sep="") 
cat("dim(",DataSetName,".NA",".rmAllNA",") = ",deparse(dim(get(paste0(DataSetName,".NA",".rmAllNA")))),"\n", sep="") 
cat(strrep("%",80),"\n",sep=""); 
...
## ------ < DataSet Summary > ------
### \$ paste0(DataSetName,".NA",".rmAllNA",".fct") ====
```{r, results="markup", collapse=TRUE, paged.print=FALSE, comment="", R.options=list(width=120)}
options(width=120)
vec_varname4ID = c("ID", "CompositeKey", "PERSON_ID", "RN_INDI", "NIHID")
for (varname in vec_varname4ID) {if(varname %in% names(get(paste0(DataSetName,".NA",".rmAllNA")))) cat("n_distinct(",paste0(DataSetName,".NA",".rmAllNA"),"$",varname,") = ",n_distinct(get(paste0(DataSetName,".NA",".rmAllNA"))[[varname]]),"\n", sep="")}


t0=Sys.time()
cat(strrep("~",80),"\n",sep=""); cat("> ",paste0(DataSetName,".NA",".rmAllNA")," |> select_if(is.numeric))"," |> summary()","\n", sep=""); get(paste0(DataSetName,".NA",".rmAllNA")) |> select_if(is.numeric) |> summary() #-----
Sys.time()-t0
t0=Sys.time()
cat(strrep("~",80),"\n",sep=""); cat("> ",paste0(DataSetName,".NA",".rmAllNA")," |> select_if(is.logical))"," |> summary()","\n", sep=""); get(paste0(DataSetName,".NA",".rmAllNA")) |> select_if(is.logical) |> summary() #-----
Sys.time()-t0

t0=Sys.time()
cat(strrep("%",80),"\n",sep=""); cat("> ",paste0(DataSetName,".NA",".rmAllNA",".fct")," = ",paste0(DataSetName,".NA",".rmAllNA")," |> mutate_if(is.character, as.factor)","\n", sep=""); assign( paste0(DataSetName,".NA",".rmAllNA",".fct"), get(paste0(DataSetName,".NA",".rmAllNA")) |> mutate_if(is.character, as.factor) )
Sys.time()-t0

t0=Sys.time()
cat(strrep("+",80),"\n",sep=""); cat("> ",paste0(DataSetName,".NA",".rmAllNA",".fct")," |> select_if(is.factor))"," |> summary()","\n", sep=""); get(paste0(DataSetName,".NA",".rmAllNA",".fct")) |> select_if(is.factor) |> summary() #-----
Sys.time()-t0
# cat(strrep("~",80),"\n",sep=""); cat("> ",paste0(DataSetName,".NA",".rmAllNA")," |> select_if(is.factor))"," |> summary()","\n", sep=""); get(paste0(DataSetName,".NA",".rmAllNA")) |> select_if(is.character) |> map_df(as.factor) |> summary() #-----
# Sys.time()-t0
