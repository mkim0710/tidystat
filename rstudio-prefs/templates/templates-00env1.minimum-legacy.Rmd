...
cat("    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%    \n")
if(!exists("DSname.Date")) DSname.Date = ifelse(exists(paste0(DSN,".Date")), paste0(DSN,".Date"), DSN)
.vec_nlevel_lt20.value_number.unique = 'get(DSname.Date) %>% map(function (vec) {vec.unique = unique(vec); if(length(vec.unique) < 20) return(vec.unique) else return(NULL) }) %>% unlist %>% unique %>% str_subset("^[0-9]+$") %>% sort' |> env1$f$f_CodeText.ECHO(EXECUTE = TRUE, deparse_cat = TRUE, LinePrefix4CodeText = "> ", LinePrefix4Output = "", CodeEqualsOutput = FALSE)
...
cat("    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%    \n")
DSname.Date.NA = DSname.Date |> paste0(".NA"); assign( DSname.Date.NA, get(DSname.Date) |> mutate_if(is.factor, as.character) ); MetaData$DSnames[[DSname.Date.NA]] = list(DSN = DSname.Date.NA)
.vec_value4NA = c( "", "N/A", "NA"); cat("    ",".vec_value4NA == ",deparse(.vec_value4NA),"  \n", sep="")
for (.value4NA in .vec_value4NA) {
    DSname.Date.NA = DSname.Date |> paste0(".NA"); assign( DSname.Date.NA, get(DSname.Date.NA) |> mutate_if(is.character, na_if, .value4NA) )
}
### :: DSname.DNR = DataSetName.Date.NA.rmAllNA = =====
DSname.DNR = paste0(DSN,".DNR"); assign( DSname.DNR, get(DSname.Date.NA) |> select_if(function(vec) !all(is.na(vec))) ); MetaData$DSnames[[DSname.DNR]] = list(DSN = DSname.DNR)
cat(" dim(",DSname.Date,") == ",deparse(dim(get(DSname.Date))),"  \n", sep="") 
cat(" dim(",DSname.DNR,") == ",deparse(dim(get(DSname.DNR))),"  \n", sep="") 
# :: DS.DNR = DataSet.Date.NA.rmAllNA = =====
DS.DNR = get(DSname.DNR)
cat("    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%    \n")
"get(DSname.Date.NA) |> env1$f$f_df.NotNA_p_df()" |> env1$f$f_CodeText.ECHO(EXECUTE = TRUE, deparse_cat = FALSE, LinePrefix4CodeText = "> ", LinePrefix4Output = "")
```
...
for (varname in vec_candidate4ID) {if(varname %in% names(get(DSname.DNR))) cat(" n_distinct(",DSname.DNR,"$",varname,") == ",n_distinct(get(DSname.DNR)[[varname]]),"  \n", sep="")}
...
cat("    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%    \n"); 
DSname.Date.NA = DSname.Date |> paste0(".NA"); 
### :: DSname.DNR = DataSetName.Date.NA.rmAllNA = =====
DSname.DNR = paste0(DSname.DNR); 
DSname.DNR.lbl_pending.fct = paste0(DSname.DNR,".fct"); 
"assign( DSname.DNR.lbl_pending.fct, get(DSname.DNR) |> mutate_if(is.character, as.factor) , envir = .GlobalEnv)" |> env1$f$f_CodeText.ECHO(EXECUTE = TRUE, deparse_cat = FALSE, LinePrefix4CodeText = "> ", LinePrefix4Output = "") |> system.time() |> round(3) |> unclass() |> deparse() |> cat("\n")
cat("   #++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++    \n"); 
"get(DSname.DNR.lbl_pending.fct) |> select_if(is.factor) |> summary()" |> env1$f$f_CodeText.ECHO(EXECUTE = TRUE, deparse_cat = FALSE, LinePrefix4CodeText = "> ", LinePrefix4Output = "") |> system.time() |> round(3) |> unclass() |> deparse() |> cat("\n")
...
assign(DSname.DNR.lbl_pending.fct, 
       get(DSname.DNR.lbl_pending.fct) %>% mutate(
...
```{r, echo=FALSE, results="markup", collapse=TRUE, paged.print=FALSE, comment="", R.options=list(width=120)}
options(width=120)
DS1.recode = get(DSname.DNR.lbl_pending.fct)
DSN1.recode.n = paste0(DSN,".DNR",".lbl_pending",".fct",".recode",".n",nrow(DS1.recode))
DSN1.recode.n
DS1.recode
```
...
if (Sys.getenv("PARENT_RENDERING") != "YES") {    
    if(exists("MetaData") && !is.null(unlist(MetaData))) {
        ...
        # cat("    ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~    \n")
        # "MetaData$DSnames |> str(max.level = 2, give.attr = FALSE)" |> env1$f$f_CodeText.ECHO(EXECUTE = TRUE, deparse_cat = FALSE, LinePrefix4CodeText = "> ", LinePrefix4Output = "")
        # cat("    ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~    \n")
        ...
        MetaData$DSnames = MetaData$DSnames %>% map(function(ls) {  if("str" %in% names(attributes(ls))) {  if(!is.environment(attributes(ls)$str)) {  attributes(ls)$str = as.environment(list(DSN.get.str.capture.output = attributes(ls)$str))  }  }; ls  })
        MetaData |> env1$f$fLs.removeEnvAttr(convert_env_to_list  = TRUE) |> list_modify(tblVarName = NULL) |> str(vec.len = 100, list.len = 1000) |> capture.output(file = env1$path$LastSourceEditorContext.path_FileName.MetaData.rds |> str_replace("\\.rds$", "-str.txt"))
        ...
        if(!identical(.MetaData0, MetaData)) {
            ...
            # MetaData |> env1$f$f_object.get_objectname.size.write_rds.git_lfs_track_add_f(.path_file = .path_file, createBACKUP = TRUE, EXECUTE = FALSE)
            env1$f$f_objectname.size.write_rds.git_lfs_track_add_f(.objectname = "MetaData", .path_file = .path_file, createBACKUP = TRUE, EXECUTE = FALSE)
        }
