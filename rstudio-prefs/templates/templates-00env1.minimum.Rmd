...
# __________|------  
# @@ DATA) filter() -> New DataSet ----  
## :: DataSet1.recode = DataSet.Date.NA.rmAllNA.lbl_pending.fct.recode
## :: DataSetName1.recode.n = paste0(DataSetName,".Date",".NA",".rmAllNA",".lbl_pending",".fct",".recode",".n",nrow(DataSet1))
## :: DataSetName1.recode.n = paste0(DataSetName,".DNR","".lbl_pending",".fct",".recode",".n",nrow(DataSetName1.recode.n))
...
## :: DataSet2.recode.filter = DataSet1.recode |> filter(!!rlang::sym(.InclusionCriteria) == TRUE & !!rlang::sym(.ExclusionCriteria) == FALSE)
## :: DataSetName2.recode.filter.n = paste0(DataSetName,".DNR",".lbl_pending",".fct",".recode",".n",nrow(DataSet2.recode.filter))  
```{r DataSet2-recode-filter-n-writeRDS-NoEvalNoEchoNoMsgNoResults, eval=TRUE, echo=FALSE, warning=TRUE, message=NA, results="hide"}
.objectname = "DataSetName1.recode.n"; cat("> ", .objectname, " = ", deparse(get(.objectname)), "  \n", sep="")
cat("*** Old DataSetName = ", deparse(DataSetName), "  \n", sep="")
##________________________________________________________________________________  
cat("    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%    \n")
DataSet2.recode.filter = DataSet1.recode |> filter(!!rlang::sym(.InclusionCriteria) == TRUE & !!rlang::sym(.ExclusionCriteria) == FALSE)
DataSetName2.recode.filter.n = paste0(DataSetName,".DNR",".lbl_pending",".fct",".recode",".n",nrow(DataSet2.recode.filter))  
assign(DataSetName2.recode.filter.n, DataSet2.recode.filter)
##~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~  
.objectname = "DataSetName2.recode.filter.n"; cat("> ", .objectname, " = ", deparse(get(.objectname)), "  \n", sep="")
...
## :: get(DataSetName2.recode.filter.n) |> select(matches(VarName.suffix.regex))
```{r, echo=FALSE, results="markup", collapse=TRUE, paged.print=FALSE, comment="", R.options=list(width=120)}
get(DataSetName2.recode.filter.n) |> names() |> env1$env.internal.attach$deparse.cat0()
VarName.suffix.regex = get(DataSetName2.recode.filter.n) |> names() |> paste0(collapse="|")
VarName.suffix.regex |> dput()
DataSetName2.recode.filter.n.select = paste0(DataSetName2.recode.filter.n,"select",ncol(get(DataSetName2.recode.filter.n) |> names() |> str_subset(VarName.suffix.regex)))
assign(  DataSetName2.recode.filter.n.select, get(DataSetName2.recode.filter.n) |> select(matches(VarName.suffix.regex))  )
get(DataSetName2.recode.filter.n.select) |> str(max.level = 2, give.attr = TRUE)
```
