...
if(!identical(.MetaData0, MetaData)) {
    # cat("    ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~    \n")
    # cat('env1$env.internal$f_filename.ext.createBackup(backup_from_path.filename.ext = "', env1$path$CurrentSource.path.filename.ext |> str_replace("\\.([[:alnum:]]+)$",".MetaData.rds"), '", .backup_to_path="-backup", timeFormat="%y%m%d_%H", overwrite=TRUE)', "  ;  ", sep="")
    # cat('MetaData |> write_rds("', env1$path$CurrentSource.path.filename.ext |> str_replace("\\.([[:alnum:]]+)$",".MetaData.rds"), '", compress = "xz") |> system.time()', "  \n", sep="")
    # cat("'git add -f \"", env1$path$CurrentSource.path.filename.ext |> str_replace("\\.([[:alnum:]]+)$",".MetaData.rds") ,"\"' |> system(intern=TRUE)  \n", sep="")
    env1$env.internal$f_filename.ext.createBackup(backup_from_path.filename.ext = env1$path$CurrentSource.path.filename.ext |> str_replace("\\.([[:alnum:]]+)$",".MetaData.rds"), .backup_to_path="-backup", timeFormat="%y%m%d_%H", overwrite=TRUE)  
    MetaData |> write_rds(env1$path$CurrentSource.path.filename.ext |> str_replace("\\.([[:alnum:]]+)$",".MetaData.rds"), compress = "xz") |> system.time()
    paste0("git add -f ", env1$path$CurrentSource.path.filename.ext |> str_replace("\\.([[:alnum:]]+)$",".MetaData.rds") |> shQuote()) |> system(intern=TRUE)
}
```
