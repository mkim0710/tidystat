...
if (Sys.getenv("PARENT_RENDERING") != "YES") {    
    if(exists("MetaData") && !is.null(unlist(MetaData))) {
        ...
        MetaData |> env1$f$fLs.removeEnvAttr() |> list_modify(tblVarName = NULL) |> str(vec.len = 100, list.len = 1000) |> capture.output(file = env1$path$LastSourceEditorContext.path_FileName.MetaData.rds |> str_replace("\\.rds$", "-str.txt"))
