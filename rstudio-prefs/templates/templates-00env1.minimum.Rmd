...
for (varname in vec_candidate4ID) {if(varname %in% names(get(DataSetName.DNR))) cat(" n_distinct(",DataSetName.DNR,"$",varname,") == ",n_distinct(get(DataSetName.DNR)[[varname]]),"  \n", sep="")}
...
cat("    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%    \n"); 
DataSetName.Date.NA = DataSetName.Date |> paste0(".NA"); 
DataSetName.DNR = paste0(DataSetName.Date,".NA",".rmAllNA"); 
DataSetName.DNR.lbl_pending.fct = paste0(DataSetName.Date,".NA",".rmAllNA",".fct"); 
"assign( DataSetName.DNR.lbl_pending.fct, get(DataSetName.DNR) |> mutate_if(is.character, as.factor) , envir = .GlobalEnv)" |> env1$f$f_CodeText.ECHO(EXECUTE = TRUE, deparse_cat = FALSE, LinePrefix4CodeText = "> ", LinePrefix4Output = "") |> system.time() |> round(3) |> unclass() |> deparse() |> cat("\n")
cat("   #++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++    \n"); 
"get(DataSetName.DNR.lbl_pending.fct) |> select_if(is.factor) |> summary()" |> env1$f$f_CodeText.ECHO(EXECUTE = TRUE, deparse_cat = FALSE, LinePrefix4CodeText = "> ", LinePrefix4Output = "") |> system.time() |> round(3) |> unclass() |> deparse() |> cat("\n")
...
## :: DataSet1.recode = DataSet.DNR.lbl_pending.fct.recode
...
```{r DataSet.Date-recode-filter-n-writeRDS-NoEvalNoEchoNoMsgNoResults, eval=TRUE, echo=FALSE, warning=TRUE, message=NA, results="hide"}
