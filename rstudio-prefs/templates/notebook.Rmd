...
env.custom$tmp$CodeFileName.ext0=rstudioapi::getSourceEditorContext()$path 
for (CodeFileBaseName in rev(c("00env.custom", "01df_dirs_recursive.df_files", "10CodeBook"))) {
    ...
    if(env.custom$tmp$CodeFileName.ext != env.custom$tmp$CodeFileName.ext0) {
        if(!file.exists(env.custom$tmp$CodeFileName.ext)) file.create(env.custom$tmp$CodeFileName.ext)
        rstudioapi::navigateToFile(env.custom$tmp$CodeFileName.ext);
        rstudioapi::navigateToFile(env.custom$tmp$CodeFileName.ext0);
    }
}
```
...
filename.ext0=rstudioapi::getSourceEditorContext()$path; if(basename(filename.ext0) == "templates-00env.custom.minimum.Rmd") {
    ...
    file.copy.destination = file.path(path4APPDATA_RStudio, "templates", "notebook.Rmd"); if(file.copy(from=filename.ext, to=file.copy.destination, overwrite=TRUE)) message(paste0("Update successful: ", file.copy.destination)) else warning(paste0("Update failed: ", file.copy.destination)); rstudioapi::navigateToFile(file.copy.destination);
    file.copy.destination = file.path(env.custom$path$source_base_local, "rstudio-prefs", "templates", "notebook.Rmd"); if(file.copy(from=filename.ext, to=file.copy.destination, overwrite=TRUE)) message(paste0("Update successful: ", file.copy.destination)) else warning(paste0("Update failed: ", file.copy.destination)); rstudioapi::navigateToFile(file.copy.destination);
