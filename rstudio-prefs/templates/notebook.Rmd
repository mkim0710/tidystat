...
?survival::lung
analyticDF_time2event = survival::lung %>% mutate(event = as.logical(status-1), Group = c("Male", "Female")[sex] %>% as.factor, StudyPopulation = time >= 30) %>% dplyr::select(-status, -sex) %>% {cat("> ",deparse(substitute(.))," %>% as_tibble %>% print(n=99)","\n", sep=""); print(as_tibble(.),n=99)}
