...
```{r sizeFiles-NoEchoNoMsgNoResults, echo=FALSE, warning=TRUE, error=TRUE, message=FALSE, results="hide"}
...
#### @ Open R Notebook ------------
```{r open-envCustom-dfDirs, eval=TRUE, include=FALSE}
filename.ext = paste0("00env.custom","-",basename(getwd()),".Rmd"); if(!file.exists(filename.ext)) file.create(filename.ext); filename.ext0=rstudioapi::getSourceEditorContext()$path;rstudioapi::navigateToFile(filename.ext);rstudioapi::navigateToFile(filename.ext0);
filename.ext = paste0("01df_dirs_recursive.df_files","-",basename(getwd()),".Rmd"); if(!file.exists(filename.ext)) file.create(filename.ext); filename.ext0=rstudioapi::getSourceEditorContext()$path;rstudioapi::navigateToFile(filename.ext);rstudioapi::navigateToFile(filename.ext0);
filename.ext = paste0("10CodeBook","-",basename(getwd()),".Rmd"); if(!file.exists(filename.ext)) file.create(filename.ext); filename.ext0=rstudioapi::getSourceEditorContext()$path;rstudioapi::navigateToFile(filename.ext);rstudioapi::navigateToFile(filename.ext0);
```
...
assign(
    DataSetName, 
    survival::lung %>% mutate(event = as.logical(status-1), Group = c("Male", "Female")[sex] %>% as.factor, StudyPopulation = time >= 30) %>% dplyr::select(-status, -sex)
)
...
### \$ paste0(DataSetName,".NA",".rmNotNA0")  ====
```{r, results="markup", collapse=TRUE, paged.print=FALSE, comment="", R.options=list(width=120)}
options(width=120)
...
assign( paste0(DataSetName,".NA"), get(DataSetName) %>% mutate_if(is.factor, as.character) %>%  mutate_if(is.character, na_if, "") %>% mutate_if(is.character, na_if, "N/A") %>% mutate_if(is.character, na_if, "NA") %>% as_tibble )
assign( paste0(DataSetName,".NA",".rmNotNA0"), get(paste0(DataSetName,".NA")) %>% select_if(function(vec) !all(is.na(vec))) )
cat("dim(",DataSetName,") = ",deparse(dim(get(DataSetName))),"\n", sep="") 
cat("dim(",DataSetName,".NA",".rmNotNA0",") = ",deparse(dim(get(paste0(DataSetName,".NA",".rmNotNA0")))),"\n", sep="") 
cat(strrep("#",80),"\n",sep=""); 
cat("> ",DataSetName,".NA"," %>% data.NotNA_p_df","\n", sep=""); data.NotNA_p_df(get(paste0(DataSetName,".NA")))
# https://github.com/mkim0710/tidystat/blob/master/Rdev/data.variable.Date.nID_anyTRUE_allTRUE.dev.r
```


## ------ < DataSet Summary > ------
### \$ paste0(DataSetName,".NA",".rmNotNA0",".fct") ====
```{r, results="markup", collapse=TRUE, paged.print=FALSE, comment="", R.options=list(width=120)}
options(width=120)
vec_varname4ID = c("ID", "PERSON_ID", "RN_INDI", "NIHID")
for (varname in vec_varname4ID) {if(varname %in% names(get(paste0(DataSetName,".NA",".rmNotNA0")))) cat("n_distinct(",paste0(DataSetName,".NA",".rmNotNA0"),"$",varname,") = ",n_distinct(get(paste0(DataSetName,".NA",".rmNotNA0"))[[varname]]),"\n", sep="")}


t0=Sys.time()
cat(strrep("-",80),"\n",sep=""); cat("> ",paste0(DataSetName,".NA",".rmNotNA0")," %>% dplyr::select_if(is.numeric))"," %>% summary","\n", sep=""); get(paste0(DataSetName,".NA",".rmNotNA0")) %>% dplyr::select_if(is.numeric) %>% summary #-----
Sys.time()-t0
t0=Sys.time()
cat(strrep("-",80),"\n",sep=""); cat("> ",paste0(DataSetName,".NA",".rmNotNA0")," %>% dplyr::select_if(is.logical))"," %>% summary","\n", sep=""); get(paste0(DataSetName,".NA",".rmNotNA0")) %>% dplyr::select_if(is.logical) %>% summary #-----
Sys.time()-t0

t0=Sys.time()
cat(strrep("#",80),"\n",sep=""); cat("> ",paste0(DataSetName,".NA",".rmNotNA0",".fct")," = ",paste0(DataSetName,".NA",".rmNotNA0")," %>% mutate_if(is.character, as.factor)","\n", sep=""); assign( paste0(DataSetName,".NA",".rmNotNA0",".fct"), get(paste0(DataSetName,".NA",".rmNotNA0")) %>% mutate_if(is.character, as.factor) )
Sys.time()-t0

t0=Sys.time()
cat(strrep("=",80),"\n",sep=""); cat("> ",paste0(DataSetName,".NA",".rmNotNA0",".fct")," %>% dplyr::select_if(is.factor))"," %>% summary","\n", sep=""); get(paste0(DataSetName,".NA",".rmNotNA0",".fct")) %>% dplyr::select_if(is.factor) %>% summary #-----
Sys.time()-t0
# cat(strrep("-",80),"\n",sep=""); cat("> ",paste0(DataSetName,".NA",".rmNotNA0")," %>% dplyr::select_if(is.factor))"," %>% summary","\n", sep=""); get(paste0(DataSetName,".NA",".rmNotNA0")) %>% select_if(is.character) %>% map_df(as.factor) %>% summary #-----
# Sys.time()-t0
...
filename.ext = "templates-00env.custom.minimum.Rmd"; if(file.exists(filename.ext)) {
    ...
    filename.ext0=rstudioapi::getSourceEditorContext()$path;
    rstudioapi::navigateToFile(file.path(path4APPDATA_RStudio, "templates", "notebook.Rmd"));
    rstudioapi::navigateToFile(filename.ext0);
}
