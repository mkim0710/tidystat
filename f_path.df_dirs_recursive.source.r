...
f_path.df_dirs_recursive(max_depth = 2) %>% as.tibble
f_path.df_dirs_recursive("Rdev", max_depth = 2) %>% as.tibble
...
df_dirs_recursive %>% select(path.level, path.parent, path.basename, full_path) %>% as.tibble %>% print(n=99) #----
df_dirs_recursive %>% select(print_path_tree) %>% as.tibble
df_dirs_recursive %>% select(print_path_tree) %>% unlist %>% paste(collapse = "\n") %>% cat; cat("\n")  #----
