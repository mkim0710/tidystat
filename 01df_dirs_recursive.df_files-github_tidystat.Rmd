...
(for (i.file in files.selected[1]) {
    ...
    .FileNameExt4write = i.objectname.lbl.fct %>% paste0(".rds.xz")
    .path4write = env1$path$path1
    env1$f$f_objectname.size.write_rds.git_lfs_track_add_f(.objectname = i.objectname.lbl.fct, .path_file = NULL, .path4write = NULL, .FileNameExt4write = .FileNameExt4write, createBACKUP = TRUE, .BACKUP_to_path="-BACKUP", EXECUTE = FALSE, path.size_files = TRUE, git_lfs_track = TRUE, git_add_f = TRUE, CompressionMethod = "xz", VERBOSE = isTRUE(getOption("verbose")))
    (Sys.time() - .t0) |> print()
