...
env1$info$DocumentTitle0 = paste0("01df_dirs_recursive.df_files","-",basename(getwd()))
...
# env1$path$.path4write = file.path(env1$path$path1, paste0(env1$path$project_base, env1$path$data_suffix, env1$path$project_suffix))
...
.path.level.max = 5
.sourcename = "f_path.df_dirs_recursive.df_files" |> paste0(".source.r"); .subpath=r"()"|>str_replace_all("\\\\","/"); .subpath_filename.source.r = .sourcename %>% paste0(.subpath,ifelse(.subpath=="","","/"),.); if(isTRUE(options()$function.reload) || !.sourcename %in% names(.GlobalEnv$env1$source)) {  message('> source("',file.path(env1$path$source_base,.subpath_filename.source.r),'")'); .GlobalEnv$env1$source[[.sourcename]] = file.path(env1$path$source_base,.subpath_filename.source.r); suppressPackageStartupMessages(source(.GlobalEnv$env1$source[[.sourcename]]))  }
...
env1$path$print_tree_path = env1$path$df_dirs_recursive.df_files |> dplyr::filter(path.level <= .path.level.max) |> 
    dplyr::select(print_tree_path) |> unlist() |> paste(collapse="") 
# env1$path$print_tree_path |> cat("  \n", sep="")
env1$path$print_tree_path_files.codes = env1$path$df_dirs_recursive.df_files |> dplyr::filter(path.level <= .path.level.max) |> 
    dplyr::select(print_tree_path_files.codes) |> unlist() |> paste(collapse="") 
# env1$path$print_tree_path_files.codes |> cat("  \n", sep="")
env1$path$print_tree_path_files.data = env1$path$df_dirs_recursive.df_files |> dplyr::filter(path.level <= .path.level.max) |> 
    dplyr::select(print_tree_path_files.data) |> unlist() |> paste(collapse="") 
# env1$path$print_tree_path_files.data |> cat("  \n", sep="")
```
  
  
##### env1\$path\$df_dirs_recursive.df_files\$path ----  
...
##### env1\$path\$print_tree_path_files.codes ----  
```{r, echo=FALSE, results="markup", collapse=TRUE, paged.print=FALSE}
env1$path$print_tree_path_files.codes |> cat("  \n", sep="")
```
  
##### env1\$path\$print_tree_path_files.data ----  
```{r, echo=FALSE, results="markup", collapse=TRUE, paged.print=FALSE}
env1$path$print_tree_path_files.data |> cat("  \n", sep="")
```
...
# .sourcename = "f_path.size_files" |> paste0(".source.r"); .subpath=r"()"|>str_replace_all("\\\\","/"); .subpath_filename.source.r = .sourcename %>% paste0(.subpath,ifelse(.subpath=="","","/"),.); if(isTRUE(options()$function.reload) || !.sourcename %in% names(.GlobalEnv$env1$source)) {  message('> source("',file.path(env1$path$source_base,.subpath_filename.source.r),'")'); .GlobalEnv$env1$source[[.sourcename]] = file.path(env1$path$source_base,.subpath_filename.source.r); suppressPackageStartupMessages(source(.GlobalEnv$env1$source[[.sourcename]]))  }
.sourcename = "f_DataSetName.Search.read.checkEntity" |> paste0(".source.r"); .subpath=r"()"|>str_replace_all("\\\\","/"); .subpath_filename.source.r = .sourcename %>% paste0(.subpath,ifelse(.subpath=="","","/"),.); if(isTRUE(options()$function.reload) || !.sourcename %in% names(.GlobalEnv$env1$source)) {  message('> source("',file.path(env1$path$source_base,.subpath_filename.source.r),'")'); .GlobalEnv$env1$source[[.sourcename]] = file.path(env1$path$source_base,.subpath_filename.source.r); suppressPackageStartupMessages(source(.GlobalEnv$env1$source[[.sourcename]]))  }
...
path.size_files = env1$f$f_path.size_files(.path4read = .path4read)

files.selected = 
    path.size_files %>% 
    filter(filename %>% str_detect("\\.sas7bdat$") %>% `!`) %>% 
    filter(filename %>% str_detect("MetaData") %>% `!`) %>% 
    filter(filename %>% str_detect("tblVarName.rds") %>% `!`) %>% 
    filter(filename %>% str_detect("CODEBOOK") %>% `!`) %>% 
    pull(filename) 
files.selected %>% dput
...
##@ for (i.file in files.selected[1]) { ----
```{r, echo=FALSE, results="markup", collapse=TRUE, paged.print=FALSE, comment="#", R.options=list(width=120)}
options(width=120)
files.selected %>% dput

function4files.selected = function(df) {
    stop()
}
##@ for (i.file in files.selected) { ----
.t0 = Sys.time()
(for (i.file in files.selected[1]) {
    i.objectname.lbl.fct = i.file %>% str_replace("\\.xz$", "") %>% str_replace("\\.rds$", "") %>% 
        paste0(".lbl.fct")
    assign(i.objectname.lbl.fct, 
           readRDS(i.file) |> function4files.selected()
    )

    i.objectname.lbl.fct |> print()
    paste0("object.size(get(i.objectname.lbl.fct))", " == ", object.size(get(i.objectname.lbl.fct))) |> print()
    
    .filename.ext4write = i.objectname.lbl.fct %>% paste0(".rds.xz")
    .path4write = env1$path$path1
    env1$f$f_objectname.size.write_rds.git_lfs_track_add_f(.object = NULL, .objectname = i.objectname.lbl.fct, .path_file = NULL, .path4write = NULL, .filename.ext4write = .filename.ext4write, createBackup = TRUE, .backup_to_path="-backup", Execute = FALSE, path.size_files = TRUE, git_lfs_track = TRUE, git_add_f = TRUE, CompressionMethod = "xz", VERBOSE = options()$verbose)
    (Sys.time() - .t0) |> print()
}) |> system.time() |> round(3) |> unclass() |> deparse() |> cat("\n")
```
...
# cat("    ________________________________________________________________________    \n")
env1$path$lastSourceEditorContext.path_filename.ext %>% {paste0(env1$path$path1,"/",.)} %>% {cat('  Sys.setenv(PARENT_RENDERING = "YES"); rmarkdown::render(input = ',deparse(.),', output_dir = ',deparse(dirname(.)),', output_format = "html_document"); Sys.setenv(PARENT_RENDERING = "NO")  \n', sep="")}; env1$path$lastSourceEditorContext.path_filename.html %>% {paste0(env1$path$path1,"/",.)} %>% cat('  "',.,'" |> env1$env.internal$f_filename.ext.createBackup(.backup_to_path="-backup", timeFormat="%y%m%d", overwrite=TRUE)  \n', sep=""); env1$path$lastSourceEditorContext.path_filename.html %>% {paste0(env1$path$path1,"/",.)} %>% cat('  "',.,'" |> env1$env.internal$f_URL.open_in_edge_app()  \n', sep="")
```
