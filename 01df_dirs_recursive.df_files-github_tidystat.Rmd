...
.t0 = Sys.time()
(for (i.file in files.selected[1]) {
    i.objectname.labelled.fct = i.file %>% str_replace("\\.xz$", "") %>% str_replace("\\.rds$", "") %>% 
        paste0(".labelled.fct")
    assign(i.objectname.labelled.fct, 
           readRDS(i.file) |> function4files.selected()
    )

    i.objectname.labelled.fct |> print()
    paste0("object.size(get(i.objectname.labelled.fct))", " == ", object.size(get(i.objectname.labelled.fct))) |> print()
    
    .filename.ext4write = i.objectname.labelled.fct %>% paste0(".rds.xz")
    .path4write = env1$path$path1
    env1$f$f_objectname.size.write_rds.git_lfs_track_add_f(.object = NULL, .objectname = i.objectname.labelled.fct, .path_file = NULL, .path4write = NULL, .filename.ext4write = .filename.ext4write, createBackup = TRUE, .backup_to_path="-backup", Execute = TRUE, path.size_files = TRUE, git_lfs_track = TRUE, git_add_f = TRUE, CompressionMethod = "xz", VERBOSE = options()$verbose)
    (Sys.time() - .t0) |> print()
