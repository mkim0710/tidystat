...
data <- data %>%
  mutate(
    A01_VISIT.cum = ifelse(A01_VISIT == "참여", 1, 0),
    A02_VISIT.cum = A01_VISIT.cum + ifelse(A02_VISIT == "참여", 1, 0),
    A03_VISIT.cum = A02_VISIT.cum + ifelse(A03_VISIT == "참여", 1, 0),
    A04_VISIT.cum = A03_VISIT.cum + ifelse(A04_VISIT == "참여", 1, 0),
    A05_VISIT.cum = A04_VISIT.cum + ifelse(A05_VISIT == "참여", 1, 0),
    A06_VISIT.cum = A05_VISIT.cum + ifelse(A06_VISIT == "참여", 1, 0),
    A07_VISIT.cum = A06_VISIT.cum + ifelse(A07_VISIT == "참여", 1, 0)
  )
data = data %>% as_tibble
data
```
...
```{r, echo=TRUE, results="markup", collapse=TRUE, paged.print=FALSE, comment="#", R.options=list(width=120)}
options(width=120)
# DS %>% select(matches("_VISIT$"), matches("_VISIT.cum$")) %>% select(matches("A0[1-4]"))

DS %>% select(matches("_VISIT$"))
DS %>% select(matches("_VISIT$")) %>% map_df(as.factor) %>% summary()
DS %>% select(matches("_VISIT.cum$"))
DS %>% select(matches("_VISIT.cum$")) %>% map_df(as.factor) %>% summary()
# > DS %>% select(matches("_VISIT$"))
# # A tibble: 5 × 7
