...
env.custom$DocumentTitle0 = "Blank) env.custom.fun.t.tribble_construct"
...
#% env.custom$get_system_info ====
```{r}
env.custom$get_system_info = function() {
    summary_list = list(
        GUI = .Platform$GUI,
...
env.custom$get_system_info = env.custom$get_system_info()
str(env.custom$get_system_info)
```
...
#% env.custom$data_suffix ====
#% env.custom$project_suffix ====
```{r}
library(tidyverse)
getwd() %>% dput #----
...
path4read = env.custom$path1
# path4write = env.custom$path2
...
# write_rds( eval(parse(text = objectname)), file.path(path4write, paste0(objectname, ".rds")), compress = "gz" )
