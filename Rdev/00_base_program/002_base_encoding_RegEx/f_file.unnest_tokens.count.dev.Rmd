...
#### \% |> unnest_tokens() |> group_by(token_chr) %>% summarise()
```{r, echo=TRUE, results="markup", collapse=TRUE, paged.print=FALSE, comment="#", R.options=list(width=120)}
input_vec_chr.df %>%
    unnest_tokens(
        output = "token_chr",
    ...
    if (use_tidytext) {
        ...
        input_vec_chr.df.unnest_tokens.count <- input_vec_chr.df %>%
            unnest_tokens(
                output = "token_chr",
                input = input_vec_chr,
                token = "regex", pattern = "[^a-zA-Z0-9._]+",
                to_lower = FALSE, drop = FALSE, collapse = NULL
            ) %>%
            group_by(token_chr) %>% summarise(n = n(), rowNum = paste0(rowNum, collapse = "|")) %>% arrange(desc(n))
    } else {
...
#
# Returns:
#   A tibble with columns 'rowNum', 'word', and 'n' indicating the frequency
...
f_file.unnest_tokens.count = function(
    ...
    inputChrVec_df <- tibble(
        rowNum = as.integer(seq_along(input_chrVec)),
        input_vec_chr = input_chrVec
    )
