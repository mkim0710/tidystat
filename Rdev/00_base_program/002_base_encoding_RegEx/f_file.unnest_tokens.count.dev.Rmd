...
env1[[.tmp$env1_subenv_name]][[.tmp$objectname]] = # Install and load necessary libraries
install.packages("tidytext")
library(tidytext)
library(dplyr)

# Define the function to count word frequencies
count_word_frequencies <- function(file_path, use_tidytext = TRUE, exclude_comments = TRUE) {
    # Read the source code file
    source_code <- readLines(file_path, warn = FALSE)
    ...
    if (use_tidytext) {
        # Create a tibble with line numbers and text content
        source_code_df <- tibble(line_number = seq_along(source_code), text = source_code)
        
        # Tokenize words with a regex that allows dots and underscores
        word_frequencies <- source_code_df %>%
            unnest_tokens(word, text, token = "regex", pattern = "\\b[a-zA-Z0-9._]+\\b") %>%
            count(line_number, word, sort = TRUE) %>%
            arrange(line_number, desc(n))
        
    } else {
        # Process with gsub
        # Extract words line by line
        word_frequencies <- lapply(seq_along(source_code), function(line_num) {
            words <- unlist(str_extract_all(source_code[line_num], "\\b[a-zA-Z0-9._]+\\b"))
            words <- words[words != ""]  # Filter out empty strings
            data.frame(line_number = line_num, word = words, stringsAsFactors = FALSE)
        }) %>%
            bind_rows() %>%
            count(line_number, word, sort = TRUE) %>%
            arrange(line_number, desc(n))
...
# Example usage with tidytext
file_path <- "your_script.R"  # Replace with your R script file path
word_frequencies_tidytext <- count_word_frequencies(file_path, use_tidytext = TRUE, exclude_comments = TRUE)

# Example usage with gsub
word_frequencies_gsub <- count_word_frequencies(file_path, use_tidytext = FALSE, exclude_comments = TRUE)

# Display the word frequencies
print("Word Frequencies Using Tidytext:")
print(word_frequencies_tidytext)

print("Word Frequencies Using Gsub:")
