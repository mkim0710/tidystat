...
env1[[.tmp$env1_subenv_name]][[.tmp$objectname]] = function(input_path_file, use_tidytext = TRUE, exclude_comments = TRUE) {
    ...
    if (exclude_comments) {
        ...
        input_vec_chr <- gsub("#.*", "", input_vec_chr)
    }
    ...
    if (use_tidytext) {
        ...
        word_frequencies <- input_vec_chr.df %>%
            unnest_tokens(
                output = "word",
                input = text,
                token = "regex",
        ...
        # Tokenize words using stringr and gsub-based approach
        word_frequencies <- input_vec_chr.df %>%
            rowwise() %>%
            mutate(words = list(
                unlist(str_extract_all(text, "\\b[a-zA-Z0-9._]+\\b"))
            )) %>%
            unnest(words) %>%
...
#   word frequencies. Tokens can be identified using either tidytext or a gsub-based approach.
#
# Parameters:
