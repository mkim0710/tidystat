...
options(width=120)
# References:
# 1. tidytext::unnest_tokens() - https://cran.r-project.org/web/packages/tidytext/tidytext.pdf
...
if (exclude_comments) {
    ...
    input_vec_chr <- input_vec_chr %>% str_replace_all("#.*", "")
}
input_vec_chr %>% str
...
input_vec_chr.df <- tibble(
    line_number = seq_along(input_vec_chr),
...
input_vec_chr.df.unnest_tokens.count <- input_vec_chr.df %>%
    ...
    count(line_number, word, sort = TRUE)
input_vec_chr.df.unnest_tokens.count
```


#### rowwise() |> mutate(str_extract_all) |> unnest(words) |> count()  
```{r, echo=TRUE, results="markup", collapse=TRUE, paged.print=FALSE, comment="#", R.options=list(width=120)}
# Tokenize words using stringr and str_replace_all-based approach
input_vec_chr.df.unnest_tokens.count <- unnestedTokens_df <- input_vec_chr.df %>%
    unnest_tokens(
        output = "word_chr",
        input = input_chr,
        token = "regex",
        pattern = "[^a-zA-Z0-9._]+"  # match these sequences as delimiters (caution~!!)
    ) %>%
    count(line_number_int, word_chr, sort = TRUE)
...
## https://chatgpt.com/c/6772bb5e-3100-800e-a04b-415906b0c197
## :: f_file.unnest_tokens.count = function() -dev -pending  
```{r, echo=TRUE, results="markup", collapse=TRUE, paged.print=FALSE, comment="#", R.options=list(width=120)}
options(width=120)
##%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%  
...
## Rdev/00_base_program/002_base_encoding_RegEx/f_file.unnest_tokens.count.dev.Rmd
.tmp$env1_subenv_name = "env.internal"
.tmp$objectname = "f_file.unnest_tokens.count" 
env1[[.tmp$env1_subenv_name]][[.tmp$objectname]] = function(input_path_file, use_tidytext = TRUE, exclude_comments = TRUE) {
    ...
    if (use_tidytext) {
        ...
        input_vec_chr.df.unnest_tokens.count <- input_vec_chr.df %>%
            ...
            unnest(words) %>%
            count(line_number, word = words, sort = TRUE)
    }
...
###############################################################################
# Function: env1$env.internal$f_file.unnest_tokens.count
# Description:
#   Reads an R script from 'input_path_file', optionally removes comments, and counts 
#   word frequencies. Tokens can be identified using either tidytext or a str_replace_all-based approach.
#
# Parameters:
#   - input_path_file (character): Path to the R script file.
#   - use_tidytext (logical): If TRUE, uses tidytext::unnest_tokens().
#   - exclude_comments (logical): If TRUE, removes R comments (lines starting with '#').
#
# Returns:
#   A tibble with columns 'line_number', 'word', and 'n' indicating the frequency
#   of each token in the specified file.
###############################################################################
...
# Example Usage
###############################################################################
# Replace 'your_script.R' with the path to your R script file.
# input_path_file = rstudioapi::getSourceEditorContext()$path
input_path_file = "Rdev/00_base_program/002_base_encoding_RegEx/FileSample_with_TABLE_OF_CONTENTS.r" |> env1$f$f_path_relative.path_normalized_based_on_path1()  # input_path_file = paste0(env1$path$path1,ifelse(env1$path$path1=="","","/"),input_path_file)


# Using tidytext
word_frequencies_tidytext <- env1$env.internal$f_file.unnest_tokens.count(
  input_path_file,
  use_tidytext = TRUE,
  exclude_comments = TRUE
)

# Using str_replace_all-based approach
word_frequencies_gsub <- env1$env.internal$f_file.unnest_tokens.count(
  input_path_file,
  use_tidytext = FALSE,
  exclude_comments = TRUE
)
...
# Display the Results
###############################################################################
cat("Word Frequencies Using Tidytext:\n")
print(word_frequencies_tidytext)

cat("\nWord Frequencies Using Gsub:\n")
print(word_frequencies_gsub)
```
...
options(width=120)
###############################################################################
# Revised Function: f_file.unnest_tokens.count
#
# This function reads an R script from `input_path_file`, optionally removes
# comments, and tokenizes its words. It follows tidyverse style, respects your
# naming conventions (e.g., .chrVec, .df, .int), and allows for verbose debugging.
###############################################################################
f_file.unnest_tokens.count = function(
    input_path_file,
    use_tidytext = TRUE,
    exclude_comments = TRUE,
    VERBOSE_lgl = isTRUE(getOption("verbose"))
) {
    # Load libraries within function scope (for demonstration)
    library(tidytext)  # For unnest_tokens()
    library(dplyr)     # For pipe operations and count()
    library(stringr)   # For str_extract_all()
    ...
    input_chrVec <- readLines(input_path_file, warn = FALSE)
    
    # Optional debug print
    if (isTRUE(VERBOSE_lgl)) {
        cat("[DEBUG] First 3 lines of input_chrVec:\n")
        print(head(input_chrVec, 3))
    }
    
    # 3) Remove comments if requested
    if (isTRUE(exclude_comments)) {
        input_chrVec <- input_chrVec %>% str_replace_all("#.*", "")
    }
    
    # 4) Create a tibble with integer line numbers
    inputChrVec_df <- tibble(
        line_number_int = as.integer(seq_along(input_chrVec)),
        input_chr = input_chrVec
    )
    
    # Optional debug print
    if (isTRUE(VERBOSE_lgl)) {
        cat("[DEBUG] Head of inputChrVec_df after removing comments:\n")
        print(head(inputChrVec_df, 3))
    }
    
    # 5) Tokenize words
    if (isTRUE(use_tidytext)) {
        # Approach 1: Tidytext
        unnestedTokens_df <- inputChrVec_df %>%
            unnest_tokens(
                output = "word_chr",
                input = input_chr,
                token = "regex",
                pattern = "[a-zA-Z0-9._]+",  # match these sequences
                collapse = FALSE
            ) %>%
            count(line_number_int, word_chr, sort = TRUE)
    } else {
        # Approach 2: Stringr / gsub
        unnestedTokens_df <- inputChrVec_df %>%
            rowwise() %>%
            mutate(words_chrVec = list(
                unlist(str_extract_all(
                    input_chr, 
                    "\\b[a-zA-Z0-9._]+\\b"
                ))
            )) %>%
            unnest(cols = c(words_chrVec)) %>%
            rename(word_chr = words_chrVec) %>%
            count(line_number_int, word_chr, sort = TRUE)
    }
    
    # Another optional debug print
    if (isTRUE(VERBOSE_lgl)) {
        cat("[DEBUG] Head of unnestedTokens_df:\n")
        print(head(unnestedTokens_df, 5))
    }
    
    # 6) Return the tibble
    return(unnestedTokens_df)
}



# Example usage with Tidytext approach and verbose debugging
outputTokens_df <- f_file.unnest_tokens.count(
...
cat("\n=== Partial Output (First 10 Rows) ===\n")
print(head(outputTokens_df, 10))
