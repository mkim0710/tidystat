         ...
         OS_subdirs = .OStype())
{
    ...
    if(type %in% c("code", "docs")) {
        for(os in OS_subdirs) {
            os_dir <- here::here(dir, os)
            if(dir.exists(os_dir)) {
                os_files <- list_files_with_exts(os_dir, exts,
                                                 all.files = all.files,
                                                 full.names = FALSE)
                os_files <- here::here(if(full.names) os_dir else os,
                                      os_files)
...
path_FileName = path_FileNameExt |> basename() |> str_remove("\\.([[:alnum:]]+)$")
# ext = path_FileNameExt |> basename() |> str_extract("\\.([[:alnum:]]+)$") |> str_replace("^\\.", "")
ext = path_FileNameExt |> basename() |> str_extract("\\.([[:alnum:]]+)$") |> str_remove("^\\.")
```
...
### :: f_path.df_FileNameExt_extended = function(input_path, vec_RegEx4ext_extended = c("\\.[^.]*\\.[gx]z$", "\\.nb\\.html$")) ----  
```{r, echo=TRUE, results="markup", collapse=TRUE, paged.print=FALSE, comment="#", R.options=list(width=120)}
options(width=120)
library(stringr)
...
env1$f$f_path.df_FileNameExt_extended(input_path)
