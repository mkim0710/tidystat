...
#'
#' @param input_str1 A string containing the first regex character class, e.g. "[abc]".
#' @param input_str2 A string containing the second regex character class, e.g. "[xyz]".
...
#'   \item{union}{Union of the characters from \code{input_str1} and \code{input_str2}.}
#'   \item{intersect}{Intersection of the characters.}
#'   \item{setdiff_1_2}{Characters in \code{input_str1} but not in \code{input_str2}.}
#'   \item{setdiff_2_1}{Characters in \code{input_str2} but not in \code{input_str1}.}
#'
...
#'
#' @export
f_str1_str2.compareCharClasses_list <- function(input_str1, 
                                                input_str2, 
    ...
    input_str1_noBrackets_chr <- str_remove_all(input_str1, "^\\[|\\]$")
    input_str2_noBrackets_chr <- str_remove_all(input_str2, "^\\[|\\]$")
    
    # 2) Split into individual characters and keep them unique
    set1_vec <- input_str1_noBrackets_chr %>%
        str_split(pattern = "", n = Inf) %>%
        unlist() %>%
        unique()
    
    set2_vec <- input_str2_noBrackets_chr %>%
        str_split(pattern = "", n = Inf) %>%
        unlist() %>%
        unique()
    ...
    if (VERBOSE) {
        ...
        print(list(input_str1_noBrackets_chr = input_str1_noBrackets_chr,
                   input_str2_noBrackets_chr = input_str2_noBrackets_chr))
        print("Sets from each input:")
        print(list(set1_vec = set1_vec, set2_vec = set2_vec))
    }
    
    # 3) Perform set operations
    union_vec       <- union(set1_vec, set2_vec)
    intersect_vec   <- intersect(set1_vec, set2_vec)
    setdiff_1_2_vec <- setdiff(set1_vec, set2_vec)
    setdiff_2_1_vec <- setdiff(set2_vec, set1_vec)
    ...
    if (output_as_data_frame) {
        df_out <- tibble(
            ...
            vec1            = union_vec %in% set1_vec,
            vec2            = union_vec %in% set2_vec,
        ...
        return(
            list(
                vec1.character = set1_vec, 
                vec2.character = set2_vec, 
                union       = union_vec,
...
### \% f_str1_str2.compareCharClasses_list(RegExChrClass_strings_cheatsheet, ) 
```{r, echo=FALSE, results="markup", collapse=TRUE, paged.print=FALSE, comment="", R.options=list(width=120)}
options(width=120)
RegExChrClass_strings_cheatsheet = "[]$*+.?[^{|(\\#%&~_/<=>'!,:;`\")}@-]"
RegExChrClass_gpt_o1 = "[.\\\\+*?^$(){}|\\[\\]\\-]"
RegExChrClass_gpt_o1p = "[.*+?^=!:|\\\\/()\\[\\]{}]"
cat("    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%    \n")
f_str1_str2.compareCharClasses_list(RegExChrClass_strings_cheatsheet, RegExChrClass_gpt_o1) %>% str
f_str1_str2.compareCharClasses_list(RegExChrClass_strings_cheatsheet, RegExChrClass_gpt_o1)$union %>% dput.deparse.cat0()
f_str1_str2.compareCharClasses_list(RegExChrClass_strings_cheatsheet, RegExChrClass_gpt_o1)$intersect %>% dput.deparse.cat0()
cat("   ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++    \n")
...
RegExChrClass_gpt_o1p = "[.*+?^=!:|\\\\/()\\[\\]{}]"
cat("    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%    \n")
vec_chr.ASCII_nonalphanumeric |> f_str1_str2.compareCharClasses_list(RegExChrClass_strings_cheatsheet) %>% str
vec_chr.ASCII_nonalphanumeric |> f_str1_str2.compareCharClasses_list(RegExChrClass_strings_cheatsheet) %>% str
vec_chr.ASCII_nonalphanumeric |> f_str1_str2.compareCharClasses_list(RegExChrClass_strings_cheatsheet) %>% str
```
