...
# __________|------  
# @@ START) dev) f_chr.f_path_shell.escapeSpecialCharacters ----  
## :: vec_path.test =
```{r, echo=TRUE, results="markup", collapse=TRUE, paged.print=FALSE, comment="#", R.options=list(width=120)}
options(width=120)
library(stringr)
...
### \% f_path_shell.escapeSpecialCharacters.str_replace_all() 
```{r, echo=TRUE, results="markup", collapse=TRUE, paged.print=FALSE, comment="#", R.options=list(width=120)}
# Function to escape special characters in a string for use in regex
f_path_shell.escapeSpecialCharacters.str_replace_all <- function(input_path) {
  input_path |>  str_replace_all("([]\\[{}()+*?^$|.-])", "\\\\\\1")
}

result.str_replace_all = vec_path.test |> map(f_path_shell.escapeSpecialCharacters.str_replace_all)
result.str_replace_all |> str(max.level = 2, give.attr = TRUE)
```


### \% f_path_shell.escapeSpecialCharacters.str_replace_all_v2() 
```{r, echo=TRUE, results="markup", collapse=TRUE, paged.print=FALSE, comment="#", R.options=list(width=120)}
# Function to escape special characters in a string for use in regex
f_path_shell.escapeSpecialCharacters.str_replace_all_v2 <- function(input_path) {
  input_path |>  str_replace_all("([\\[\\]{}()+*?^$|.-])", "\\\\\\1")
}

result.str_replace_all = vec_path.test |> map(f_path_shell.escapeSpecialCharacters.str_replace_all_v2)
result.str_replace_all |> str(max.level = 2, give.attr = TRUE)
result.str_replace_all |> identical(result.str_replace_all)
```
  
  
### \% f_path_shell.escapeSpecialCharacters.gsub() 
```{r, echo=TRUE, results="markup", collapse=TRUE, paged.print=FALSE, comment="#", R.options=list(width=120)}
options(width=120)
library(stringr)
# Function to escape special characters in a string for use in regex
f_path_shell.escapeSpecialCharacters.gsub <- function(input_path) {
  gsub("([][{}()+*?^$|.-])", "\\\\\\1", input_path)
}

result.gsub = vec_path.test |> map(f_path_shell.escapeSpecialCharacters.gsub)
result.gsub |> str(max.level = 2, give.attr = TRUE)
result.gsub |> identical(result.str_replace_all)
```


### \% f_path_shell.escapeSpecialCharacters.gsub_v2() 
```{r, echo=TRUE, results="markup", collapse=TRUE, paged.print=FALSE, comment="#", R.options=list(width=120)}
options(width=120)
# library(stringr)
# 
# # Function to escape special characters in a string for use in regex
# f_path_shell.escapeSpecialCharacters.gsub_v2 <- function(input_path) {
#     # gsub("([\\[\\]{}()+*^$|.])", "\\\\\\1", string)
#     # gsub("([\\[\\]\\{\\}()+*^$|.])", "\\\\\\1", string)
#     gsub("([\\[\\]\\{\\}()+*?^$|.])", "\\\\\\1", string)
...
# }
# 
# result.gsub_v2 = vec_path.test |> map(f_path_shell.escapeSpecialCharacters.gsub_v2)
# result.gsub_v2 |> str(max.level = 2, give.attr = TRUE)
...
# # Backtrace:
# #  1. purrr::map(vec_path.test, f_path_shell.escapeSpecialCharacters.gsub_v2)
...
### \% f_path_shell.escapeSpecialCharacters.manual() 
```{r, echo=TRUE, results="markup", collapse=TRUE, paged.print=FALSE, comment="#", R.options=list(width=120)}
options(width=120)
f_path_shell.escapeSpecialCharacters.manual <- function(input_path) {
    specials <- c("\\", ".", "?", "+", "*", "^", "$", "(", ")", "[", "]", "{", "}", "|", "-")
...
result.manual <- vec_path.test |> map(f_path_shell.escapeSpecialCharacters.manual)
result.manual |> str(max.level = 2, give.attr = TRUE)
...
vec_path.test |> map(file_name_without_shell_chars) |> str(max.level = 2, give.attr = TRUE)


improved_shell_regex <- "[<>()|&;!$`\\\\'\"*?\\[\\]{}~#^%+=,:/@]"
# Function to escape special characters in a string for use in regex
f_path_shell.escapeSpecialCharacters.str_replace_all_improved <- function(input_path) {
  input_path |>  str_replace_all(improved_shell_regex, "\\\\\\&")
}

result.str_replace_all_improved = vec_path.test |> map(f_path_shell.escapeSpecialCharacters.str_replace_all_improved)
result.str_replace_all_improved |> str(max.level = 2, give.attr = TRUE)
```
  
  
### \% f_path_shell.escapeSpecialCharacters.shell_chars_regex() 
```{r, echo=TRUE, results="markup", collapse=TRUE, paged.print=FALSE, comment="#", R.options=list(width=120)}
options(width=120)
...
f_path_shell.escapeSpecialCharacters.shell_chars_regex <- function(input_path) {
  input_path |>  str_replace_all(.shell_chars_regex, "\\\\\\&")
}

f_path_shell.escapeSpecialCharacters.shell_chars_regex = vec_path.test |> map(f_path_shell.escapeSpecialCharacters.shell_chars_regex)
f_path_shell.escapeSpecialCharacters.shell_chars_regex |> str(max.level = 2, give.attr = TRUE)
```


##@ https://chatgpt.com/c/66f601c7-8c30-800e-8e13-499fee371a88 ----  
## :: improved_shell_regex <- "[<>()|&;!$`\\\\'\"*?\\[\\]{}~#^%+=,:/@]"
### \% f_path_shell.escapeSpecialCharacters.gsub_improved() 
```{r, echo=TRUE, results="markup", collapse=TRUE, paged.print=FALSE, comment="#", R.options=list(width=120)}
options(width=120)
improved_shell_regex <- "[<>()|&;!$`\\\\'\"*?\\[\\]{}~#^%+=,:/@]"
# Function to escape special characters in a string for use in regex
...
# improved_shell_regex <- "[<>()|&;!$`\\\\'\"*?\\[\\]{}~#^%+=,:/@]"
# # Function to escape special characters in a string for use in regex
# f_path_shell.escapeSpecialCharacters.gsub_improved <- function(input_path) {
#   gsub(improved_shell_regex, "\\\\\\&", string)
# }
# 
# result.gsub_improved = vec_path.test |> map(f_path_shell.escapeSpecialCharacters.gsub_improved)
# result.gsub_improved |> str(max.level = 2, give.attr = TRUE)
...
# # Backtrace:
# #  1. purrr::map(vec_path.test, f_path_shell.escapeSpecialCharacters.gsub_improved)
# #  2. purrr:::map_("list", .x, .f, ..., .progress = .progress)
# #  6. global .f(.x[[i]], ...)
# #  7. base::gsub(improved_shell_regex, "\\\\\\1", string)
# # Error in map(vec_path.test, f_path_shell.escapeSpecialCharacters.gsub_improved) : 
# # Caused by error in `gsub()`:
# # ! invalid regular expression '[<>()|&;!$`\\'"*?\[\]{}~#^%+=,:/@]', reason 'Invalid contents of {}'
```
...
##@ https://chatgpt.com/c/6703f128-b4b4-800e-b718-68b28c7de362 ----  
###### ECHO \% |> () ----  
```{r, echo=TRUE, results="markup", collapse=TRUE, paged.print=FALSE, comment="#", R.options=list(width=120)}
options(width=120)
# document.title.replace(/[^a-zA-Z0-9가-힣 \-_().,@+=;&]/g, '_');
# document.title.replace(/[^a-zA-Z0-9가-힣 \-_().,@+=;&!#$%^\[\]'~`]/g, '_');
