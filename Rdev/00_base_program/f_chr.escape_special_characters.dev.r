...
suppressPackageStartupMessages({for(.tmp$packagename in c("tidyverse","formatR")) {if(!require(.tmp$packagename,character.only=TRUE))install.packages(.tmp$packagename) else library(.tmp$packagename,character.only=TRUE)}});cat("  \n", sep="")
if(!exists("env1", envir=.GlobalEnv)) 
    assign("env1", new.env(), envir=.GlobalEnv)
## env1\$path ============  
