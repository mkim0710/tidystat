...
## \% substitute(dim(get(DSN)), list(DSN = DSN))  
...
ADS_time2event <- data.frame(matrix(1:12, nrow=228, ncol=12))

# Step 1: Define the expression
expr <- quote(dim(get(DSN)))
...
substituted_expr <- substitute(dim(get(DSN)), list(DSN = DSN))
...
# Step 2: Use substitute to replace the variable name with its actual value as a string
# substituted_expr <- substitute(as.expression(.text_expression), list(.text_expression = .text_expression, DSN = DSN))
# substituted_expr <- substitute(expr, list(expr = expr, .text_expression = .text_expression, DSN = DSN))
substituted_expr <- substitute(eval(substitute(expr, list(expr = expr))), list(expr = expr, .text_expression = .text_expression, DSN = DSN))
cat("\n@ Step 2: Substituted expression  \n")
cat(" > print(", deparse(substitute(substituted_expr)), "); str(", deparse(substitute(substituted_expr)), ")  \n", sep="")
print(substituted_expr)
str(substituted_expr)
...
substitute_in_env <- function(expr, name) {
  ...
  substituted_expr <- substitute(expr, list(DSN = as.name(name)))
...
substituted_expr <- substitute_in_env(expr, DSN)
cat("\n@ Step 2: Substituted expression  \n")
cat(" > print(", deparse(substitute(substituted_expr)), "); str(", deparse(substitute(substituted_expr)), ")  \n", sep="")
print(substituted_expr)
str(substituted_expr)
...
substitute_and_evaluate <- function(expr, dataset_name) {
  ...
  substituted_expr <- substitute(expr, list(DSN = as.name(dataset_name)))
...
result <- substitute_and_evaluate(expr, DSN)
...
options(width=120)
substitute_and_evaluate <- function(expr, dataset_name) {
  # Substitute DSN in the expression
  substituted_expr <- substitute(expr, list(DSN = dataset_name))
...
substitute_and_evaluate <- function(expr, dataset_name) {
  # Substitute DSN in the expression
  substituted_expr <- eval(substitute(substitute(expr, list(DSN = as.name(dataset_name)))))
...
substitute_and_evaluate <- function(expr, dataset_name) {
  ...
  evaluated_expr <- eval(substitute(expr, list(DSN = as.name(dataset_name))))
...
substitute_and_evaluate <- function(expr, dataset_name) {
  ...
  substituted_expr <- custom_substitute(expr, list(DSN = dataset_name))
...
### \% gsub("DSN", paste0('"', dataset_name, '"'), expr_text)  
```{r, echo=TRUE, results="markup", collapse=TRUE, paged.print=FALSE, comment="#", R.options=list(width=120)}
options(width=120)
substitute_and_evaluate <- function(expr_text, dataset_name) {
  ...
  substituted_text <- gsub("DSN", paste0('"', dataset_name, '"'), expr_text)
...
expr_text <- "dim(get(DSN))"

# Use the function to substitute and evaluate the expression
result <- substitute_and_evaluate(expr_text, DSN)
