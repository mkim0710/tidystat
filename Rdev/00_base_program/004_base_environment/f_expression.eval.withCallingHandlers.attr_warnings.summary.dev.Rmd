...
warnings()
warnings() |> str(max.level = 2, give.attr = TRUE)
warnings() %>% dput
# > warnings()
# Warning messages:
# 1: In matrix(1:7, 3, 4) :
...
summary(warnings())
summary(warnings()) |> str(max.level = 2, give.attr = TRUE)
...
# __________|------  
# @@ START) function ----  
## :: f_expression.eval.withCallingHandlers.list_warnings.summary = function() -dev  
```{r, echo=TRUE, results="markup", collapse=TRUE, paged.print=FALSE, comment="#", R.options=list(width=120)}
options(width=120)
f_expression.eval.withCallingHandlers.list_warnings.summary <- function(expression.eval) {
  ...
  result <- tryCatch(
    {
      withCallingHandlers(
        { expression.eval }, # Run the expression.eval provided as an argument
        warning = function(w) {
          # Capture warnings
...
## :: f_expression.eval.withCallingHandlers.list_warnings.summary = function() -dev -success  
```{r, echo=TRUE, results="markup", collapse=TRUE, paged.print=FALSE, comment="#", R.options=list(width=120)}
options(width=120)
f_expression.eval.withCallingHandlers.list_warnings.summary <- function(expression.eval) {
  # Save the current 'warn' option
  ...
  # Return the result and captured warnings
  list(result = result, warnings = list_warning, summary = list_warnings.summary)
}
...
## :: f_expression.eval.withCallingHandlers.attr_warnings.summary = function() -dev -success  
```{r, echo=TRUE, results="markup", collapse=TRUE, paged.print=FALSE, comment="#", R.options=list(width=120)}
options(width=120)
...
# Run the example function with suppressed warnings
output <- f_expression.eval.withCallingHandlers.attr_warnings.summary(example_function(), warnings.summary_only = FALSE, warnings.summary.print = TRUE)
