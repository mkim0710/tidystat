...
kormaps2014_kormap321 = kormap3 |> rename(adm_dr_nm.UTF8 = name, adm_dr_nm.CP949 = name1) %>% 
    ...
    mutate(SIDO_CD = code %>% substr(1, 2) %>% as.factor) %>% 
    left_join(kormap1 %>% mutate(SIDO_NM.UTF8 = name) %>% select(SIDO_CD, SIDO_NM, SIDO_NM.UTF8) %>% distinct)
# > kormaps2014_kormap21 = kormap2 %>% 
# +     mutate(SIDO_CD = code %>% substr(1, 2) %>% as.factor) %>% 
# +     left_join(kormap1 %>% mutate(SIDO_NM.UTF8 = name) %>% select(SIDO_CD, SIDO_NM, SIDO_NM.UTF8) %>% distinct)
# Joining, by = "SIDO_CD"
# > kormaps2014_kormap321 = kormap3 %>% 
# +     mutate(sigungu_cd = code %>% substr(1, 5) %>% as.factor) %>% 
# +     left_join(kormap2 %>% mutate(sigungu_nm.UTF8 = name) %>% select(sigungu_cd, sigungu_nm, sigungu_nm.UTF8) %>% distinct) %>% 
# +     mutate(SIDO_CD = code %>% substr(1, 2) %>% as.factor) %>% 
# +     left_join(kormap1 %>% mutate(SIDO_NM.UTF8 = name) %>% select(SIDO_CD, SIDO_NM, SIDO_NM.UTF8) %>% distinct)
# Joining, by = "sigungu_cd"
# Joining, by = "SIDO_CD"
...
#  $ sigungu_nm.UTF8 : Factor w/ 229 levels "媛\u0080\xed룊援\xb0",..: 180 180 180 180 180 180 180 180 180 180 ...
#  $ sigungu_nm.CP949: Factor w/ 229 levels "가평군","강남구",..: 182 182 182 182 182 182 182 182 182 182 ...
...
#  $ adm_dr_nm.UTF8 : Factor w/ 3128 levels "媛\u0080寃쎈룞",..: 1266 1266 1266 1266 1266 1266 1266 1266 1266 1338 ...
#  $ adm_dr_nm.CP949: Factor w/ 3128 levels "첨단1동","첨단2동",..: 1259 1259 1259 1259 1259 1259 1259 1259 1259 1320 ...
