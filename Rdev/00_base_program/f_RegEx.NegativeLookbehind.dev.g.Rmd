...
# @@ TRE vs PCRE vs ICU ----  
## https://chatgpt.com/c/8f370edc-8df9-4ca9-96c1-e26d9bbb6ff9  
```{r, echo=TRUE, results="markup", collapse=TRUE, paged.print=FALSE, comment="", R.options=list(width=120)}
options(width=120)
...
cat("    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%    \n")
### Example 1: Dot `.` Behavior
cat('### Example 1: Dot `.` Behavior', '"    \n', sep="")
text <- "line1\nline2
line3"
cat("  > ",'text', '\n    "', text, '"    \n', sep="")

cat("  > ", 'gsub(".+", "replacement", text)', '\n    "', gsub(".+", "replacement", text), '"    \n', sep="")
cat("  > ", 'gsub(".+", "replacement", text, perl = TRUE)', '\n    "', gsub(".+", "replacement", text, perl = TRUE), '"    \n', sep="")
cat("  > ", 'gsub("(?s).+", "replacement", text, perl = TRUE)', '\n    "', gsub("(?s).+", "replacement", text, perl = TRUE), '"    \n', sep="")
cat("  > ", 'str_replace_all(text, ".+", "replacement")', '\n    "', str_replace_all(text, ".+", "replacement"), '"    \n', sep="")



cat("    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%    \n")
### Example 2: Bracket Expressions and Shorthand Classes
cat('### Example 2: Bracket Expressions and Shorthand Classes', '"    \n', sep="")
text <- "123abc\\d"
cat("Raw string:", text, sep = "\n")

cat("  > ", 'gsub("[\\d]", "X", text)', '\n    "', gsub("[\\d]", "X", text), '"    \n', sep="")
cat("  > ", 'gsub("[\\d]", "X", text, perl = TRUE)', '\n    "', gsub("[\\d]", "X", text, perl = TRUE), '"    \n', sep="")
cat("  > ", 'str_replace_all(text, "[\\d]", "X")', '\n    "', str_replace_all(text, "[\\d]", "X"), '"    \n', sep="")



cat("    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%    \n")
### Example 3: Word Boundaries
cat('### Example 3: Word Boundaries', '"    \n', sep="")
text <- "word"
cat("Raw string:", text, sep = "\n")

cat("  > ", 'gsub("\\\\<", "~", text)', '\n    "', gsub("\\<", "~", text), '"    \n', sep="")
cat("  > ", 'gsub("\\\\>", "~", text)', '\n    "', gsub("\\>", "~", text), '"    \n', sep="")
cat("  > ", 'gsub("\\\\b", "~", text, perl = TRUE)', '\n    "', gsub("\\b", "~", text, perl = TRUE), '"    \n', sep="")
cat("  > ", 'str_replace_all(text, "\\\\b", "~")', '\n    "', str_replace_all(text, "\\b", "~"), '"    \n', sep="")



cat("    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%    \n")
### Example 4: Anchors
cat('### Example 4: Anchors', '"    \n', sep="")
text <- "pattern\npattern2"
cat("Raw string:", text, sep = "\n")

cat("  > ", 'gsub("^pattern", "replacement", text)', '\n    "', gsub("^pattern", "replacement", text), '"    \n', sep="")
cat("  > ", 'gsub("^pattern", "replacement", text, perl = TRUE)', '\n    "', gsub("^pattern", "replacement", text, perl = TRUE), '"    \n', sep="")
cat("  > ", 'gsub("(?m)^pattern", "replacement", text, perl = TRUE)', '\n    "', gsub("(?m)^pattern", "replacement", text, perl = TRUE), '"    \n', sep="")
cat("  > ", 'str_replace_all(text, "^pattern", "replacement")', '\n    "', str_replace_all(text, "^pattern", "replacement"), '"    \n', sep="")
cat("  > ", 'str_replace_all(text, "(?m)^pattern", "replacement")', '\n    "', str_replace_all(text, "(?m)^pattern", "replacement"), '"    \n', sep="")

```


### @ positive/negative lookforward/lookbehind  
```{r, echo=TRUE, results="markup", collapse=TRUE, paged.print=FALSE, comment="", R.options=list(width=120)}
library(stringr)

cat("    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%    \n")
### Example 1: Positive Lookahead:  PatternToMatch(?=PositiveLookahead)
cat('### Example 1: Positive Lookahead:  PatternToMatch(?=PositiveLookahead)', '"    \n', sep="")

# Using gsub with perl=TRUE
cat("  > ", '"apple123banana" %>% gsub("\\d+(?=banana)", "XYZ", ., perl=TRUE)'|>str_replace_all(fixed("\\"),"\\\\"), '\n    "', "apple123banana" %>% gsub("\\d+(?=banana)", "XYZ", ., perl=TRUE), '"    \n', sep="")

# Using stringr::str_replace_all
cat("  > ", '"apple123banana" |> str_replace_all("\\d+(?=banana)", "XYZ")'|>str_replace_all(fixed("\\"),"\\\\"), '\n    "', "apple123banana" |> str_replace_all("\\d+(?=banana)", "XYZ"), '"    \n', sep="")



cat("    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%    \n")
### Example 2: Negative Lookahead:  PatternToMatch(?!NegativeLookahead)
cat('### Example 2: Negative Lookahead:  PatternToMatch(?!NegativeLookahead)', '"    \n', sep="")
...
cat("    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%    \n")
### Example 3: Positive Lookbehind:  (?<=PositiveLookbehind)PatternToMatch
cat('### Example 3: Positive Lookbehind:  (?<=PositiveLookbehind)PatternToMatch', '"    \n', sep="")

# Using gsub with perl=TRUE
cat("  > ", '"apple123banana" %>% gsub("(?<=apple)\\d+", "XYZ", ., perl=TRUE)'|>str_replace_all(fixed("\\"),"\\\\"), '\n    "', "apple123banana" %>% gsub("(?<=apple)\\d+", "XYZ", ., perl=TRUE), '"    \n', sep="")

# Attempt for positive lookbehind using stringr::str_replace_all 
cat("  > ", '"apple123banana" |> str_replace_all("(?<=apple)\\d+", "XYZ")'|>str_replace_all(fixed("\\"),"\\\\"), '\n    "', "apple123banana" |> str_replace_all("(?<=apple)\\d+", "XYZ"), '"    \n', sep="")

# Using stringr::str_replace_all (not supported, so using a workaround)
cat("  > ", '"apple123banana" |> str_replace_all("apple(\\d+)", "appleXYZ")'|>str_replace_all(fixed("\\"),"\\\\"), '\n    "', "apple123banana" |> str_replace_all("apple(\\d+)", "appleXYZ"), '"    \n', sep="")



cat("    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%    \n")
### Example 4: Negative Lookbehind:  (?<!NegativeLookbehind)PatternToMatch
cat('### Example 4: Negative Lookbehind:  (?<!NegativeLookbehind)PatternToMatch', '"    \n', sep="")

# Using gsub with perl=TRUE
cat("  > ", '"apple123banana" %>% gsub("(?<!apple)\\d+", "XYZ", ., perl=TRUE)'|>str_replace_all(fixed("\\"),"\\\\"), '\n    "', "apple123banana" %>% gsub("(?<!apple)\\d+", "XYZ", ., perl=TRUE), '"    \n', sep="")

# Attempt for negative lookbehind using stringr::str_replace_all 
cat("  > ", '"apple123banana" |> str_replace_all("(?<!apple)\\d+", "XYZ")'|>str_replace_all(fixed("\\"),"\\\\"), '\n    "', "apple123banana" |> str_replace_all("(?<!apple)\\d+", "XYZ"), '"    \n', sep="")


# Using stringr::str_replace_all (not supported, so using a workaround)
cat("Workaround needed: ICU does not support lookbehind, so you'll need to handle this differently..?", "  \n", sep="")

```


### @ negative lookbehind workaround in ICU  
```{r, echo=TRUE, results="markup", collapse=TRUE, paged.print=FALSE, comment="", R.options=list(width=120)}
options(width=120)
...
# Original string
text <- "apple123banana 456orange 789apple101112"
cat("  > ", 'text = "', text, '"', "\n", sep="")
cat("    ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~    \n")

# Step 1: Use a non-lookbehind method to mark where the condition does not apply.
# Temporarily mark parts of the string where the lookbehind condition is true
# In this case, we mark "apple123" and "apple101112" with "TOBEIGNORED"
text_temp <- str_replace_all(text, "(apple\\d+)", "TOBEIGNORED")
cat("\n > ", 'Step 1: Temporarily mark parts with lookbehind condition', "\n", text_temp, "\n", sep="")

# Step 2: Replace all remaining digits that are not marked
text_replaced <- str_replace_all(text_temp, "\\d+", "XYZ")
cat("\n > ", 'Step 2: Replace all remaining digits that are not marked', "\n", text_replaced, "\n", sep="")

# Step 3: Restore the original digits from the marked parts
final_result <- str_replace_all(text_replaced, "TOBEIGNORED", function(x) str_extract(text, "apple\\d+"))
cat("\n > ", 'Step 3: Restore original digits from marked parts', "\n", final_result, "\n", sep="")

# Expected output for Step 3:
# "apple123banana XYZorange XYZapple101112"



# Expected output for Step 1:
# "appleTOBEIGNORED123banana 456orange 789appleTOBEIGNORED101112"
# Expected output for Step 2:
# "appleTOBEIGNOREDXYZbanana XYZorange XYZappleTOBEIGNOREDXYZ"
# Expected output for Step 3:
# "apple123banana XYZorange XYZapple101112"
...
# Step 1: Use a non-lookbehind method to mark where the condition does not apply.
# Temporarily mark parts of the string where the lookbehind condition is true.
matches <- str_extract_all(text, "apple\\d+")[[1]]
text_temp <- text
for (i in seq_along(matches)) {
  text_temp <- str_replace(text_temp, matches[i], paste0("TOBEIGNORED", i))
}
cat("\n > ", 'Step 1: Temporarily mark parts with lookbehind condition', "\n", text_temp, "\n", sep="")
...
# Step 3: Restore the original digits from the marked parts
for (i in seq_along(matches)) {
  text_replaced <- str_replace(text_replaced, paste0("TOBEIGNORED", i), matches[i])
}
cat("\n > ", 'Step 3: Restore original digits from marked parts', "\n", text_replaced, "\n", sep="")
...
# Step 1: Split the string at every occurrence of "apple" (to handle all "apple"-prefixed segments separately)
parts <- str_split(text, "(apple\\d*)", simplify = TRUE)
cat("\n > ", 'Step 1: Split the string at "apple" occurrences', "\n", deparse(parts), "\n", sep="")

# Step 2: Replace digits only in parts that are not preceded by "apple"
# Note: The first part (parts[1]) is not preceded by "apple", so we replace there
#       Any subsequent parts that do not start with "apple" should be replaced
for (i in seq_along(parts)) {
  if (i == 1 || parts[i] == "" || !str_starts(parts[i], "apple")) {
    parts[i] <- str_replace_all(parts[i], "\\d+", "XYZ")
  }
}
cat("\n > ", 'Step 2: Replace digits in parts not preceded by "apple"', "\n", parts, "\n", sep="")

# Step 3: Recombine the string, ensuring to reattach the "apple"-prefixed parts correctly
# Since str_split removed the "apple" parts, we reinsert them where necessary
final_result <- paste0(parts[1], paste0("apple", parts[-1]), collapse = "")
cat("\n > ", 'Step 3: Recombine the parts', "\n", final_result, "\n", sep="")
...
# Step 1: Extract all segments where we either have "apple" followed by digits or any other text
segments <- str_match_all(text, "(apple\\d+|[^apple]+)")[[1]]
cat("\n > ", 'Step 1: Extract segments with and without "apple"', "\n", deparse(segments), "\n", sep="")

# Step 2: Replace digits only in segments that do not start with "apple"
for (i in seq_along(segments)) {
  if (!str_starts(segments[i], "apple")) {
    segments[i] <- str_replace_all(segments[i], "\\d+", "XYZ")
  }
}
cat("\n > ", 'Step 2: Replace digits in segments not preceded by "apple"', "\n", deparse(segments), "\n", sep="")

# Step 3: Recombine the segments
final_result <- paste(segments, collapse = "")
cat("\n > ", 'Step 3: Recombine the segments', "\n", final_result, "\n", sep="")
...
# Step 1: Split the string by occurrences of "apple" followed by digits
# This will separate the string into parts that are either prefixed with "apple" or not
parts <- str_split(text, "(apple\\d+)", simplify = TRUE)
cat("\n > ", 'Step 1: Split the string at "apple" occurrences', "\n", parts, "\n", sep="")

# Step 2: Replace digits only in the parts that are not preceded by "apple"
# We should replace only in segments that are not part of "apple" prefixed parts
for (i in seq_along(parts)) {
  if (parts[i] != "" && !str_starts(parts[i], "apple")) {
    parts[i] <- str_replace_all(parts[i], "\\d+", "XYZ")
  }
}
cat("\n > ", 'Step 2: Replace digits in parts not preceded by "apple"', "\n", parts, "\n", sep="")

# Step 3: Recombine the string, inserting back the "apple" prefixed parts
# Recombine the parts to form the final string
final_result <- paste(parts, collapse = "")
cat("\n > ", 'Step 3: Recombine the parts', "\n", final_result, "\n", sep="")
...
# @@ START) dev) perl=TRUE ----  
# https://chatgpt.com/c/8f370edc-8df9-4ca9-96c1-e26d9bbb6ff9
## \% vec_names |> str_replace_all  
```{r, echo=TRUE, results="markup", collapse=TRUE, paged.print=FALSE, comment="", R.options=list(width=120)}
options(width=120)
df_vec_names |> mutate(
    vec_names %>% {gsub("(?<![\\w.])name(?![\\w.])", "nm", ., perl=TRUE)}
) |> rename_with(~str_replace_all(., fixed("str_replace_all(vec_names, "), "")) |>
    rename_with(~str_replace_all(., ', "[^"]*"\\)', "")) |> 
    rename_with(~str_replace_all(., fixed("\\\\"), "\\")) |> 
    t() |> as.data.frame() %>% {mutate(  ., `==goal` = apply(., 1, function(vec) paste0(as.numeric(vec == goal), collapse=""))  )} |>
