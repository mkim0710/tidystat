...
### \% as.list(.GlobalEnv)[as.list(.GlobalEnv)|>map_lgl(is.character)]  
```{r, echo=TRUE, results="markup", collapse=TRUE, paged.print=FALSE, comment="", R.options=list(width=120)}
options(width=120)
library(purrr)
...
f_expression.substitute_echo_and_dput <- function(expr_text) {
  ...
  char_vars = as.list(.GlobalEnv)[as.list(.GlobalEnv)|>map_lgl(is.character)]
  
  # Substitute all the defined names in .GlobalEnv
  ...
  cat(" > ", deparse(substituted_expr), "==", deparse(evaluated_expr), "\n")
}

DataSetName <- "analyticDF_time2event"
analyticDF_time2event <- data.frame(matrix(1:12, nrow=24, ncol=12))

"dim(get(DataSetName))" |> f_expression.substitute_echo_and_dput()
