...
if (Sys.getenv("PARENT_RENDERING") != "YES") {
    if (Sys.info()["sysname"] == "Windows") {
        ...
        cat('Starting: rstudioapi::getSourceEditorContext()$path |> rmarkdown::render(output_dir = dirname(env1$path$CurrentSource.path.filename.ext), output_format = "pdf_document") \n')
        Sys.setenv(PARENT_RENDERING = "YES"); env1$path$CurrentSource.path.filename.ext %>% {paste0(env1$path$path1,"/",.)} |> rmarkdown::render(output_dir = dirname(env1$path$CurrentSource.path.filename.ext), output_format = "pdf_document"); Sys.setenv(PARENT_RENDERING = "NO")
...
# env1$path$CurrentSource.path.filename.ext %>% {paste0(env1$path$path1,"/",.)} %>% cat('  Sys.setenv(PARENT_RENDERING = "YES"); "',.,'" |> rmarkdown::render(output_dir = dirname(env1$path$CurrentSource.path.filename.ext), output_format = "pdf_document"); Sys.setenv(PARENT_RENDERING = "NO")  \n', sep=""); .path.file = env1$path$CurrentSource.path.filename.pdf %>% {paste0(env1$path$path1,"/",.)}; paste0( "git add -f ",shQuote(.path.file) ) |> deparse() |> cat(" |> system(intern=TRUE)  \n", sep=""); '  env1$env.internal$f_filename.ext.createBackup(backup_from_path.filename.ext = .path.file, .backup_to_path="-backup", timeFormat="%y%m%d_%H", overwrite=TRUE)' |> cat("  \n", sep=""); '  if (Sys.info()["sysname"] == "Windows") { env1$env.internal$f_file_PDF.sumatra(.path.file) } else { browseURL(.path.file) } ' |> cat("  \n", sep="")
# cat("    ________________________________________________________________________    \n")
env1$path$CurrentSource.path.filename.ext %>% {paste0(env1$path$path1,"/",.)} %>% cat('  Sys.setenv(PARENT_RENDERING = "YES"); "',.,'" |> rmarkdown::render(output_dir = dirname(env1$path$CurrentSource.path.filename.ext), output_format = "html_document"); Sys.setenv(PARENT_RENDERING = "NO")  \n', sep=""); .path.file = env1$path$CurrentSource.path.filename.html %>% {paste0(env1$path$path1,"/",.)}; paste0( "git add -f ",shQuote(.path.file) ) |> deparse() |> cat(" |> system(intern=TRUE)  \n", sep=""); '  env1$env.internal$f_filename.ext.createBackup(backup_from_path.filename.ext = .path.file, .backup_to_path="-backup", timeFormat="%y%m%d_%H", overwrite=TRUE)' |> cat("  \n", sep=""); '  env1$env.internal$f_URL.open_in_edge_app(.path.file)' |> cat("  \n", sep="")
```
...
# paste0("https://github.com/mkim0710/",basename(getwd()),"/commints/main/",env1$path$CurrentSource.path.filename.ext) %>% paste0('"C:/Program Files (x86)/Microsoft/Edge/Application/msedge_proxy.exe" --app="',.,'"') |> system(intern=TRUE)
