...
.path4write = env1$path$path4write
.t0 = Sys.time()
save( list = .objectname, file = here::here(.path4write, paste0(.objectname,".rda", "")), compress="xz", compression_level=9 )
print(Sys.time() - .t0)
...
.path4write = env1$path$path4write
.objectname = "ADS200621"
# saveRDS( get(.objectname), xzfile(here::here(.path4write, paste0(.objectname,".rds","")), compression=9) )
# R.utils::gzip(here::here(.path4write, paste0(.objectname,".rds")), overwrite=T, remove=F, compression=9)
# system.time(write_rds( get(.objectname), here::here(.path4write, paste0(.objectname,".rds","")), compress="gz", compression=9 ))
system.time(write_rds( get(.objectname), here::here(.path4write, paste0(.objectname,".rds","")), compress="xz", compression=9 ))
# .path_FileName.xlsx = paste0(.path4write,ifelse(.path4write=="","","/"),.objectname,".xlsx")  ;  openxlsx2::write_xlsx(get(.objectname), file = .path_FileName.xlsx, as_table=TRUE, table_style="none", row_names=TRUE, col_widths="auto", first_active_row=2, first_active_col=2) |> system.time() |> round(3) |> unclass() |> deparse() |> cat("\n")  ;  if (Sys.info()["sysname"] == "Linux") browseURL(.path_FileName.xlsx) else openxlsx2::xl_open(.path_FileName.xlsx)
...
FileName %>% {file.info(here::here(.path4read, .))} %>% rownames_to_column("FileName") %>% select(FileName, size) %>% mutate(KB = size/2^10, MB = KB/2^10, GB = MB/2^10) |> print() #----

assign(.objectname, read_rds(here::here(.path4read, paste0(.objectname,".rds")))    )
...
.t0 = Sys.time()
assign(.objectname
       , readRDS(here::here(.path4read, FileName)) %>% 
           as_tibble
)
Sys.time() - .t0
...
Sys.time() - .t0
write_rds( get(.objectname), here::here(.path4write, paste0(.objectname,".rds")) )
Sys.time() - .t0
