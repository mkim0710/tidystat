...
f_path_fileRegEX.rename <- function(path = ".", gsub.pattern_from, gsub.replacement_to, full.names = FALSE, recursive = FALSE, execute = FALSE) {
    
    cat("path == ", deparse(path), "\n", sep="")
    
    # Get a list of files that match the old pattern, recursively if specified
    old_files <- list.files(path, pattern = gsub.pattern_from, full.names = full.names, recursive = recursive)
    ...
    new_files <- gsub(gsub.pattern_from, gsub.replacement_to, old_files)
    
    # If preview is requested, just return the proposed changes without executing
    if (!execute) {
        message("Preview mode: No paths or files have been renamed.")
        return(data.frame(old_path_files, new_path_files))
    }
    
    # If execute is TRUE, perform the renaming
    renamed <- file.rename(from = old_files, to = new_files)
    ...
    return(data.frame(old_files, new_files, renamed))
}
...
env1$path$path1 %>% f_path_fileRegEX.rename(gsub.pattern_from = "source", gsub.replacement_to = "dev", full.names = FALSE, recursive = FALSE, execute = FALSE)
...
env1$path$path1 %>% f_path_fileRegEX.rename(gsub.pattern_from = "data", gsub.replacement_to = "DataSet", full.names = FALSE, recursive = FALSE, execute = FALSE)

env1$path$path1 %>% f_path_fileRegEX.rename(gsub.pattern_from = "data", gsub.replacement_to = "DataSet", full.names = FALSE, recursive = TRUE, execute = FALSE)
...
env1$path$path1 %>% f_path_fileRegEX.rename(gsub.pattern_from = "labelled", gsub.replacement_to = "lbl", full.names = FALSE, recursive = TRUE, execute = FALSE)

```
  
    path == "/home/rstudio/github_tidystat"
    Preview mode: No files have been renamed.
                                                                                 old_files
    1 Rdev/10_import_clean_datatype/16_categorical_labelled/f_vec_labelled.as_factor.dev.r
                                                                       new_files
    1 Rdev/10_import_clean_datatype/16_categorical_lbl/f_vec_lbl.as_factor.dev.r
