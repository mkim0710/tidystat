...
## :: f_path_filenameRegEX.rename()  
```{r, echo=TRUE, results="markup", collapse=TRUE, paged.print=FALSE, comment="#", R.options=list(width=120)}
options(width=120)

f_path_filenameRegEX.rename <- function(path = ".", gsub.pattern_from, gsub.replacement_to, recursive = FALSE, execute = FALSE) {
  # Get a list of files that match the old pattern, recursively if specified
  old_files <- list.files(path, pattern = gsub.pattern_from, full.names = TRUE, recursive = recursive)
  ...
  if (!execute) {
    message("Preview mode: No files have been renamed.")
    return(data.frame(old_files, new_files))
  }
...
# f_path_filenameRegEX.rename("path/to/your/directory", "old", "new", recursive = TRUE, execute = FALSE)

# Execute the renaming:
# f_path_filenameRegEX.rename("path/to/your/directory", "old", "new", recursive = TRUE, execute = TRUE)
