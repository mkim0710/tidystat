...
## :: f_path.dir.file_Rcode_documents.info.xlsx = function() -dev -pending  
```{r, echo=TRUE, results="markup", collapse=TRUE, paged.print=FALSE, comment="#", R.options=list(width=120)}
options(width=120)
...
env1[[.tmp$env1_subenv_name]][[.tmp$objectname]] = function(
    ...
    DataSetName = "Rcode"
    DataSet = input_path |> env1$f$f_path.file.info(pattern = pattern, all.files = all.files, full.names = full.names, recursive = recursive, ignore.case = ignore.case, include.dirs = include.dirs, no.. = no.., arrange_by = arrange_by)
    ...
    wb$set_active(sheet = DataSetName, row = 2, col = 5)
    columns_to_show = c("path.relative", "filename.ext", "ext", "size_MiB", "isdir", "ModifiedDate")
    columns_to_hide.index = which(!colnames(DataSet) %in% columns_to_show)
