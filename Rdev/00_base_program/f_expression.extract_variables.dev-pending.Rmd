...
parse(text = "Female = ifelse(A00_SEX == '2', TRUE, FALSE)")
parse(text = "Female = ifelse(A00_SEX == '2', TRUE, FALSE)")[[1]]
parse(text = "Female = ifelse(A00_SEX == '2', TRUE, FALSE)")[[1]][[1]]
parse(text = "Female = ifelse(A00_SEX == '2', TRUE, FALSE)")[[1]][[2]]
parse(text = "Female = ifelse(A00_SEX == '2', TRUE, FALSE)")[[1]][[3]]
```
...
# @@ START) function ----  
## \% |> f_expression.extract_variables 
```{r, echo=TRUE, results="markup", collapse=TRUE, paged.print=FALSE, comment="", R.options=list(width=120)}
options(width=120)
f_expression.extract_variables <- function(e) {
  if (is.symbol(e)) {
    return(as.character(e))
  } else if (is.call(e)) {
    # Apply extract_variables to each part of the call, except for the function name
    return(unlist(lapply(e[-1], f_expression.extract_variables)))
  } else if (is.language(e)) {
    # Handle language objects (e.g., logical comparisons)
    return(unlist(lapply(as.list(e), f_expression.extract_variables)))
  } else {
    return(NULL)
  }
}
...
parse(text = "Female = ifelse(A00_SEX == '2', TRUE, FALSE)")[[1]][[3]]
parse(text = "Female = ifelse(A00_SEX == '2', TRUE, FALSE)")[[1]][[3]] %>% str
parse(text = "Female = ifelse(A00_SEX == '2', TRUE, FALSE)")[[1]][[3]] |> f_expression.extract_variables()
```
