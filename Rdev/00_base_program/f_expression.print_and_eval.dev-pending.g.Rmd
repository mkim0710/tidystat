...
# @@ START) dev ----  
## \% substitute(dim(get(DataSetName)), list(DataSetName = DataSetName))  
```{r, echo=TRUE, results="markup", collapse=TRUE, paged.print=FALSE, comment="", R.options=list(width=120)}
options(width=120)
# https://chatgpt.com/c/2c33cac5-5297-4385-90de-4277a03196f1
#|%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%|#  
# Define the dataset name
DataSetName <- "analyticDF_time2event"

# Create a dummy data frame as an example
analyticDF_time2event <- data.frame(matrix(1:12, nrow=228, ncol=12))

# Step 1: Define the expression
expr <- quote(dim(get(DataSetName)))
cat("@ Step 1: Original expression  \n")
cat(" > print(",deparse(substitute(expr)),"); str(",deparse(substitute(expr)),")  \n", sep="")
print(expr)
str(expr)

# Step 2: Use substitute to replace the variable name with its actual value as a string
substituted_expr <- substitute(dim(get(DataSetName)), list(DataSetName = DataSetName))
cat("\n@ Step 2: Substituted expression  \n")
cat(" > print(", deparse(substitute(substituted_expr)), "); str(", deparse(substitute(substituted_expr)), ")  \n", sep="")
print(substituted_expr)
str(substituted_expr)

# Step 3: Evaluate the substituted expression
evaluated_expr <- eval(substituted_expr)
cat("\n@ Step 3: Evaluated expression result  \n")
cat(" > print(", deparse(substitute(evaluated_expr)), "); str(", deparse(substitute(evaluated_expr)), ")  \n", sep="")
print(evaluated_expr)
str(evaluated_expr)

# Step 4: Print the substituted expression and its evaluation
cat("\n@ Step 4: Final output  \n")
cat(" > ", deparse(substituted_expr), "==", deparse(evaluated_expr), "\n")
```


## \% deparse(eval(eval(substituted_expr)))  
```{r, echo=TRUE, results="markup", collapse=TRUE, paged.print=FALSE, comment="", R.options=list(width=120)}
options(width=120)
# https://chatgpt.com/c/2c33cac5-5297-4385-90de-4277a03196f1
#|%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%|#  
...
expr <- quote(as.expression(.text_expression))
cat("@ Step 1: Original expression  \n")
cat(" > print(",deparse(substitute(expr)),"); str(",deparse(substitute(expr)),")  \n", sep="")
print(expr)
str(expr)
...
# Step 2: Use substitute to replace the variable name with its actual value as a string
# substituted_expr <- substitute(as.expression(.text_expression), list(.text_expression = .text_expression, DataSetName = DataSetName))
# substituted_expr <- substitute(expr, list(expr = expr, .text_expression = .text_expression, DataSetName = DataSetName))
substituted_expr <- substitute(eval(substitute(expr, list(expr = expr))), list(expr = expr, .text_expression = .text_expression, DataSetName = DataSetName))
cat("\n@ Step 2: Substituted expression  \n")
cat(" > print(", deparse(substitute(substituted_expr)), "); str(", deparse(substitute(substituted_expr)), ")  \n", sep="")
print(substituted_expr)
str(substituted_expr)
...
# Step 4: Print the substituted expression and its evaluation
cat("\n@ Step 4: Final output  \n")
cat(" > ", deparse(substituted_expr), "==", deparse(eval(eval(substituted_expr))), "\n")
```
...
substitute(parse(text = .text_expression))
substitute(parse(text = .text_expression), env = list(.text_expression = .text_expression))
substitute(parse(text = .text_expression), env = list(.text_expression = .text_expression, DataSetName = DataSetName))
cat("    ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++    \n")
...
## *** \% |> substitute(as.environment(.GlobalEnv))  
```{r, echo=TRUE, results="markup", collapse=TRUE, paged.print=FALSE, comment="", R.options=list(width=120)}
options(width=120)
# https://chatgpt.com/c/2c33cac5-5297-4385-90de-4277a03196f1
#|%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%|#  
...
# \$ e1 = as.list(.GlobalEnv)[ls(envir=.GlobalEnv)[ls(envir=.GlobalEnv) |> map_lgl(function(objectname) is.character(get(objectname, envir=.GlobalEnv)))]]
```{r, echo=TRUE, results="markup", collapse=TRUE, paged.print=FALSE, comment="", R.options=list(width=120)}
options(width=120)
e1 = as.list(.GlobalEnv)[ls(envir=.GlobalEnv)[ls(envir=.GlobalEnv) |> map_lgl(function(objectname) is.character(get(objectname, envir=.GlobalEnv)))]] |> as.environment()
ls(envir = e1)
```


## *** \% |> substitute(as.list(.GlobalEnv)[ls(envir=.GlobalEnv)[ls(envir=.GlobalEnv) |> map_lgl(function(objectname) is.character(get(objectname, envir=.GlobalEnv)))]])  
```{r, echo=TRUE, results="markup", collapse=TRUE, paged.print=FALSE, comment="", R.options=list(width=120)}
options(width=120)
# https://chatgpt.com/c/2c33cac5-5297-4385-90de-4277a03196f1
#|%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%|#  
...
dim(get(DataSetName)) |> substitute(list(DataSetName = DataSetName)) |> eval() |> dput() 
cat("    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%    \n")
dim(get(DataSetName)) |> substitute(as.list(.GlobalEnv)[ls(envir=.GlobalEnv)[ls(envir=.GlobalEnv) |> map_lgl(function(objectname) is.character(get(objectname, envir=.GlobalEnv)))]]) 
dim(get(DataSetName)) |> substitute(as.list(.GlobalEnv)[ls(envir=.GlobalEnv)[ls(envir=.GlobalEnv) |> map_lgl(function(objectname) is.character(get(objectname, envir=.GlobalEnv)))]]) |> str()
dim(get(DataSetName)) |> substitute(as.list(.GlobalEnv)[ls(envir=.GlobalEnv)[ls(envir=.GlobalEnv) |> map_lgl(function(objectname) is.character(get(objectname, envir=.GlobalEnv)))]]) |> deparse() %>% cat(" > ",.," |> dput()  \n", sep="")
dim(get(DataSetName)) |> substitute(as.list(.GlobalEnv)[ls(envir=.GlobalEnv)[ls(envir=.GlobalEnv) |> map_lgl(function(objectname) is.character(get(objectname, envir=.GlobalEnv)))]]) |> eval() |> dput() 
```
...
cat("    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%    \n")
dim(get(DataSetName)) |> expression() |> substitute(as.list(.GlobalEnv)[ls(envir=.GlobalEnv)[ls(envir=.GlobalEnv) |> map_lgl(function(objectname) is.character(get(objectname, envir=.GlobalEnv)))]]) 
dim(get(DataSetName)) |> expression() |> substitute(as.list(.GlobalEnv)[ls(envir=.GlobalEnv)[ls(envir=.GlobalEnv) |> map_lgl(function(objectname) is.character(get(objectname, envir=.GlobalEnv)))]]) |> str()
dim(get(DataSetName)) |> expression() |> substitute(as.list(.GlobalEnv)[ls(envir=.GlobalEnv)[ls(envir=.GlobalEnv) |> map_lgl(function(objectname) is.character(get(objectname, envir=.GlobalEnv)))]]) |> deparse() %>% cat(" > ",.," |> dput()  \n", sep="")
dim(get(DataSetName)) |> expression() |> substitute(as.list(.GlobalEnv)[ls(envir=.GlobalEnv)[ls(envir=.GlobalEnv) |> map_lgl(function(objectname) is.character(get(objectname, envir=.GlobalEnv)))]]) |> eval() |> dput() 
```  
  
## \% deparse(substitute(path$path1, env = env1))  
```{r, echo=TRUE, results="markup", collapse=TRUE, paged.print=FALSE, comment="", R.options=list(width=120)}
options(width=120)
env1$path$path1
deparse(substitute(env1$path$path1))
deparse(substitute(path$path1, env = env1))
```
  
  
## *** \% deparse(substitute(path1, env = as.environment(env1$path)))  
```{r, echo=TRUE, results="markup", collapse=TRUE, paged.print=FALSE, comment="", R.options=list(width=120)}
options(width=120)
env1$path$path1
deparse(substitute(env1$path$path1))
...
# Define a function to handle substitution in a non-global environment
substitute_in_env <- function(expr, name) {
  # Substitute the variable in the expression
  substituted_expr <- substitute(expr, list(DataSetName = as.name(name)))
  return(substituted_expr)
}

# Step 1: Define the expression with quote
expr <- quote(dim(get(DataSetName)))
cat("@ Step 1: Original expression  \n")
cat(" > print(", deparse(substitute(expr)), "); str(", deparse(substitute(expr)), ")  \n", sep="")
print(expr)
...
# Step 2: Perform substitution using the function
substituted_expr <- substitute_in_env(expr, DataSetName)
cat("\n@ Step 2: Substituted expression  \n")
cat(" > print(", deparse(substitute(substituted_expr)), "); str(", deparse(substitute(substituted_expr)), ")  \n", sep="")
print(substituted_expr)
str(substituted_expr)
...
```{r, echo=TRUE, results="markup", collapse=TRUE, paged.print=FALSE, comment="", R.options=list(width=120)}
options(width=120)
# Function to substitute and evaluate an expression
substitute_and_evaluate <- function(expr, dataset_name) {
  # Step 1: Substitute the variable name with its actual value
  substituted_expr <- substitute(expr, list(DataSetName = as.name(dataset_name)))
  
  # Step 2: Evaluate the substituted expression
  evaluated_expr <- eval(substituted_expr)
  
  # Return both substituted and evaluated expressions
  list(substituted_expr = substituted_expr, evaluated_expr = evaluated_expr)
}
...
# Define the expression only once
expr <- quote(dim(get(DataSetName)))

# Use the function to substitute and evaluate the expression
result <- substitute_and_evaluate(expr, DataSetName)

# Step 1: Print original expression
cat("@ Step 1: Original expression  \n")
cat(" > print(", deparse(substitute(expr)), "); str(", deparse(substitute(expr)), ")  \n", sep="")
print(expr)
str(expr)

# Step 2: Print substituted expression
cat("\n@ Step 2: Substituted expression  \n")
cat(" > print(", deparse(substitute(result$substituted_expr)), "); str(", deparse(substitute(result$substituted_expr)), ")  \n", sep="")
print(result$substituted_expr)
str(result$substituted_expr)

# Step 3: Print evaluated expression
cat("\n@ Step 3: Evaluated expression result  \n")
cat(" > print(", deparse(substitute(result$evaluated_expr)), "); str(", deparse(substitute(result$evaluated_expr)), ")  \n", sep="")
print(result$evaluated_expr)
str(result$evaluated_expr)

# Step 4: Print the final output
cat("\n@ Step 4: Final output  \n")
cat(" > ", deparse(result$substituted_expr), "==", deparse(result$evaluated_expr), "\n")
...
.text_expression = "dim(get(DataSetName))"
# Step 1: Define a function to handle substitution in a non-global environment
