...
GermanBreastCancerStudyGroup$`Hormone treatment` <- fct_recode(GermanBreastCancerStudyGroup$`Hormone treatment`,
                                                               Treated = "1",
                                                               Placebo = "0")
GermanBreastCancerStudyGroup |> str(max.level = 2, give.attr = TRUE)
...
f_DS.Table1.print.df <- function(DS, VarNames4Exposure = c("InterventionGroup"), Varnames_factor.exclude = NULL, addOverall = TRUE, includeNA = TRUE, print.Varnames_nonnormal = NULL, print.showAllLevels = TRUE) {
...
Table1_print_df <- f_DS.Table1.print.df(
    GermanBreastCancerStudyGroup, 
...
env1$f$f_file.system_switch_open("library(flextable) flextable1.docx")
```
...
#   package ‘flexlsx’ is not available for this version of R
# 
...
if (Sys.getenv("PARENT_RENDERING") != "YES") {
    ...
    .names4data.frame = if(exists("MetaData") && !is.null(unlist(MetaData))) {
        MetaData$DSNs |> names()
    ...
    # .path_file <- tempfile(fileext = ".xlsx")
    .path4write = env1$path$path1
    .LastSourceEditorContext.path_FileName = env1$path$LastSourceEditorContext.path_FileNameExt |> str_replace("\\.([[:alnum:]]+)$","")
    .path_file = paste0(.path4write,"/",.LastSourceEditorContext.path_FileName,".xlsx")
    .wb$save(.path_file)
    cat(".path_file == ", deparse(.path_file), "  \n", sep="")
    if(Sys.info()["sysname"] == "Windows") openxlsx2::xl_open(.path_file)
