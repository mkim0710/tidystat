...
# __________|------  
## @ EPI library(gtsummary) library(Z.gtsummary.addons) Z.gtsummary.addons::add_SMD().Rmd
...
### ECHO \% |> tbl_summary() ----  
#### **ECHO** \% |> Z.gtsummary.addons::add_SMD(location = "both") ----  
```{r, echo=TRUE, results="markup", collapse=TRUE, paged.print=FALSE, comment="#", R.options=list(width=120)}
options(width=120)
trial %>%
  tbl_summary(by = grade, include = c(age, stage)) %>%
  Z.gtsummary.addons::add_SMD(location = "both") |>
    print() |> invisible()
```
  
  
### ECHO \% |> tbl_summary() ----  
#### **ECHO** \% |> Z.gtsummary.addons::add_SMD(location = "level", ci = TRUE, ci_bracket="[]", ci_sep=";") ----  
```{r, echo=TRUE, results="markup", collapse=TRUE, paged.print=FALSE, comment="#", R.options=list(width=120)}
options(width=120)
trial %>% 
  tbl_summary(by = grade, include = c(age, stage)) %>%
  Z.gtsummary.addons::add_SMD(location = "level", ci = TRUE, ci_bracket="[]", ci_sep=";") |>
    print() |> invisible()
```
...
##%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%  
# :: DS.DNR = DataSet.Date.NA.rmAllNA = ===== 
# if(!exists("DSN")) {  warning('!exists("DSN"); '); DSN = DSsampleName = "df_NewDMv3.CensorEND.n88.select293.NR.recode.lbl.fct"; cat("DSsampleName == ", deparse(DSsampleName), "\n", sep = ""); assign(DSsampleName, read_csv(paste0(env1$path$path0,"/Rproject_KoGES_AA10030/",DSsampleName,".UTF8.csv")) %>% mutate( A01_BMI = A01_WEIGHT / (A01_HEIGHT/100)^2, A01_BMI.5bins = A01_BMI %>% cut(breaks = c(0, 18.5, 23, 25, 30, Inf), include.lowest = T, right = F) ) %>% select(matches("BMI")))  }  
if(!exists("DSN")) {  warning('!exists("DSN"); '); DSN = DSsampleName = "as1_7.na.Date.fct.select.sample.03"; cat("DSsampleName == ", deparse(DSsampleName), "\n", sep = ""); assign(DSsampleName, read_rds(paste0(env1$path$path0,"/Rproject_KoGES_AA10030/",DSsampleName,"/",DSsampleName,".rds.xz")))  }  
DS.DNR = get(DSN) %>% 
...
#### \% |> add_n() ----  
#### \% |> add_overall() ----  
#### **ECHO** \% |> Z.gtsummary.addons::add_SMD(location = "both", ci = TRUE, ci_bracket = "()", ci_sep = ";", ref_group = TRUE) ----  
#### **ECHO** \% |> add_p() ----  
...
GT1 = trial %>% 
    ...
    # add_n() %>%
    # add_overall() |>
    Z.gtsummary.addons::add_SMD(location = "both", ci = TRUE, ci_bracket = "()", ci_sep = ";", ref_group = TRUE) %>%
    add_p() |>
    modify_header(all_stat_cols() ~ "**{level}**  \nN = {n} ({style_percent(p)}%)", p.value ~ "***p***") |>
