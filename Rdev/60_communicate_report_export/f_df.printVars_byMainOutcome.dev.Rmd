...
get(DataSetName2.NA.rmAllNA) %>% select(any_of(MainOutcome), any_of(.VarName.selected)) %>% 
    ...
    as.data.frame() %>%  
    CreateTableOne(vars = names(.)[names(.)!=MainOutcome], strata = MainOutcome, data = ., test = T, includeNA = T, addOverall = T)
cat("    ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~    \n")
...
```{r, echo=TRUE, results="markup", collapse=TRUE, paged.print=FALSE, comment="", R.options=list(width=120)}
options(width=120)
f_df.printVars_byMainOutcome = function(DataSet, MainOutcome, VarName.selected, StudyPopulation) {
