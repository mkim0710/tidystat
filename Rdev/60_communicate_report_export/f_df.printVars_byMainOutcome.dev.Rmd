...
get(DataSetName2.NA.rmAllNA) %>% filter(!!rlang::quo_name(StudyPopulation) := TRUE) %>% select(any_of(MainOutcome), any_of(.VarName.selected)) %>% 
    # {.[map_lgl(., function(vec) if_else(is.numeric(vec), T, n_distinct(vec) <= 10) )]} |> 
    mutate_if(haven::is.labelled, as_factor) |>
    mutate_if(function(vec) {is.numeric(vec) && n_distinct(vec) <= 10}, as.factor) |>
    as.data.frame() %>%  
...
#|%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%|#  
f_df.printVars_byMainOutcome = env1$f$f_df.printVars_byMainOutcome
#|%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%|#  
get(DataSetName2.NA.rmAllNA) %>% f_df.printVars_byMainOutcome(MainOutcome = MainOutcome, VarName.selected = .VarName.selected)
cat("    ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~    \n")
cat("  ## @ StudyPopulation == ",deparse(StudyPopulation),"    \n", sep="")
get(DataSetName2.NA.rmAllNA) %>% filter(!!rlang::quo_name(StudyPopulation) := TRUE) %>% f_df.printVars_byMainOutcome(MainOutcome = MainOutcome, VarName.selected = .VarName.selected)
```
