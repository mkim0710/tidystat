...
get(DSN.DNR) %>% select(any_of(MainOutcome), any_of(.VarName.selected)) %>% 
...
get(DSN.DNR) %>% filter(!!rlang::sym(StudyPopulation) == TRUE) %>% select(any_of(MainOutcome), any_of(.VarName.selected)) %>% 
    # {.[map_lgl(., function(vec) if_else(is.numeric(vec), T, n_distinct(vec) <= 10) )]} |> 
    mutate_if(haven::is.labelled, as_factor) |>
    mutate_if(function(vec) {is.numeric(vec) && n_distinct(vec) <= 10}, as.factor) |>
    as.data.frame() %>%  
