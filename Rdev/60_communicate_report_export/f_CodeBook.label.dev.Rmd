...
create_factor <- function(var, value_options) {
  ...
  # Split the ValueOptions by comma
  options_split <- str_split(value_options, ",\\s*")[[1]]
  
  # Debug: Print the split ValueOptions
  print(paste("Split ValueOptions:", paste(options_split, collapse = ", ")))

  # Split each option into level and label
  levels <- as.numeric(sapply(options_split, function(x) str_split(x, "=")[[1]][1]))
  labels <- sapply(options_split, function(x) str_split(x, "=")[[1]][2])
...
# Apply the transformation for each variable in your dataset
for (i in 1:nrow(MetaData$tblVarName$tblVarName.suffix)) {
  var_name <- MetaData$tblVarName$tblVarName.suffix$VarName.suffix[i]
  value_options <- MetaData$tblVarName$tblVarName.suffix$ValueOptions[i]
  
  # Check if the column exists in the dataset
  if (var_name %in% colnames(your_data)) {
    print(paste("Processing variable:", var_name))
    print(paste("Original data:", paste(your_data[[var_name]], collapse = ", ")))

    # Convert the relevant column in your_data
    your_data[[var_name]] <- create_factor(your_data[[var_name]], value_options)
    
    # Debug: Print the updated column after conversion
    print(paste("Converted data:", paste(your_data[[var_name]], collapse = ", ")))
  } else {
