...
## \% create_factor() 
## \% apply_codebook_transformations() 
```{r, echo=TRUE, results="markup", collapse=TRUE, paged.print=FALSE, comment="", R.options=list(width=120)}
# Function to create levels and labels from ValueOptions, with customizable separators and optional intermediate printing
create_factor <- function(var, value_options, sep4levels = ",\\s*", sep4name_value = "=", print.intermediate = FALSE) {
  if (is.na(value_options) || value_options == "") {
    return(var)
  }

  if (print.intermediate) {
    # Print the variable and corresponding ValueOptions for debugging
  ...
  if (print.intermediate) {
    # Debug: Print the split ValueOptions
  ...
  if (print.intermediate) {
    # Debug: Print levels and labels
    print(paste("Levels:", paste(levels, collapse = ", ")))
    print(paste("Labels:", paste(labels, collapse = ", ")))
  }
  
  # Convert the variable into a factor with the specified levels and labels
  factor_var <- factor(var, levels = levels, labels = labels)

  # Optionally print the factor variable (optional)
  # print(paste("Converted factor:", paste(factor_var, collapse = ", ")))

  return(factor_var)
}

# Function to apply the codebook transformations to the dataset
apply_codebook_transformations <- function(data, codebook, sep4levels = ",\\s*", sep4name_value = "=", print.intermediate = FALSE) {
  ...
  for (i in 1:nrow(codebook)) {
    ...
    if (var_name %in% colnames(data)) {
      if (print.intermediate) {
        ...
        print(paste("Original data:", paste(data[[var_name]], collapse = ", ")))
      }

      # Convert the relevant column in data, using customizable separators
      data[[var_name]] <- create_factor(data[[var_name]], value_options, sep4levels, sep4name_value, print.intermediate)
      
      if (print.intermediate) {
        # Print only the final updated column after conversion
        print(paste("Converted data:", paste(data[[var_name]], collapse = ", ")))
      }
    } else {
      if (print.intermediate) {
...
# Apply the codebook transformations to the dataset
your_data2 <- apply_codebook_transformations(your_data, MetaData$tblVarName$tblVarName.suffix, sep4levels = ",\\s*", sep4name_value = "=", print.intermediate = TRUE)

# Print the final updated dataset
print("Final dataset:")
print(your_data2)
