...
## \% f_df.CodeBook_factor() 
```{r, echo=TRUE, results="markup", collapse=TRUE, paged.print=FALSE, comment="", R.options=list(width=120)}
# Function to apply the codebook transformations to the dataset
f_df.CodeBook_factor <- function(data, codebook, sep4levels = ",\\s*", sep4name_value = "=", print.intermediate = FALSE) {
  # Iterate over the codebook and apply transformations
  for (i in 1:nrow(codebook)) {
    var_name <- codebook$VarName.suffix[i]
    ValueOptions <- codebook$ValueOptions[i]
    ...
    if (var_name %in% colnames(data)) {
      ...
      data[[var_name]] <- f_vec_ValueOptions.factor(data[[var_name]], ValueOptions, sep4levels, sep4name_value, print.intermediate)
...
your_data2 <- f_df.CodeBook_factor(your_data, MetaData$tblVarName$tblVarName.suffix, sep4levels = ",\\s*", sep4name_value = "=", print.intermediate = TRUE)
