...
## \$ df_NewDMv3.CensorEND.n88.select293.NA.rmAllNA.recode.labelled  
...
DataSetName = "df_NewDMv3.CensorEND.n88.select293.NA.rmAllNA.recode.labelled"
...
### **ECHO** \% |> tbl_summary(by = trt, statistic = list(all_continuous() ~ "{median} ({p25};{p75})", all_categorical() ~ "{n} ({p}%)")) ---- 
#### **ECHO** \% |> modify_header(all_stat_cols() ~ "**{level}**  \nN = {n} ({style_percent(p)}%)") ----  
##### **ECHO** \% |> modify_spanning_header(all_stat_cols() ~ paste0("**",var_label(byVar),"**"), starts_with("p.value") ~ "**p-values**") ----  
#### \% |> add_n() ----  
#### \% |> add_overall() ----  
...
tbl = trial %>% 
    ...
    add_p() |>
    modify_header(all_stat_cols() ~ "**{level}**  \nN = {n} ({style_percent(p)}%)") |>
    # modify_spanning_header(all_stat_cols() ~ paste0("**",labelled::var_label(trial$grade),"**"), starts_with("SMD") ~ "**SMD**", starts_with("p.value") ~ "**p**") |>
