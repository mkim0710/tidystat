...
log_file_availability <- function(path) {
  if (file.exists(path)) {
    cat(Sys.time(), " - File exists\n")
  } else {
    cat(Sys.time(), " - File does not exist\n")
  }
}

# Function to safely read an RDS file with logging
safely_read_rds_with_logging <- function(path, max_attempts = 9) {
  attempt <- 1
...
paste0(path4read,"/",objectname,".rds")
file.exists(paste0(path4read,"/",objectname,".rds"))
assign( objectname, safely_read_rds_with_logging(paste0(path4read,"/",objectname,".rds")) )
