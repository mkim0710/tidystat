...
Rproject_Rmd/library/library(flextable) Publication-ready tables with flextable and own theme in R.g.Rmd
# @@ START) data example ----  
...
f_DataSet.table1 <- function(DataSet, strata_var, nonnormal_vars, cat_vars_exclude = NULL) {
    library(tableone)
...
# -> now within f_table1.flextable() function
```
...
# Function to Create Custom Table
f_table1.flextable <- function(df, header_text, footer_text, 
...
### :: table1_df = GermanBreastCancerStudyGroup |> generate_table_one() ----  
```{r, echo=TRUE, results="markup", collapse=TRUE, paged.print=FALSE, comment="", R.options=list(width=120)}
# Generate Table 1 Data Frame
table1_df <- f_DataSet.table1(
    GermanBreastCancerStudyGroup, 
    strata_var = "Hormone treatment", 
    nonnormal_vars = c("Progesterone receptors, fmol/L (median [IQR])",
                       "Estrogen receptors, fmol/L (median [IQR])"),
    cat_vars_exclude = "Hormone treatment"
)
table1_df
```

### :: flextable1 = table1_df |> f_table1.flextable() ----
```{r, echo=TRUE, results="markup", collapse=TRUE, paged.print=FALSE, comment="", R.options=list(width=120)}
...
flextable1 <- f_table1.flextable(table1_df, header_text = header_text, footer_text = footer_text)
flextable1
```
...
save_as_docx(flextable1, path = "library(flextable) flextable1.docx",
             pr_section = prop_section(
                 page_size = page_size(orient = "portrait", width = 210 / 25.4, height = 297 / 25.4),  # A4 size in inches
                 type = "continuous",
