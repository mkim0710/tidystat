...
# Function to create labeled data (without converting to factor)
create_labeled_data <- function(var, value_options, sep4levels = ",\\s*", sep4name_value = "=", print.intermediate = FALSE) {
  ...
  # Map numeric values to their corresponding labels (for output, not as a factor)
  labeled_data <- sapply(var, function(x) {
    label_idx <- which(levels == x)
    if (length(label_idx) > 0) {
      return(labels[label_idx])
    } else {
      return(NA)  # If no label is found, return NA
    }
  })
...
# Function to apply the codebook transformations for labeled data
apply_codebook_labeling <- function(data, codebook, sep4levels = ",\\s*", sep4name_value = "=", print.intermediate = FALSE) {
  ...
  for (i in 1:nrow(codebook)) {
    ...
    if (var_name %in% colnames(labeled_data)) {
      ...
      # Convert the relevant column in labeled_data using create_labeled_data
      labeled_data[[var_name]] <- create_labeled_data(labeled_data[[var_name]], value_options, sep4levels, sep4name_value, print.intermediate)
      
      if (print.intermediate) {
        print(paste("Labeled data:", paste(labeled_data[[var_name]], collapse = ", ")))
      }
    } else {
      if (print.intermediate) {
