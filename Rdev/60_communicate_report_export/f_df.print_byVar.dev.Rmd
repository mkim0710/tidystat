...
# # cat("    ________________________________________________________________________    \n")
# cat(' Sys.setenv(PARENT_RENDERING = "YES"); "',env1$path$CurrentSource.path.filename.ext %>% {paste0(env1$path$path1,"/",.)},'" |> rmarkdown::render(output_dir = dirname(env1$path$CurrentSource.path.filename.ext), output_format = "html_document"); Sys.setenv(PARENT_RENDERING = "NO")',"\n", 
#     ' if (Sys.info()["sysname"] == "Windows") { system("\\"C:/Program Files (x86)/Microsoft/Edge/Application/msedge_proxy.exe\\" --app=\\"',env1$path$CurrentSource.path.filename.html %>% {paste0(env1$path$path1,"/",.)} |> normalizePath(winslash="/"),'\\"") } else { browseURL("',env1$path$CurrentSource.path.filename.html %>% {paste0(env1$path$path1,"/",.)},'") }',"\n",
