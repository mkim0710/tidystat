...
#     }
# }
# # cat(' Sys.setenv(PARENT_RENDERING = "YES"); "',env1$path$CurrentSource.path_filename.ext %>% {paste0(env1$path$path1,"/",.)},'" |> rmarkdown::render(output_dir = dirname(env1$path$CurrentSource.path_filename.ext), output_format = "pdf_document"); Sys.setenv(PARENT_RENDERING = "NO")',"\n", 
# #     ' if (Sys.info()["sysname"] == "Windows") { env1$env.internal$f_file_PDF.sumatra("',env1$path$CurrentSource.path_filename.pdf %>% {paste0(env1$path$path1,"/",.)},'") } else { browseURL("',env1$path$CurrentSource.path_filename.pdf %>% {paste0(env1$path$path1,"/",.)},'") }',"\n",
# #     ' env1$env.internal$f_filename.ext.createBackup(backup_from_path_filename.ext = "',env1$path$CurrentSource.path_filename.pdf %>% {paste0(env1$path$path1,"/",.)},'", .backup_to_path="-backup", timeFormat="%y%m%d", overwrite=TRUE)',"  \n", sep="")
# # cat("    ________________________________________________________________________    \n")
# cat(' Sys.setenv(PARENT_RENDERING = "YES"); "',env1$path$CurrentSource.path_filename.ext %>% {paste0(env1$path$path1,"/",.)},'" |> rmarkdown::render(output_dir = dirname(env1$path$CurrentSource.path_filename.ext), output_format = "html_document"); Sys.setenv(PARENT_RENDERING = "NO")',"\n", 
#     ' if (Sys.info()["sysname"] == "Windows") { system("\\"C:/Program Files (x86)/Microsoft/Edge/Application/msedge_proxy.exe\\" --app=\\"',env1$path$CurrentSource.path_filename.html %>% {paste0(env1$path$path1,"/",.)} |> normalizePath(winslash="/"),'\\"") } else { browseURL("',env1$path$CurrentSource.path_filename.html %>% {paste0(env1$path$path1,"/",.)},'") }',"\n",
#     ' env1$env.internal$f_filename.ext.createBackup(backup_from_path_filename.ext = "',env1$path$CurrentSource.path_filename.html %>% {paste0(env1$path$path1,"/",.)},'", .backup_to_path="-backup", timeFormat="%y%m%d", overwrite=TRUE)',"  \n", sep="")
```
