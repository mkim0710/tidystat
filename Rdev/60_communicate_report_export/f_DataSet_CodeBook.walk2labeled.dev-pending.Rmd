...
library(dplyr)
library(stringr)
.packagename = "haven"; if (!paste0("package:",.packagename) %in% search()) {library(.packagename, character.only = TRUE)}
...
create_labelled_data <- function(var, value_options, sep4levels = ",\\s*", sep4name_value = "=", VERBOSE = options()$verbose) {
...
apply_codebook_labeling <- function(data, codebook, sep4levels = ",\\s*", sep4name_value = "=", VERBOSE = options()$verbose) {
  ...
  for (i in 1:nrow(codebook)) {
    ...
    if (var_name %in% colnames(labeled_data)) {
      ...
      labeled_data[[var_name]] <- create_labelled_data(labeled_data[[var_name]], value_options, sep4levels, sep4name_value, VERBOSE)
...
labeled_data <- apply_codebook_labeling(your_data, MetaData$tblVarName$tblVarName.suffix, sep4levels = ",\\s*", sep4name_value = "=", VERBOSE = TRUE)


labeled_data |> str(max.level = 2, give.attr = TRUE)
