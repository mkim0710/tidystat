...
TheTrainSet.cv.glmnet_a10.lambda_list.glm %>% map(function(ob) ob %>% tbl_regression(exponentiate = TRUE) %>% as_tibble() )
```
...
summarize_model <- function(cox_model, adjust_p = c("fdr", "bonferroni")) {
  if (!inherits(cox_model, "glm")) {
...
generate_summaries <- function(glmnet_models, train_formula, test_data, lambdas = c("lambda.min", "lambda.1se")) {
  ...
  for (i in 1:nrow(model_combinations)) {
    ...
    cox_model <- f_cv_glmnet_object.lamda_1se.glm(
      cv_glmnet_object = glmnet_model,
    ...
    # Summarize the model
    summary_df <- summarize_model(cox_model, adjust_p = c("fdr", "bonferroni"))
    
    # Store the summary with a descriptive name
    glm_summaries[[model_name]] <- summary_df
  }
  
  return(glm_summaries)
}
```
...
glm_summaries <- generate_summaries(
  glmnet_models = TheTrainSet.cv.glmnet_a10,
...
# write.xlsx(glm_summaries, "glm_Summaries.xlsx", asTable = TRUE)
# 
# # Open the Excel file (optional)
# openXL("glm_Summaries.xlsx")
