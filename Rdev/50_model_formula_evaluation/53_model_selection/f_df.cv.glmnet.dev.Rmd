...
### :: f_cv_glmnet_object.lamda.coxph() ----
### :: runph_on_test() ----
```{r, echo=TRUE, results="markup", collapse=TRUE, paged.print=FALSE, comment="#", R.options=list(width=120)}
# Function to create and run Cox PH model on test set
f_cv_glmnet_object.lamda.coxph <- function(cv_glmnet_object, train_formula, test_data, lambda = "lambda.min") {
    ...
    new_formula <- as.formula(
        paste(response_vars, "~", paste(vec_VarName_lamda, collapse = " + "))
    )
...
coxph_summaries <- generate_summaries(
