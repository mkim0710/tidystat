...
## :: f_cv_glmnet_object.lamda.vec_VarName = function()
### :: f_cv_glmnet_object.lamda_list.vec_VarName = function()
```{r, echo=TRUE, results="markup", collapse=TRUE, paged.print=FALSE, comment="#", R.options=list(width=120)}
f_cv_glmnet_object.lamda.vec_VarName <- function(cv_glmnet_object, lambda = "lambda.min") {
...
f_cv_glmnet_object.lamda_list.vec_VarName = function(cv_glmnet_object, vec_lambda = c("lambda.min", "lambda.1se")) {
    list_VarName_lamda = vec_lambda %>% set_names() %>% map(~ cv_glmnet_object %>% f_cv_glmnet_object.lamda.vec_VarName(lambda = .x))
...
TheTrainSet.cv.glmnet_a10 %>% f_cv_glmnet_object.lamda.vec_VarName(lambda = "lambda.min") %>% dput
cat("    ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~    \n")
TheTrainSet.cv.glmnet_a10 %>% f_cv_glmnet_object.lamda.vec_VarName(lambda = "lambda.1se") %>% dput
...
### :: f_cv_glmnet_object.lamda.glm() ----
```{r, echo=TRUE, results="markup", collapse=TRUE, paged.print=FALSE, comment="#", R.options=list(width=120)}
# Function to create and run Cox PH model on test set
f_cv_glmnet_object.lamda.glm <- function(cv_glmnet_object, formula = as.formula(y ~ .), family = "binomial", test_data, lambda = "lambda.1se") {
    vec_VarName_lamda <- f_cv_glmnet_object.lamda.vec_VarName(cv_glmnet_object, lambda = lambda)
...
cat("    ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++    \n")
cat("    ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~    \n")
TheTrainSet.cv.glmnet_a10.lambda.1se.glm <- 
    TheTrainSet.cv.glmnet_a10 %>% f_cv_glmnet_object.lamda.glm(
        formula = as.formula(y ~ .),
        family = "binomial", 
        test_data = TheTrainSet,
        lambda = "lambda.1se"
    )
TheTrainSet.cv.glmnet_a10.lambda.1se.glm %>% coef %>% exp %>% round(3) %>% dput
library(gtsummary)
cat("    ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++    \n")
TheTrainSet.cv.glmnet_a10.lambda.1se.glm %>% tbl_regression(exponentiate = TRUE) %>% as_tibble()
```
...
generate_summaries <- function(glmnet_models, train_formula, test_data, lambdas = c("lambda.min", "lambda.1se")) {
  ...
  for (i in 1:nrow(model_combinations)) {
    ...
    cox_model <- f_cv_glmnet_object.lamda.glm(
