...
cv_glmnet_object <- TheTrainSet.cv.glmnet.a10
...
## :: f_cv_glmnet_object.vec_VarName.select = function()
### |> f_cv_glmnet_object.vec_VarName.select()
```{r, echo=TRUE, results="markup", collapse=TRUE, paged.print=FALSE, comment="#", R.options=list(width=120)}
f_cv_glmnet_object.vec_VarName.select <- function(cv_glmnet_object, lambda = "lambda.min") {
    # Extract coefficients at specified lambda
    coef_matrix <- coef(cv_glmnet_object, s = lambda)
    
    # Identify non-zero coefficients
    vec_VarName.select <- rownames(coef_matrix)[which(coef_matrix != 0)]
    
    # Check if any variables were selected
    if (length(vec_VarName.select) == 0) {
        message("No variables were selected at the specified lambda.")
        return(NULL)
    }
    
    # Remove the intercept if present
    vec_VarName.select <- vec_VarName.select[vec_VarName.select != "(Intercept)"]
    
    return(vec_VarName.select)
}

# Extract variables for alpha = 1
vec_VarName.select_a1 <- f_cv_glmnet_object.vec_VarName.select(TheTrainSet.cv.glmnet.a10)
vec_VarName.select_a1 %>% dput

# # Extract variables for alpha = 0.5
# vec_VarName.select_a0_5 <- f_cv_glmnet_object.vec_VarName.select(TheTrainSet.cv.glmnet.a05)
# vec_VarName.select_a0_5 %>% dput
...
runph_on_test <- function(cv_glmnet_object, train_formula, test_data, lambda = "lambda.min") {
  vec_VarName.select <- f_cv_glmnet_object.vec_VarName.select(cv_glmnet_object, lambda = lambda)
  
  # Construct new formula
  response_vars <- all.vars(train_formula)[1:2]  # Extract response variables
  new_formula <- as.formula(
    paste(response_vars, "~", paste(vec_VarName.select, collapse = " + "))
  )
