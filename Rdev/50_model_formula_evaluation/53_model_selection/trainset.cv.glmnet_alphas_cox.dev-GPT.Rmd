...
train_cv_glmnet_cox <- function(
  ...
  for (alpha_val in alphas) {
    message(paste("Training glmnet model with alpha =", alpha_val))
    ...
    if (save_models) {
      model_filename <- file.path(output_dir, paste0("cv_glmnet_alpha_", alpha_val, ".rds"))
    ...
    if (save_plots) {
      plot_filename <- file.path(output_dir, paste0("cv_glmnet_alpha_", alpha_val, "_plot.png"))
      png(plot_filename, width = plot_size, height = plot_size, units = "px", bg = "transparent")
      plot(cv_model, main = paste("Alpha =", alpha_val))
      dev.off()
      message(paste("Saved plot to", plot_filename))
    }
    
    # Store the model in the list
    glmnet_models[[paste0("alpha_", alpha_val)]] <- cv_model
  }
...
# saveRDS(TheTrainSet_cv_glmnet_cox, file = "TheTrainSet_cv_glmnet_cox.rds")


# Extract coefficients at lambda.min for alpha = 1
coef_matrix_alpha_1 <- coef(TheTrainSet_cv_glmnet_cox$alpha_1, s = "lambda.min")
print(coef_matrix_alpha_1)

# Extract coefficients at lambda.min for alpha = 0.5
coef_matrix_alpha_0_5 <- coef(TheTrainSet_cv_glmnet_cox$alpha_0.5, s = "lambda.min")
print(coef_matrix_alpha_0_5)
...
selected_vars_alpha_1 <- extract_selected_variables(TheTrainSet_cv_glmnet_cox$alpha_1)
print(selected_vars_alpha_1)

# Extract variables for alpha = 0.5
selected_vars_alpha_0_5 <- extract_selected_variables(TheTrainSet_cv_glmnet_cox$alpha_0.5)
print(selected_vars_alpha_0_5)
...
generate_coxph_summaries <- function(glmnet_models, train_formula, test_data, lambdas = c("lambda.min", "lambda.1se")) {
  ...
  for (i in 1:nrow(model_combinations)) {
    alpha_val <- model_combinations$alpha[i]
    lambda_val <- model_combinations$lambda[i]
    
    model_name <- paste0("alpha", alpha_val, "_", lambda_val)
    
    # Retrieve the glmnet model
    glmnet_model <- glmnet_models[[paste0("alpha_", alpha_val)]]
...
coxph_summaries <- generate_coxph_summaries(
  glmnet_models = TheTrainSet_cv_glmnet_cox,
...
#'
#' @examples
#' glmnet_models <- train_cv_glmnet_cox(
...
generate_coxph_summaries <- function(
  ...
  for (i in 1:nrow(model_combinations)) {
    ...
    model_name <- paste0(alpha_val, "_", lambda_val)
    
    # Retrieve the glmnet model
    glmnet_model <- glmnet_models[[alpha_val]]
