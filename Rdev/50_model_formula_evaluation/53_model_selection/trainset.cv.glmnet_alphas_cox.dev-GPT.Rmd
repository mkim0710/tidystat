...
f_df.cv.glmnet_cox <- function(
  ...
  for (i.alpha in alphas) {
    ...
    if (save_models) {
      ...
      saveRDS(cv_model, model_filename)
    ...
    if (save_plots) {
      ...
      plot(cv_model, main = paste("Alpha =", i.alpha))
      dev.off()
      message(paste("Saved plot to", plot_filename))
    }
    
    # Store the model in the list
    glmnet_models[[paste0("a", sprintf("%02d", i.alpha * 10))]] <- cv_model
  }
...
f_df.cv.glmnet_cox <- function(
  ...
  for (i.alpha in alphas) {
    ...
    # Store the model in the list
    glmnet_models[[paste0("a", i.alpha)]] <- cv_model
  }
...
#'
#' @examples
#' selected_vars <- extract_selected_variables(cv_model, lambda = "lambda.1se")
extract_selected_variables <- function(cv_glmnet_model, lambda = "lambda.min") {
  # Extract coefficients at specified lambda
  coef_matrix <- coef(cv_glmnet_model, s = lambda)
...
#'
#' @examples
#' cox_model <- run_coxph_on_test(cv_model, Surv(time, status) ~ ., test_data)
run_coxph_on_test <- function(cv_glmnet_model, train_formula, test_data, lambda = "lambda.min") {
  selected_vars <- extract_selected_variables(cv_glmnet_model, lambda = lambda)
