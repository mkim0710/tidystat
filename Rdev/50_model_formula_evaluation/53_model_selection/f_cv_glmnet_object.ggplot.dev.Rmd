...
# 
# glmnet_gaussianExample_tibble %>% str
...
glmnet_binomialExample_tibble %>% str
...
# __________|------  
# @@ START) dev) f_glmnet_object.ggplot ----  
```{r env0-NoEchoNoResults, echo=FALSE, results="hide"}
env0 = env1
```

  
# https://chatgpt.com/c/672f1a80-34c8-800e-9d67-0c694bab56c7
## :: glmnet_object = glmnet(x, y)
...
# Fit glmnet model
glmnet_object = glmnet(x, y)
cv_glmnet_object_enet = glmnet(x, y, alpha = 0.5)  # Elastic net
```

### :: f_glmnet_object.plot = function() -dev -tmp(xvar)
```{r, echo=FALSE, results="markup", collapse=TRUE, paged.print=FALSE, comment="", R.options=list(width=120)}
?plot.glmnet
...
# glmnet_object |> plot(label = TRUE, type.coef = "2norm")
glmnet_object |> plot(label = TRUE, xvar = "lambda")
glmnet_object |> plot(label = TRUE, xvar = "dev")
glmnet_object |> plot(label = TRUE, xvar = "norm")
cv_glmnet_object_enet |> plot(label = TRUE, xvar = "norm")
```


#### matrix(1:6, nrow = 2) |> f_matrix.set_names.to_vector()
```{r}
matrix(1:6, nrow = 2) |> f_matrix.set_names.to_vector()
```
...
#### vec_l1norm <- apply(abs(mat_coef[-1, ]), 2, sum)  # Exclude intercept
```{r, echo=FALSE, results="markup", collapse=TRUE, paged.print=FALSE, comment="", R.options=list(width=120)}
# Calculate L1-l1norm for each lambda
vec_l1norm <- apply(abs(mat_coef[-1, ]), 2, sum)  # Exclude intercept
vec_l1norm %>% str
```
...
#### :: coef_df = 
```{r, echo=FALSE, results="markup", collapse=TRUE, paged.print=FALSE, comment="", R.options=list(width=120)}
# Total number of variables (excluding intercept)
nvars <- nrow(mat_coef) - 1

# Create a data frame for plotting
coef_df <- data.frame(
  lambda = rep(vec_lambda, each = nvars),
  l1norm = rep(vec_l1norm, each = nvars),
  coefficient = as.vector(mat_coef[-1, ]),
  variable = rep(rownames(mat_coef)[-1], times = length(vec_lambda))
)

coef_df = coef_df %>% mutate(log_lambda = log(lambda)) 
coef_df %>% as_tibble
coef_df %>% summary
```

### :: f_glmnet_object.tbl_lambda_coefs.pivot_longer = function()
#### :: tbl_lambda_coefs.pivot_longer ====
```{r}
tbl_lambda_coefs = mat_coef[-1, ] %>% t %>% as.data.frame %>% as_tibble(rownames = NA) %>% add_column(lambda = vec_lambda, .before = 1) %>% add_column(l1norm = vec_l1norm, .before = 2)
tbl_lambda_coefs 
tbl_lambda_coefs.pivot_longer = tbl_lambda_coefs %>% pivot_longer(-c(lambda, l1norm), names_to = "variable", values_to = "coefficient")
tbl_lambda_coefs.pivot_longer
```


### :: ...ggplot_log_lambda = function() -dev -temp(scales)
```{r, echo=FALSE, results="markup", collapse=TRUE, paged.print=FALSE, comment="", R.options=list(width=120)}
library(ggplot2)

# Plot coefficient paths with ggplot
tbl_lambda_coefs.pivot_longer %>% 
    filter(variable %in% paste0("V", 1:5)) %>% 
    ggplot(aes(x = log(lambda), y = coefficient, color = variable, group = variable)) +
    geom_line() +
    theme_minimal()
...
tbl_lambda_coefs.pivot_longer %>% 
    filter(variable %in% paste0("V", 1:5)) %>% 
    # ggplot(aes(x = log(lambda), y = coefficient, color = variable, group = variable)) +
    ggplot(aes(x = lambda, y = coefficient, color = variable, group = variable)) +
    scale_x_log10() +  # If lambda values are in log10 scale
    geom_line() +
    theme_minimal()
...
tbl_lambda_coefs.pivot_longer %>% 
    ...
    # scale_x_log10() +  # If lambda values are in log10 scale
    scale_x_continuous(
        trans = scales::log_trans(base = exp(1)),  # Natural logarithm
...
### :: ...ggplot_log_lambda = function() -dev -succeed(line labels) -pending(non-zero vars)
```{r, echo=FALSE, results="markup", collapse=TRUE, paged.print=FALSE, comment="", R.options=list(width=120)}
library(ggplot2)
library(dplyr)
library(scales)
# Calculate the label positions
label_data_start <- tbl_lambda_coefs.pivot_longer %>%
    group_by(variable) %>%
...
tbl_lambda_coefs.pivot_longer %>% 
    ...
    geom_line() +
    geom_text(
        data = label_data_start,
        aes(label = variable),
        hjust = 1.1,  # Shift labels slightly to the left
        size = 3       # Adjust text size as needed
    ) +
    theme_minimal() + 
    labs(
        title = "Coefficient Paths Across Lambda",
        x = expression(lambda~(log[e]~scale)),
        y = "Coefficient Value"
    ) +
    theme_minimal() +
...
### :: ...ggplot_log_lambda = function() -dev -succeed(line labels) -void(non-zero vars)
```{r, echo=FALSE, results="markup", collapse=TRUE, paged.print=FALSE, comment="", R.options=list(width=120)}
library(ggplot2)
library(dplyr)
library(scales)
...
# Calculate the number of non-zero variables for each lambda
non_zero_counts <- tbl_lambda_coefs.pivot_longer %>%
    group_by(lambda) %>%
    summarize(non_zero_count = sum(coefficient != 0)) %>%
    ungroup()

# Add non-zero counts to the original data for alignment
tbl_lambda_coefs.pivot_longer <- tbl_lambda_coefs.pivot_longer %>%
    left_join(non_zero_counts, by = "lambda")
...
tbl_lambda_coefs.pivot_longer %>% 
    ...
    scale_x_continuous(
        ...
        sec.axis = sec_axis(
            ...
            breaks = non_zero_counts$lambda,
            labels = non_zero_counts$non_zero_count,
            name = "Number of Non-Zero Variables"
        )
    ) +
    geom_line() +
    geom_text(
...
### :: ...ggplot_log_lambda = function() -dev -succeed(line labels) -succeed(non-zero vars)
```{r, echo=FALSE, results="markup", collapse=TRUE, paged.print=FALSE, comment="", R.options=list(width=120)}
library(ggplot2)
library(dplyr)
library(scales)
...
# Filter for changes in the number of non-zero variables, including the first row
non_zero_counts_filtered <- non_zero_counts %>%
    filter(row_number() == 1 | non_zero_count != lag(non_zero_count, default = first(non_zero_count)))

# Add the filtered non-zero counts to the original data for alignment
tbl_lambda_coefs.pivot_longer <- tbl_lambda_coefs.pivot_longer %>%
    left_join(non_zero_counts_filtered, by = "lambda", suffix = c("", "_filtered"))
...
tbl_lambda_coefs.pivot_longer %>% 
    ...
    scale_x_continuous(
        ...
        sec.axis = sec_axis(
            transform = ~ .,  # Identity transformation
            breaks = non_zero_counts_filtered$lambda,
            labels = non_zero_counts_filtered$non_zero_count,
            name = "Number of Non-Zero Variables"
        )
    ) +
    geom_line() +
...
### :: ...ggplot_l1norm = function() -dev -pending(line labels) -pending(non-zero vars)
```{r, echo=FALSE, results="markup", collapse=TRUE, paged.print=FALSE, comment="", R.options=list(width=120)}
library(ggplot2)
...
tbl_lambda_coefs.pivot_longer %>% 
    filter(variable %in% paste0("V", 1:5)) %>% 
    ggplot(aes(x = l1norm, y = coefficient, color = variable, group = variable)) +
    geom_line() +
    theme_minimal()

```

### :: ...ggplot_l1norm = function() -dev -succeed(line labels) -pending(non-zero vars)
```{r, echo=FALSE, results="markup", collapse=TRUE, paged.print=FALSE, comment="", R.options=list(width=120)}
library(ggplot2)

# Calculate the label positions
label_data <- tbl_lambda_coefs.pivot_longer %>%
    group_by(variable) %>%
    filter(l1norm == max(l1norm)) %>%
    ungroup()
...
tbl_lambda_coefs.pivot_longer %>% 
    ...
    geom_text(
        data = label_data,
        aes(label = variable),
        hjust = -0.1,  # Slightly shift labels outside the plot
        size = 3       # Adjust text size as needed
    ) +
    theme_minimal() +
    theme(legend.position = "none")  # Remove legend
...
### :: ...ggplot_l1norm = function() -dev -succeed(line labels) -succeed(non-zero vars)
```{r, echo=FALSE, results="markup", collapse=TRUE, paged.print=FALSE, comment="", R.options=list(width=120)}
library(ggplot2)
...
# Calculate the number of non-zero variables for each vec_l1norm
non_zero_counts_l1 <- tbl_lambda_coefs.pivot_longer %>%
    group_by(l1norm) %>%
    summarize(non_zero_count = sum(coefficient != 0)) %>%
    ungroup() %>%
    filter(row_number() == 1 | non_zero_count != lag(non_zero_count, default = first(non_zero_count)))
...
tbl_lambda_coefs.pivot_longer %>% 
    ...
    scale_x_continuous(
        ...
        sec.axis = sec_axis(
            ...
            breaks = non_zero_counts_l1$l1norm,
            labels = non_zero_counts_l1$non_zero_count,
            name = "Number of Non-Zero Variables"
        )
    ) +
    labs(
        title = "Coefficient Paths Across L1 Norm",
        y = "Coefficient Value"
    ) +
    theme_minimal() +
    theme(legend.position = "none")  # Remove legend
...
# __________|------  
# @@ START) dev) f_cv_glmnet_object.ggplot ----  
...
f_df.cv.glmnet <- function(
    ...
    if (save_models) {
        ...
        message(paste0("Saved model to: ", model_filename))
    }
...
#### :: TheTrainSet.cv.glmnet_a10
##### :: TheTrainSet.cv.glmnet_a10.coef = TheTrainSet.cv.glmnet_a10 |> 
```{r, echo=TRUE, results="markup", collapse=TRUE, paged.print=FALSE, comment="#", R.options=list(width=120)}

alpha = 1
# Train glmnet Cox models
TheTrainSet.cv.glmnet_a10 <- f_df.cv.glmnet(
...
# .objectname = "TheTrainSet.cv.glmnet_a10"
...
TheTrainSet.cv.glmnet_a10 %>% f_cv_glmnet_object.mat_coef()
cat("    ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~    \n")
TheTrainSet.cv.glmnet_a10 %>% f_cv_glmnet_object.mat_coef() %>% str()
cat("    ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~    \n")
TheTrainSet.cv.glmnet_a10 %>% f_cv_glmnet_object.mat_coef() %>% as.matrix() %>% as.data.frame() %>% str()
```
...
### :: cv_glmnet_object = TheTrainSet.cv.glmnet_a10
```{r, echo=TRUE, results="markup", collapse=TRUE, paged.print=FALSE, comment="#", R.options=list(width=120)}
cv_glmnet_object = TheTrainSet.cv.glmnet_a10
cv_glmnet_object %>% str(max.level = 2)
```
...
### :: |> plot()
```{r, echo=TRUE, results="markup", collapse=TRUE, paged.print=FALSE, comment="#", R.options=list(width=120)}
TheTrainSet.cv.glmnet_a10 %>% plot
```



### :: f_cv_glmnet_object.plot = function() -old -pending
```{r, echo=TRUE, results="markup", collapse=TRUE, paged.print=FALSE, comment="#", R.options=list(width=120)}
f_cv_glmnet_object.plot <- function(
        cv_glmnet_object, 
        lambda = "lambda.1se", 
        i.alpha = 1,
        family = "binomial",
...
TheTrainSet.cv.glmnet_a10 %>% f_cv_glmnet_object.plot(save_plots = FALSE)

```

  
### :: f_cv_glmnet_object.ggplot = function() -succeed(lambda) -pending(dev: line labels) -succeed(norm)
```{r, echo=FALSE, results="markup", collapse=TRUE, paged.print=FALSE, comment="", R.options=list(width=120)}
options(width=120)
library(ggplot2)
library(dplyr)
...
f_cv_glmnet_object.ggplot <- function(cv_glmnet_object, xvar = c("norm", "lambda", "dev"), label = TRUE) {
    xvar <- match.arg(xvar)
    names(xvar) = xvar
    if (xvar == "dev") names(xvar) = "DevianceExplained"
    if (xvar == "norm") names(xvar) = "l1norm"

    # Extract coefficients for all lambda values
    mat_coef <- as.matrix(coef(cv_glmnet_object$glmnet.fit))
    vec_lambda <- cv_glmnet_object$lambda
    vec_l1norm <- apply(abs(mat_coef[-1, ]), 2, sum)  # Exclude intercept
    
    # Calculate proportion deviance explained if xvar == "dev"
    vec_dev.ratio <- NULL
    if (xvar == "dev") {
        vec_dev.ratio <- cv_glmnet_object$glmnet.fit$dev.ratio
    }
    ...
    coef_df <- mat_coef[-1, ] %>%
        ...
        add_column(lambda = vec_lambda, .before = 1) %>%
        add_column(l1norm = vec_l1norm, .before = 2) %>%  # Rename vec_l1norm to l1norm
        pivot_longer(-c(lambda, l1norm), names_to = "variable", values_to = "coefficient")

    if (xvar == "dev") {
        coef_df <- coef_df %>%
            mutate(DevianceExplained = rep(vec_dev.ratio, each = nrow(mat_coef) - 1))
    }
    ...
    label_data <- coef_df %>%
        group_by(variable) %>%
        filter(
            if (xvar == "norm") l1norm == max(l1norm) else 
            if (xvar == "lambda") lambda == min(lambda) else 
            DevianceExplained == max(DevianceExplained)
        ) %>%
        ungroup()

    # Calculate the number of non-zero variables
    non_zero_counts <- coef_df %>%
        group_by(.data[[names(xvar)]]) %>%
        summarize(non_zero_count = sum(coefficient != 0)) %>%
        ungroup() %>%
        filter(row_number() == 1 | non_zero_count != lag(non_zero_count, default = first(non_zero_count)))
    ...
    if (!any(non_zero_counts$non_zero_count == 0)) {
        non_zero_counts <- bind_rows(
            tibble(!!names(xvar) := min(coef_df[[names(xvar)]]), non_zero_count = 0),
            non_zero_counts
        )
    }
    ...
    # Plotting
    ggplot(coef_df, aes_string(x = names(xvar), y = "coefficient", color = "variable", group = "variable")) +
        geom_line() +
        scale_x_continuous(
            name = xvar_label,
            trans = if (xvar == "lambda") scales::log_trans(base = exp(1)) else "identity",
            breaks = if (xvar == "lambda") scales::trans_breaks("log", function(x) exp(x)) else waiver(),
            labels = if (xvar == "lambda") scales::trans_format("log", scales::math_format(e^.x)) else waiver(),
            sec.axis = sec_axis(
                trans = ~ .,  # Identity transformation
                breaks = non_zero_counts[[names(xvar)]],
                labels = non_zero_counts$non_zero_count,
                name = "Number of Non-Zero Variables"
            )
        ) +
        labs(
            title = "Coefficient Paths Across Regularization",
            y = "Coefficient Value"
        ) +
        theme_minimal() +
        ...
        { if (label) geom_text(
            data = label_data,
            aes(label = variable),
            hjust = if (xvar == "norm") -0.1 else 1.1,
            size = 3
        ) else NULL }
}
...
# Plot with lambda as xvar
f_cv_glmnet_object.ggplot(cv_fit, xvar = "lambda", label = TRUE)

# Plot with deviance explained as xvar
f_cv_glmnet_object.ggplot(cv_fit, xvar = "dev", label = TRUE)

f_cv_glmnet_object.ggplot(cv_fit, xvar = "norm", label = TRUE)
```
  
  
### :: f_cv_glmnet_object.ggplot = function() -succeed(lambda) -succeed(dev) -succeed(norm)
```{r, echo=FALSE, results="markup", collapse=TRUE, paged.print=FALSE, comment="", R.options=list(width=120)}
options(width=120)
f_cv_glmnet_object.ggplot <- function(cv_glmnet_object, xvar = c("norm", "lambda", "dev"), label = TRUE) {
    xvar <- match.arg(xvar)
    ...
    coef_df <- mat_coef[-1, ] %>%
        ...
        as.data.frame() %>%
        as_tibble(rownames = NA) %>%
        add_column(lambda = vec_lambda, l1norm = vec_l1norm, DevianceExplained = vec_dev.ratio, .before = 1) %>%
        pivot_longer(-c(lambda, l1norm, DevianceExplained), names_to = "variable", values_to = "coefficient")

    # Calculate label positions
    label_data <- coef_df %>%
        group_by(variable) %>%
        filter(.data[[names(xvar)]] == if (xvar == "lambda") min(.data[[names(xvar)]]) else max(.data[[names(xvar)]])) %>%
        ungroup()

    # Calculate the number of non-zero variables
    non_zero_counts <- coef_df %>%
        group_by(.data[[names(xvar)]]) %>%
        summarize(non_zero_count = sum(coefficient != 0), .groups = "drop") %>%
        filter(row_number() == 1 | non_zero_count != lag(non_zero_count, default = first(non_zero_count))) %>%
        bind_rows(tibble(!!names(xvar) := min(coef_df[[names(xvar)]]), non_zero_count = 0)) %>%
        arrange(.data[[names(xvar)]])
    ...
    # Determine axis transformation for lambda
    x_trans <- if (xvar == "lambda") scales::log_trans(base = exp(1)) else "identity"
    x_breaks <- if (xvar == "lambda") scales::trans_breaks("log", function(x) exp(x)) else waiver()
    ...
    ggplot(coef_df, aes_string(x = names(xvar), y = "coefficient", color = "variable", group = "variable")) +
        ...
        { if (label) geom_text(
            data = label_data,
            aes(label = variable),
            hjust = if (xvar == "lambda") 1.1 else -0.1,  # Adjust based on xvar
            size = 3
        ) else NULL }
}
...
# Example: Using "lambda" as x-axis
f_cv_glmnet_object.ggplot(cv_fit, xvar = "lambda", label = TRUE)

# Example: Using "norm" as x-axis
f_cv_glmnet_object.ggplot(cv_fit, xvar = "norm", label = TRUE)

# Example: Using "dev" as x-axis
f_cv_glmnet_object.ggplot(cv_fit, xvar = "dev", label = TRUE)
...
f_cv_glmnet_object.ggplot <- function(cv_glmnet_object, xvar = c("norm", "lambda", "dev"), label = TRUE) {
    ...
    label_positions <- coef_df %>%
        group_by(variable) %>%
        filter(
            .data[[names(xvar)]] == if (xvar == "lambda") min(.data[[names(xvar)]]) else max(.data[[names(xvar)]])
        ) %>%
        ungroup()
    ...
    if (!any(non_zero_counts$non_zero_count == 0)) {
        initial_row <- tibble(
            !!names(xvar) := min(coef_df[[names(xvar)]]),
            non_zero_count = 0
        )
        non_zero_counts <- bind_rows(initial_row, non_zero_counts)
        non_zero_counts <- non_zero_counts %>% arrange(.data[[names(xvar)]])
    }
    ...
    x_labels <- if (xvar == "lambda") scales::trans_format("log", scales::math_format(e^.x)) else waiver()
    hjust_value <- if (xvar == "lambda") 1.1 else -0.1  # Adjust label position
    
    # Create the plot
    plot <- ggplot(coef_df, aes(x = .data[[names(xvar)]], y = coefficient, color = variable, group = variable)) +
        geom_line() +
        scale_x_continuous(
            name = xvar_labels[[names(xvar)]],
...
# Fit cross-validated glmnet model
cv_fit <- cv.glmnet(x, y)

# Plot with lambda as x-axis
f_cv_glmnet_object.ggplot(cv_fit, xvar = "lambda", label = TRUE)

# Plot with L1 Norm as x-axis
f_cv_glmnet_object.ggplot(cv_fit, xvar = "norm", label = TRUE)

# Plot with Proportion Deviance Explained as x-axis
f_cv_glmnet_object.ggplot(cv_fit, xvar = "dev", label = TRUE)


# Save the plot object
plot_obj <- f_cv_glmnet_object.ggplot(cv_fit, xvar = "norm", label = TRUE)
...
### :: f_cv_glmnet_object_enet.ggplot = function() pending
```{r, echo=FALSE, results="markup", collapse=TRUE, paged.print=FALSE, comment="", R.options=list(width=120)}
options(width=120)
f_cv_glmnet_object_enet.ggplot <- function(cv_glmnet_object, xvar = c("norm", "lambda", "dev", "penalty"), label = TRUE) {
    # Load necessary libraries
    library(ggplot2)
    library(dplyr)
    library(tidyr)
    ...
    alpha <- glmnet_object$call$alpha
    if (!is.numeric(alpha)) alpha = get(alpha)
    if (is.null(alpha)) alpha <- 1  # Default to Lasso if alpha is not specified
...
# Fit cross-validated Elastic Net model with alpha = 0.5
cv_glmnet_object_enet <- cv.glmnet(x, y, alpha = 0.5)

# Plot with Elastic Net penalty as x-axis
f_cv_glmnet_object_enet.ggplot(cv_glmnet_object_enet, xvar = "lambda", label = TRUE)
f_cv_glmnet_object_enet.ggplot(cv_glmnet_object_enet, xvar = "dev", label = TRUE)
f_cv_glmnet_object_enet.ggplot(cv_glmnet_object_enet, xvar = "norm", label = TRUE)
f_cv_glmnet_object_enet.ggplot(cv_glmnet_object_enet, xvar = "penalty", label = TRUE)


f_cv_glmnet_object_enet.ggplot(cv_glmnet_object, xvar = "norm", label = TRUE)
f_cv_glmnet_object_enet.ggplot(cv_glmnet_object, xvar = "penalty", label = TRUE)
