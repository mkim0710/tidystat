...
mat_coef <- as.matrix(coef(glmnet_object))
mat_coef[1:9, 1:9]
mat_coef |> str(max.level = 2, give.attr = TRUE)
mat_coef[-1, ] |> str(max.level = 2, give.attr = TRUE)
mat_coef[-1, ] %>% as.vector() |> str(max.level = 2, give.attr = TRUE)
```
...
vec_lambda <- glmnet_object$lambda
vec_lambda |> str(max.level = 2, give.attr = TRUE)
```
...
vec_l1norm <- apply(abs(mat_coef[-1, ]), 2, sum)  # Exclude intercept
vec_l1norm |> str(max.level = 2, give.attr = TRUE)
```
...
cat("    ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~    \n")
TheTrainSet.cv.glmnet_a10 %>% f_cv_glmnet_object.mat_coef() |> str(max.level = 2, give.attr = TRUE)
cat("    ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~    \n")
TheTrainSet.cv.glmnet_a10 %>% f_cv_glmnet_object.mat_coef() %>% as.matrix() %>% as.data.frame() |> str(max.level = 2, give.attr = TRUE)
```
...
```{r, echo=TRUE, results="markup", collapse=TRUE, paged.print=FALSE, comment="#", R.options=list(width=120)}
cv_glmnet_object = TheTrainSet.cv.glmnet_a10
cv_glmnet_object |> str(max.level = 2, give.attr = TRUE)
```
...
f_cv_glmnet_object.ggplot <- function(cv_glmnet_object, xvar = c("norm", "lambda", "dev"), label = TRUE) {
    ...
    # Plotting
    ggplot(coef_df, aes_string(x = xvar_name, y = "coefficient", color = "variable", group = "variable")) +
        geom_line() +
        scale_x_continuous(
            name = xvar_label,
...
f_cv_glmnet_object.ggplot <- function(cv_glmnet_object, xvar = c("norm", "lambda", "dev"), label = TRUE) {
    ...
    label_data <- coef_df %>%
        group_by(variable) %>%
        filter(.data[[xvar_name]] == if (xvar == "lambda") min(.data[[xvar_name]]) else max(.data[[xvar_name]])) %>%
        ungroup()
    ...
    non_zero_counts <- coef_df %>%
        ...
        filter(row_number() == 1 | non_zero_count != lag(non_zero_count, default = first(non_zero_count))) %>%
        bind_rows(tibble(!!xvar_name := min(coef_df[[xvar_name]]), non_zero_count = 0)) %>%
        arrange(.data[[xvar_name]])
...
### :: f_cv_glmnet_object_enet.ggplot = function() -pending(i.alpha)
```{r, echo=FALSE, results="markup", collapse=TRUE, paged.print=FALSE, comment="", R.options=list(width=120)}
options(width=120)
f_cv_glmnet_object_enet.ggplot <- function(cv_glmnet_object, xvar = c("norm", "lambda", "dev", "penalty"), label = TRUE) {
    ...
    alpha <- tryCatch(
        eval(glmnet_object$call$alpha, envir = environment(cv_glmnet_object$glmnet.fit$call)),
...
# Alpha supplied as a variable
i.alpha <- 0.5
cv_glmnet_object_enet <- cv.glmnet(x, y, alpha = i.alpha)

# Plot with Elastic Net penalty as x-axis
# Provide 'alpha' explicitly
# f_cv_glmnet_object_enet.ggplot(cv_glmnet_object_enet, xvar = "penalty", label = TRUE, alpha = i.alpha)

# Alternatively, if 'alpha' is not provided, the function attempts to retrieve it
# If 'alpha' cannot be retrieved, it defaults to 'alpha = 1' and issues a warning
f_cv_glmnet_object_enet.ggplot(cv_glmnet_object_enet, xvar = "lambda", label = TRUE)
f_cv_glmnet_object_enet.ggplot(cv_glmnet_object_enet, xvar = "dev", label = TRUE)
f_cv_glmnet_object_enet.ggplot(cv_glmnet_object_enet, xvar = "norm", label = TRUE)
f_cv_glmnet_object_enet.ggplot(cv_glmnet_object_enet, xvar = "penalty", label = TRUE)
...
cv_glmnet_object <- cv.glmnet(x, y)

f_cv_glmnet_object_enet.ggplot(cv_glmnet_object, xvar = "norm", label = TRUE)
f_cv_glmnet_object_enet.ggplot(cv_glmnet_object, xvar = "penalty", label = TRUE)
f_cv_glmnet_object.ggplot(cv_glmnet_object, xvar = "norm", label = TRUE)
