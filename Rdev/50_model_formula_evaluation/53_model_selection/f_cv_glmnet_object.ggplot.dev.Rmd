...
coefs <- as.matrix(coef(glmnet_object))
coefs[1:9, 1:9]
coefs %>% str
coefs[-1, ] %>% str
coefs[-1, ] %>% as.vector() %>% str
```


```{r, echo=FALSE, results="markup", collapse=TRUE, paged.print=FALSE, comment="", R.options=list(width=120)}
# Extract lambda values
lambdas <- glmnet_object$lambda
lambdas %>% str
```


```{r, echo=FALSE, results="markup", collapse=TRUE, paged.print=FALSE, comment="", R.options=list(width=120)}
# Calculate L1-norm for each lambda
l1norm <- apply(abs(coefs[-1, ]), 2, sum)  # Exclude intercept
l1norm %>% str
```
...
coef_df <- data.frame(
  lambda = rep(lambdas, each = nvars),
  l1norm = rep(l1norm, each = nvars),
  coefficient = as.vector(coefs[-1, ]),
  variable = rep(rownames(coefs)[-1], times = length(lambdas))
)
...
```{r}
tbl_lambda_coefs = coefs[-1, ] %>% t %>% as.data.frame %>% as_tibble(rownames = NA) %>% add_column(lambda = lambdas, .before = 1) %>% add_column(l1norm = l1norm, .before = 2)
tbl_lambda_coefs 
...
f_cv_glmnet_object.ggplot <- function(cv_glmnet_object, xvar = c("norm", "lambda", "dev"), label = TRUE) {
    ...
    # Extract coefficients for all lambda values
    coefs <- as.matrix(coef(cv_glmnet_object$glmnet.fit))
    lambdas <- cv_glmnet_object$lambda
    norm_values <- apply(abs(coefs[-1, ]), 2, sum)  # Exclude intercept

    # Calculate proportion deviance explained if xvar == "dev"
    deviance_explained <- NULL
    if (xvar == "dev") {
        deviance_explained <- cv_glmnet_object$glmnet.fit$dev.ratio
    }
    ...
    coef_df <- coefs[-1, ] %>%
        ...
        add_column(lambda = lambdas, .before = 1) %>%
        add_column(norm = norm_values, .before = 2) %>%  # Rename l1_norm to norm
        pivot_longer(-c(lambda, norm), names_to = "variable", values_to = "coefficient")

    if (xvar == "dev") {
        coef_df <- coef_df %>%
            mutate(dev = rep(deviance_explained, each = nrow(coefs) - 1))
    }
    ...
    label_data <- coef_df %>%
        group_by(variable) %>%
        filter(
            if (xvar == "norm") norm == max(norm) else 
...
f_cv_glmnet_object.ggplot <- function(cv_glmnet_object, xvar = c("norm", "lambda", "dev"), label = TRUE) {
    ...
    coef_df <- coefs[-1, ] %>%
        ...
        as.data.frame() %>%
        as_tibble(rownames = NA) %>%
        add_column(lambda = lambdas, norm = norm_values, dev = deviance_explained, .before = 1) %>%
        pivot_longer(-c(lambda, norm, dev), names_to = "variable", values_to = "coefficient")
...
f_cv_glmnet_object.ggplot <- function(cv_glmnet_object, xvar = c("norm", "lambda", "dev"), label = TRUE) {
    ...
    glmnet_fit <- cv_glmnet_object$glmnet.fit
    coefs <- as.matrix(coef(glmnet_fit))
    lambdas <- glmnet_fit$lambda
    variables <- rownames(coefs)[-1]  # Exclude intercept
    
    # Calculate measures for x-axis
    coef_matrix <- coefs[-1, ]  # Remove intercept
    norm_values <- apply(abs(coef_matrix), 2, sum)
    deviance_explained <- glmnet_fit$dev.ratio  # Proportion of deviance explained
    
    # Prepare data frame
    coef_df <- as.data.frame(t(coef_matrix))
    coef_df$lambda <- lambdas
    coef_df$norm <- norm_values
    coef_df$dev <- deviance_explained
    coef_df <- coef_df %>%
