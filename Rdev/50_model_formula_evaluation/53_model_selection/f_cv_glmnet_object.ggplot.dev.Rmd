...
f_cv_glmnet_object.ggplot <- function(cv_glmnet_object, xvar = c("norm", "lambda", "dev"), label = TRUE) {
    ...
    l1_norm <- apply(abs(coefs[-1, ]), 2, sum)  # Exclude intercept
    ...
    coef_df <- coefs[-1, ] %>%
        ...
        add_column(l1_norm = l1_norm, .before = 2) %>%
        pivot_longer(-c(lambda, l1_norm), names_to = "variable", values_to = "coefficient")

    if (xvar == "dev") {
        coef_df <- coef_df %>% mutate(dev = rep(deviance_explained, each = nrow(coefs) - 1))
    }

    # Calculate label positions
    label_data <- coef_df %>%
        group_by(variable) %>%
        filter(if (xvar == "norm") l1_norm == max(l1_norm) else if (xvar == "lambda") lambda == min(lambda) else dev == max(dev)) %>%
        ungroup()
    ...
    if (xvar == "dev" && !any(non_zero_counts$non_zero_count == 0)) {
    ...
    ggplot(coef_df, aes_string(x = xvar, y = "coefficient", color = "variable", group = "variable")) +
        ...
        { if (label) geom_text(data = label_data, aes(label = variable), hjust = if (xvar == "norm") -0.1 else 1.1, size = 3) else NULL }
}
