...
f_cv_glmnet_object.ggplot <- function(cv_glmnet_object, xvar = c("norm", "lambda", "dev"), label = TRUE) {
    ...
    if (!any(non_zero_counts$non_zero_count == 0)) {
        non_zero_counts <- bind_rows(
            tibble(!!xvar := min(coef_df[[xvar]]), non_zero_count = 0),
            non_zero_counts
        )
    }
...
f_cv_glmnet_object.ggplot <- function(cv_glmnet_object, xvar = c("norm", "lambda", "dev"), label = TRUE) {
    ...
    label_data <- coef_df %>%
        group_by(variable) %>%
        filter(.data[[xvar]] == if (xvar == "lambda") min(.data[[xvar]]) else max(.data[[xvar]])) %>%
        ungroup()
    ...
    non_zero_counts <- coef_df %>%
        ...
        filter(row_number() == 1 | non_zero_count != lag(non_zero_count, default = first(non_zero_count))) %>%
        bind_rows(tibble(!!names(xvar) := min(coef_df[[xvar]]), non_zero_count = 0)) %>%
        arrange(.data[[xvar]])
...
f_cv_glmnet_object.ggplot <- function(cv_glmnet_object, xvar = c("norm", "lambda", "dev"), label = TRUE) {
    ...
    label_positions <- coef_df %>%
        group_by(variable) %>%
        filter(
            .data[[xvar]] == if (xvar == "lambda") min(.data[[xvar]]) else max(.data[[xvar]])
        ) %>%
        ungroup()
    ...
    if (!any(non_zero_counts$non_zero_count == 0)) {
        initial_row <- tibble(
            !!xvar := min(coef_df[[xvar]]),
            non_zero_count = 0
        )
        non_zero_counts <- bind_rows(initial_row, non_zero_counts)
        non_zero_counts <- non_zero_counts %>% arrange(.data[[xvar]])
    }
    ...
    # Create the plot
    plot <- ggplot(coef_df, aes(x = .data[[xvar]], y = coefficient, color = variable, group = variable)) +
        geom_line() +
        scale_x_continuous(
            name = xvar_labels[[xvar]],
