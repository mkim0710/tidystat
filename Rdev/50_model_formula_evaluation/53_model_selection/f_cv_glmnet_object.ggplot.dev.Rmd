...
# Calculate the number of non-zero variables for each l1_norm
...
f_cv_glmnet_object.ggplot <- function(cv_glmnet_object, xvar = c("norm", "lambda", "dev"), label = TRUE) {
    ...
    coef_df <- mat_coef[-1, ] %>%
        ...
        add_column(l1norm = vec_l1norm, .before = 2) %>%  # Rename l1_norm to l1norm
...
f_cv_glmnet_object_enet.ggplot <- function(cv_glmnet_object, xvar = c("norm", "lambda", "dev", "penalty"), label = TRUE) {
    ...
    l1_norm <- apply(abs(mat_coef[-1, ]), 2, sum)
    l2_norm <- apply(mat_coef[-1, ]^2, 2, sum)
    
    # Compute Elastic Net penalty
    penalty_values <- (1 - alpha) * 0.5 * l2_norm + alpha * l1_norm
    ...
    coef_df <- as.data.frame(t(mat_coef[-1, ]))
    coef_df$lambda <- vec_lambda
    coef_df$norm <- l1_norm  # For compatibility
    coef_df$DevianceExplained <- vec_dev.ratio
    coef_df$penalty <- penalty_values
    coef_df <- coef_df %>%
        pivot_longer(cols = all_of(variables), names_to = "variable", values_to = "coefficient")
