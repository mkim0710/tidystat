...
f_cv_glmnet_object.ggplot <- function(cv_glmnet_object, xvar = c("norm", "lambda", "dev"), label = TRUE) {
    ...
    if (xvar == "dev") {
        coef_df <- coef_df %>%
            mutate(dev = rep(vec_dev.ratio, each = nrow(mat_coef) - 1))
    }
    ...
    xvar_label <- switch(
        ...
        dev = "Proportion Deviance Explained"
    )
...
f_cv_glmnet_object.ggplot <- function(cv_glmnet_object, xvar = c("norm", "lambda", "dev"), label = TRUE) {
    ...
    coef_df <- mat_coef[-1, ] %>%
        ...
        as.data.frame() %>%
        as_tibble(rownames = NA) %>%
        add_column(lambda = vec_lambda, l1norm = vec_l1norm, dev = vec_dev.ratio, .before = 1) %>%
        pivot_longer(-c(lambda, l1norm, dev), names_to = "variable", values_to = "coefficient")
    ...
    non_zero_counts <- coef_df %>%
        ...
        filter(row_number() == 1 | non_zero_count != lag(non_zero_count, default = first(non_zero_count))) %>%
        bind_rows(tibble(!!xvar := min(coef_df[[xvar]]), non_zero_count = 0)) %>%
        arrange(.data[[xvar]])
