...
# @@ RENDER) Rdev/50_model_formula_evaluation/53_model_selection/f_df.cv.glmnet.source-dev.Rmd
...
### :: f_cv_glmnet_object.lamda_1se.vec_VarName = function()
#### :: f_cv_glmnet_object.lamda_list.vec_VarName = function()
```{r, echo=TRUE, results="markup", collapse=TRUE, paged.print=FALSE, comment="#", R.options=list(width=120)}
...
# }
# 
# f_cv_glmnet_object.lamda_list.vec_VarName = function(cv_glmnet_object, vec_lambda = c("lambda.min", "lambda.1se")) {
...
TheTrainSet.cv.glmnet_a10 %>% f_cv_glmnet_object.lamda_1se.vec_VarName(lambda = "lambda.min") %>% dput
cat("    ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~    \n")
TheTrainSet.cv.glmnet_a10 %>% f_cv_glmnet_object.lamda_1se.vec_VarName(lambda = "lambda.1se") %>% dput

cat("    ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++    \n")
TheTrainSet.cv.glmnet_a10 %>% f_cv_glmnet_object.lamda_list.vec_VarName() %>% str()

```



### :: f_cv_glmnet_object.lamda_1se.glm() ----
```{r, echo=TRUE, results="markup", collapse=TRUE, paged.print=FALSE, comment="#", R.options=list(width=120)}
...
cat("    ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++    \n")
cat("    ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~    \n")
TheTrainSet.cv.glmnet_a10.lambda_1se.glm <- 
    TheTrainSet.cv.glmnet_a10 %>% f_cv_glmnet_object.lamda_1se.glm(
        formula = as.formula(y ~ .),
        family = "binomial", 
        test_data = TheTrainSet,
        lambda = "lambda.1se"
    ) 
TheTrainSet.cv.glmnet_a10.lambda_1se.glm %>% coef %>% exp %>% round(3) %>% dput
cat("    ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++    \n")
library(gtsummary)
# TheTrainSet.cv.glmnet_a10.lambda_1se.glm %>% tbl_regression(exponentiate = TRUE) %>% as_tibble()
# TheTrainSet.cv.glmnet_a10.lambda_1se.glm %>% tbl_regression(exponentiate = TRUE) %>% as_tibble() %>% f_expression.eval.withCallingHandlers.attr_warnings.summary()
```


### :: f_cv_glmnet_object.lamda_list.glm() ----
```{r, echo=TRUE, results="markup", collapse=TRUE, paged.print=FALSE, comment="#", R.options=list(width=120)}
...
cat("    ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++    \n")
cat("    ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~    \n")
TheTrainSet.cv.glmnet_a10.lambda_list.glm <- 
    TheTrainSet.cv.glmnet_a10 %>% f_cv_glmnet_object.lamda_list.glm(
        formula = as.formula(y ~ .),
        family = "binomial", 
        test_data = TheTrainSet
    )
TheTrainSet.cv.glmnet_a10.lambda_list.glm %>% str(max.level = 2, give.attr = FALSE)
TheTrainSet.cv.glmnet_a10.lambda_list.glm %>% map(function(ob) ob %>% tbl_regression(exponentiate = TRUE) %>% as_tibble() ) %>% f_list_tibble.print_by_element() %>% f_expression.eval.withCallingHandlers.attr_warnings.summary()
```
