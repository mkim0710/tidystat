...
# _______________
```{comment Rstudio-RMarkDown-Shortcuts, include=FALSE, eval=FALSE}
...
# cat(deparse(knitr::opts_chunk$get(), width.cutoff=100), '\n', sep='')
...
knitr::opts_chunk$set(
  eval=TRUE, echo=TRUE, results='hold', collapse=TRUE, 
...
sourcename = "f_df.t.tribble_construct"; source(file.path(file.path(env.custom$path$source_base_local,""), paste0(sourcename,".source.r")) )
...
```{r, paged.print=FALSE, collapse=TRUE, include=FALSE}
...
env.custom$info$DocumentTitle0 = "Blank) f_df.t.tribble_construct"
...
cat(env.custom$info$DocumentTitle1, "\r\n", 
    env.custom$info$DocumentTitle1, ".r", "\r\n", 
    env.custom$info$DocumentTitle1, " -dev.r", "\r\n", 
    env.custom$info$DocumentTitle1, " -clean.r", "\r\n", 
    env.custom$info$DocumentTitle1, ".Rmd", "\r\n", 
    env.custom$info$DocumentTitle1, " -dev.Rmd", "\r\n", 
    env.custom$info$DocumentTitle1, " -clean.Rmd", "\r\n", 
    sep="")
```

#### \% env.custom\$info\$info_system_info
```{r envCustom-Info-NoEchoNoMsgNoResult, echo=FALSE, warning=TRUE, error=TRUE, message=FALSE, results='hide', collapse=TRUE, paged.print=FALSE}
sourcename = "get_system_info"; source(file.path(file.path(env.custom$path$source_base_local,""), paste0(sourcename,".source.r")) )
...
```{r envCustom-Path-NoEcho, echo=FALSE, warning=TRUE, error=TRUE, message=TRUE, results='hold', collapse=TRUE, paged.print=FALSE}
...
```{r, results="hold", collapse=TRUE, paged.print=FALSE, comment='', R.options=list(width=120)}
options(width=120)
...
```{r objectname-read.checkEntity, echo=TRUE, warning=TRUE, error=TRUE, message=TRUE, results='hold', collapse=TRUE, paged.print=FALSE, comment='', R.options=list(width=120)}
options(width=120)
if (getwd() != path4read) warning("getwd() != path4read == ") else cat("getwd() == path4read == "); dput(path4read)  #----
...
# {r plotWidth10Height6, results='hold', collapse=TRUE, paged.print=FALSE, fig.width=10, fig.height=6}
library(tidyverse)
# library(survival)
# library(survminer)
...
# # library(survminer)
# ?survival::lung
# df_time2event = survival::lung %>% mutate(event = as.logical(status-1), Group = c("Male", "Female")[sex] %>% as.factor, StudyPopulation = time >= 30) %>% as_tibble
...
# @@ END -----
```{r END-NoEvalNoEchoNoMsgNoResult, eval=FALSE, echo=FALSE, warning=TRUE, error=TRUE, message=FALSE, results='hide', collapse=TRUE, paged.print=FALSE}
# @@ END -----
```
...
```{r writeRDS-NoEvalNoEchoNoMsgNoResult, eval=FALSE, echo=FALSE, warning=TRUE, error=TRUE, message=FALSE, results='hide', collapse=TRUE, paged.print=FALSE}
# 
# @ write_rds( get(objectname), file.path(path4write, paste0(objectname,".rds"))) -----
# objectname = "df_NewDMv3.NAtime200621"
