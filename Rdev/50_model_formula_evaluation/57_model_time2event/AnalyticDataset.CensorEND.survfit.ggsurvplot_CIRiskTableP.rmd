...
#     )
as1_7.na.Date.fct.cumulative_DM.list$Time2Event_NewDMv3 %>% {cat("> str(",deparse(substitute(.)),")","\n", sep=""); str(.)}
...
#     )
as1_7.na.Date.fct.cumulative_DM.list$Time2Censor_NewDMv3 %>% {cat("> str(",deparse(substitute(.)),")","\n", sep=""); str(.)}
...
df_NewDMv3.Time2Censor_NewDMv3 = as1_7.na.Date.fct.cumulative_DM.list$Time2Censor_NewDMv3 %>% 
    ...
    as_tibble
df_NewDMv3.Time2Censor_NewDMv3
df_NewDMv3.Time2Censor_NewDMv3 %>% {cat("> str(",deparse(substitute(.)),")","\n", sep=""); str(.)}
df_NewDMv3.Time2Censor_NewDMv3 %>% select(time, event, Group, StudyPopulation) %>% by(.$StudyPopulation, summary)
df_NewDMv3.CensorEND %>% {cat("> str(",deparse(substitute(.)),")","\n", sep=""); str(.)}
...
# # library(survminer)
# ?survival::lung
# df_time2event = survival::lung %>% mutate(event = as.logical(status-1), Group = c("Male", "Female")[sex] %>% as.factor, StudyPopulation = time >= 30) %>% {cat("> as_tibble(",deparse(substitute(.)),")","\n", sep=""); as_tibble(.)}
# # df_time2event %>% select(sex, Group) %>% {cat("> str(",deparse(substitute(.)),")","\n", sep=""); str(.)}
