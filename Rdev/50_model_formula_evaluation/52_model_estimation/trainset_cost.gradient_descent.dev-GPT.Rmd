...
# Sample data: A simple dataset for binary classification
df_2vars <- data.frame(
  x1 = c(2.3, 1.9, 3.2, 2.9, 1.5, 1.1, 2.0, 3.5),
  x2 = c(1.2, 2.5, 1.7, 2.9, 1.5, 0.8, 1.1, 3.1),
  y  = c(0, 0, 1, 1, 0, 0, 0, 1)
)
...
# __________|------  
# @@ START) dev ----  
# https://chatgpt.com/c/6715514b-c7fc-800e-ab4d-02b273da8485
...
```{r, echo=TRUE, results="markup", collapse=TRUE, paged.print=FALSE, comment="#", R.options=list(width=120)}
options(width=120)
for(.packagename in c("stats")) {if(!require(.packagename,character.only=TRUE)) install.packages(.packagename)  ;  library(.packagename,character.only=TRUE)}  
```
...
# Fit logistic regression using glm() with the 'binomial' family
data.glm <- glm(y ~ ., data = data, family = binomial())

# Display model summary (coefficients, log-likelihood, etc.)
summary(data.glm)
data.glm |> coef() |> round(3) |> deparse() |> cat("\n")


# Extract log-likelihood from glm()
logLik(data.glm)  # This gives the log-likelihood of the model

# Calculate negative log-likelihood (cost)
data.glm.neg_logLik <- -logLik(data.glm)
cat("Negative log-likelihood from glm():", data.glm.neg_logLik, "\n")
```
...
gradient_descent <- function(X, y, w, learning_rate, num_iterations) {
  ...
  return(list(weights = w, costs = costs))
}

# Gradient Descent function with early stopping
gradient_descent <- function(X, y, w, learning_rate, num_iterations, early_stop_threshold = 1e-6) {
  costs <- numeric(num_iterations)
  for (i in 1:num_iterations) {
    w <- w - learning_rate * gradient(X, y, w)
    costs[i] <- cost_function(X, y, w)
    ...
    if (i > 1 && abs(costs[i] - costs[i - 1]) < early_stop_threshold) {
      cat("Early stopping at iteration", i, "due to small cost change:", abs(costs[i] - costs[i - 1]), "\n")
      costs <- costs[1:i]  # Truncate the cost history
      break
    }
  }
  return(list(weights = w, costs = costs))
...
# Sample data
X <- as.matrix(cbind(1, data$x1, data$x2)) # Add a bias term (intercept)
y <- as.matrix(data$y)
initial_w <- matrix(0, nrow = ncol(X), ncol = 1) # Initialize weights to 0

# Hyperparameters
# learning_rate <- 0.01  # A typical range to try would be between 0.1 and 0.001.
learning_rate <- 0.1  # A typical range to try would be between 0.1 and 0.001.
num_iterations <- 10^7

# Run gradient descent
(gd_result <- gradient_descent(X, y, initial_w, learning_rate, num_iterations)) |> system.time() |> round(3) |> unclass() |> deparse() |> cat("\n")

# Final weights after gradient descent
final_weights <- gd_result$weights
cat("Final weights:\n")
final_weights |> round(3) |> deparse() |> cat("\n")
## Cf) coef() from glm(): c("(Intercept)" = -141.292, x1 = 44.943, x2 = 12.365)  

# Apply gradient descent and observe cost
final_cost <- cost_function(X, y, gd_result$weights)
cat("Final cost after gradient descent:", final_cost, "\n")
## Cf) Negative log-likelihood from glm(): 1.830736e-10 
```

    > learning_rate <- 0.01
    > num_iterations <- 10^7
    c(user.self = 61.274, sys.self = 0.031, elapsed = 61.243, user.child = 0,  sys.child = 0) 
    ## structure(c(-42.126, 13.519, 3.582), dim = c(3L, 1L))
    ## Final cost after gradient descent: 0.000383159 


#### gd_result$costs |> plot() ----  
```{r, echo=TRUE, results="markup", collapse=TRUE, paged.print=FALSE, comment="#", R.options=list(width=120)}

# plot(gd_result$costs, type = 'l', col = 'blue', lwd = 2, 
#      main = 'Cost Function Convergence', xlab = 'Iterations', ylab = 'Cost')
# Plot cost over iterations with log scale on y-axis
plot(gd_result$costs, type = 'l', col = 'blue', lwd = 2, 
     main = 'Cost Function Convergence (Log Scale)', xlab = 'Iterations', ylab = 'Cost (Log Scale)', 
     log = 'y')
