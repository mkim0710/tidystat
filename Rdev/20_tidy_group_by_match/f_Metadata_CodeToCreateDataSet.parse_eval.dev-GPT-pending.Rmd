...
# _______________  
# @@ START) function ----  
# https://chatgpt.com/c/670ddcac-976c-800e-ae18-f9847c6ba616?model=o1-mini

## \% |> initialize_metadata()  
## \% |> add_dataset_to_metadata()  
```{r, echo=TRUE, results="markup", collapse=TRUE, paged.print=FALSE, comment="#", R.options=list(width=120)}
options(width=120)
# 
...
# library(tibble)
# 
# # Function to initialize MetaData if it doesn't exist
# initialize_metadata <- function() {
#   if (!exists("MetaData", envir = .GlobalEnv)) {
#     assign("MetaData", list(CodeBook = list(), DataSets = list()), envir = .GlobalEnv)
#     message("Initialized MetaData with empty CodeBook and DataSets.")
#   }
#   
#   # Load CodeBook if the file exists
#   if (file.exists("CodeBook.xlsx")) {
#     MetaData$CodeBook <- read_xlsx("CodeBook.xlsx")
#     message("Loaded CodeBook from 'CodeBook.xlsx'.")
#   } else {
#     message("No 'CodeBook.xlsx' found. Continuing without loading CodeBook.")
#   }
# }
# 
# # Enhanced function to create or update a dataset with metadata
# add_dataset_to_metadata <- function(DataSetNameShortcut.old = NULL, 
#                                    DataSetNameShortcut.new.suffix, 
#                                    CodeToCreateDataSet, 
#                                    additional_metadata = list()) {
#   
#   # Initialize MetaData if not already initialized
#   initialize_metadata()
#   
#   # If creating the first dataset, DataSetNameShortcut.old is NULL
#   if (is.null(DataSetNameShortcut.old)) {
#     # Extract a unique shortcut for the first dataset, e.g., "DataSet0"
#     DataSetNameShortcut.new <- paste0("DataSet", length(MetaData$DataSets))
#     
#     # Assign a unique dataset name if needed
#     # Assuming the first dataset has no suffix
#     DataSetName.new <- "analyticDF_time2event"
#     
#     # Add the first dataset to MetaData
#     MetaData$DataSets[[DataSetNameShortcut.new]] <- list(
#       DataSetName = DataSetName.new,
#       CodeToCreateDataSet = CodeToCreateDataSet
#     )
#     
#     # Evaluate the code and assign the dataset to the global environment
#     assign(DataSetName.new, eval(parse(text = CodeToCreateDataSet)), envir = .GlobalEnv)
#     
#     # Store additional metadata
#     MetaData$DataSets[[DataSetNameShortcut.new]]$VarNames <- names(get(DataSetName.new))
#     MetaData$DataSets[[DataSetNameShortcut.new]]$dim <- dim(get(DataSetName.new))
#     
#     # Add any additional metadata provided
#     if (length(additional_metadata) > 0) {
#       MetaData$DataSets[[DataSetNameShortcut.new]] <- c(
#         MetaData$DataSets[[DataSetNameShortcut.new]],
#         additional_metadata
#       )
#     }
#     
#     message(paste("Added initial dataset:", DataSetName.new))
#     
#   } else {
#     # Ensure the old shortcut exists
#     if (!DataSetNameShortcut.old %in% names(MetaData$DataSets)) {
#       stop(paste("The old DataSetNameShortcut", DataSetNameShortcut.old, "does not exist in MetaData$DataSets."))
#     }
#     
#     # Define the new dataset shortcut
#     DataSetNameShortcut.new <- paste0(DataSetNameShortcut.old, DataSetName.new.suffix)
#     
#     # Check if the new shortcut already exists to prevent overwriting
#     if (DataSetNameShortcut.new %in% names(MetaData$DataSets)) {
#       stop(paste("The new DataSetNameShortcut", DataSetNameShortcut.new, "already exists in MetaData$DataSets. Choose a different suffix or shortcut."))
#     }
#     
#     # Retrieve the old dataset name
#     DataSetName.old <- MetaData$DataSets[[DataSetNameShortcut.old]]$DataSetName
#     
#     # Create the new dataset name
#     DataSetName.new <- paste0(DataSetName.old, DataSetName.new.suffix)
#     
#     # Add the new dataset to MetaData
#     MetaData$DataSets[[DataSetNameShortcut.new]] <- list(
#       DataSetName = DataSetName.new,
#       CodeToCreateDataSet = CodeToCreateDataSet
#     )
#     
#     # Evaluate the code and assign the new dataset to the global environment
#     assign(DataSetName.new, eval(parse(text = CodeToCreateDataSet)), envir = .GlobalEnv)
#     
#     # Store additional metadata
#     MetaData$DataSets[[DataSetNameShortcut.new]]$VarNames <- names(get(DataSetName.new))
...
#     }
#     
#     message(paste("Added new dataset:", DataSetName.new))
#   }
#   
#   # Optionally, you can save the MetaData to a file here if needed
#   # saveRDS(MetaData, "MetaData.rds")
#   
#   return(invisible(MetaData)) # Return MetaData invisibly to keep console output clean
# }
# 
# # Example Usage:
# 
# # Step 1: Create the initial dataset
# initial_CodeToCreateDataSet <- 'survival::lung |> 
#   as_tibble() |> 
#   mutate(
#     event = as.logical(status - 1), 
#     Group = factor(c("Male", "Female")[sex], levels = c("Female", "Male")), 
#     StudyPopulation = time >= 30
#   ) |> 
#   select(-status)'
# 
# add_dataset_to_metadata(
#   DataSetNameShortcut.old = NULL,
#   DataSetNameShortcut.new.suffix = NULL, # Not used for the initial dataset
#   CodeToCreateDataSet = initial_CodeToCreateDataSet
# )
# 
# # Step 2: Create a new dataset based on the initial one
# new_CodeToCreateDataSet <- "get(MetaData$DataSets[['DataSet0']]$DataSetName) |> na.omit()"
# 
# add_dataset_to_metadata(
#   DataSetNameShortcut.old = "DataSet0",
#   DataSetNameShortcut.new.suffix = ".na.omit",
#   CodeToCreateDataSet = new_CodeToCreateDataSet
# )
# 
# # View the structure of MetaData
...
# Enhanced function to create or update a dataset with metadata
add_dataset_to_metadata <- function(DataSetNameShortcut.old = NULL, 
                                    DataSetNameShortcut.new.suffix = NULL, 
                                    DataSetShortcut.new, # Explicitly provide the new shortcut
                                    CodeToCreateDataSet, 
                                    additional_metadata = list()) {
  ...
  if (is.null(DataSetNameShortcut.old)) {
    ...
    # Add the first dataset to MetaData
    MetaData$DataSets[[DataSetShortcut.new]] <- list(
      DataSetName = DataSetName.new,
      CodeToCreateDataSet = CodeToCreateDataSet
    )
    ...
    # Store additional metadata
    MetaData$DataSets[[DataSetShortcut.new]]$VarNames <- names(get(DataSetName.new))
    MetaData$DataSets[[DataSetShortcut.new]]$dim <- dim(get(DataSetName.new))
    
    # Add any additional metadata provided
    if (length(additional_metadata) > 0) {
      MetaData$DataSets[[DataSetShortcut.new]] <- c(
        MetaData$DataSets[[DataSetShortcut.new]],
        additional_metadata
      )
    }
    ...
    # Create the new dataset name
    DataSetName.new <- paste0(DataSetName.old, DataSetNameShortcut.new.suffix)
