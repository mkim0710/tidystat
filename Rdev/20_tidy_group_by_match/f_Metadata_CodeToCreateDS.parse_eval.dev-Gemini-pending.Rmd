...
create_dataset <- function(DSNShortcut.old, DSNShortcut.new.suffix, CodeToCreateDS) {
  # Create new shortcut name
  DSNShortcut.new <- paste0(DSNShortcut.old, DSNShortcut.new.suffix)
  print(paste("Creating dataset with shortcut:", DSNShortcut.new))

  # Create new dataset entry in MetaData
  print("Accessing MetaData$DSs...")
  print(str(MetaData$DSs)) # Debug: Print the structure of MetaData$DSs

  MetaData$DSs[[DSNShortcut.new]] <- list()  # Create the new list element here!
  print("Assigning DSN...")
  MetaData$DSs[[DSNShortcut.new]]$DSN <- 
    MetaData$DSs[[DSNShortcut.old]]$DSN |> paste0(DSNShortcut.new.suffix)
  MetaData$DSs[[DSNShortcut.new]]$CodeToCreateDS <- CodeToCreateDS

  # Evaluate the code to create the dataset
  print("Evaluating CodeToCreateDS...")
  print(CodeToCreateDS) # Debug: Print the code being evaluated
  assign(MetaData$DSs[[DSNShortcut.new]]$DSN, 
         eval(parse(text = CodeToCreateDS)), envir = .GlobalEnv) # Assign in global environment

  # Store metadata about the new dataset
  print("Storing metadata...")
  MetaData$DSs[[DSNShortcut.new]]$VarNames <- 
    names(get(MetaData$DSs[[DSNShortcut.new]]$DSN))
  MetaData$DSs[[DSNShortcut.new]]$dim <- 
    dim(get(MetaData$DSs[[DSNShortcut.new]]$DSN))
}
...
MetaData$DSs[[DSNShortcut]]$CodeToCreateDS <- 
...
assign(MetaData$DSs[[DSNShortcut]]$DSN, 
       eval(parse(text = MetaData$DSs[[DSNShortcut]]$CodeToCreateDS)))
...
CodeToCreateDS.new <- "get(MetaData$DSs$DS0$DSN) |> na.omit()"

create_dataset(DSNShortcut.old, DSNShortcut.new.suffix, CodeToCreateDS.new)
