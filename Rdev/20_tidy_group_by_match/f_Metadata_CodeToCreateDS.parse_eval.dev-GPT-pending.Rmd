...
#     assign("MetaData", list(CodeBook = list(), DSs = list()), envir = .GlobalEnv)
#     message("Initialized MetaData with empty CodeBook and DSs.")
#   }
#   
...
# add_dataset_to_metadata <- function(DSNShortcut.old = NULL, 
#                                    DSNShortcut.new.suffix, 
#                                    CodeToCreateDS, 
...
#   initialize_metadata()
#   
#   # If creating the first dataset, DSNShortcut.old is NULL
#   if (is.null(DSNShortcut.old)) {
#     # Extract a unique shortcut for the first dataset, e.g., "DS0"
#     DSNShortcut.new <- paste0("DS", length(MetaData$DSs))
#     
#     # Assign a unique dataset name if needed
#     # Assuming the first dataset has no suffix
#     DSN.new <- "ADS_time2event"
#     
#     # Add the first dataset to MetaData
#     MetaData$DSs[[DSNShortcut.new]] <- list(
#       DSN = DSN.new,
#       CodeToCreateDS = CodeToCreateDS
#     )
#     
#     # Evaluate the code and assign the dataset to the global environment
#     assign(DSN.new, eval(parse(text = CodeToCreateDS)), envir = .GlobalEnv)
#     
#     # Store additional metadata
#     MetaData$DSs[[DSNShortcut.new]]$VarNames <- names(get(DSN.new))
#     MetaData$DSs[[DSNShortcut.new]]$dim <- dim(get(DSN.new))
#     
#     # Add any additional metadata provided
#     if (length(additional_metadata) > 0) {
#       MetaData$DSs[[DSNShortcut.new]] <- c(
#         MetaData$DSs[[DSNShortcut.new]],
#         additional_metadata
#       )
#     }
#     
#     message(paste("Added initial dataset:", DSN.new))
#     
#   } else {
#     # Ensure the old shortcut exists
#     if (!DSNShortcut.old %in% names(MetaData$DSs)) {
#       stop(paste("The old DSNShortcut", DSNShortcut.old, "does not exist in MetaData$DSs."))
#     }
#     
#     # Define the new dataset shortcut
#     DSNShortcut.new <- paste0(DSNShortcut.old, DSN.new.suffix)
#     
#     # Check if the new shortcut already exists to prevent overwriting
#     if (DSNShortcut.new %in% names(MetaData$DSs)) {
#       stop(paste("The new DSNShortcut", DSNShortcut.new, "already exists in MetaData$DSs. Choose a different suffix or shortcut."))
#     }
#     
#     # Retrieve the old dataset name
#     DSN.old <- MetaData$DSs[[DSNShortcut.old]]$DSN
#     
#     # Create the new dataset name
#     DSN.new <- paste0(DSN.old, DSN.new.suffix)
#     
...
#     }
#     
#     message(paste("Added new dataset:", DSN.new))
#   }
#   
...
# initial_CodeToCreateDS <- 'survival::lung |> 
#   as_tibble() |> 
#   mutate(
...
# 
# add_dataset_to_metadata(
#   DSNShortcut.old = NULL,
#   DSNShortcut.new.suffix = NULL, # Not used for the initial dataset
#   CodeToCreateDS = initial_CodeToCreateDS
# )
# 
# # Step 2: Create a new dataset based on the initial one
# new_CodeToCreateDS <- "get(MetaData$DSs[['DS0']]$DSN) |> na.omit()"
# 
# add_dataset_to_metadata(
#   DSNShortcut.old = "DS0",
#   DSNShortcut.new.suffix = ".na.omit",
#   CodeToCreateDS = new_CodeToCreateDS
# )
# 
...
# Enhanced function to create or update a dataset with metadata
add_dataset_to_metadata <- function(DSNShortcut.old = NULL, 
                                    DSNShortcut.new.suffix = NULL, 
                                    DSShortcut.new, # Explicitly provide the new shortcut
                                    CodeToCreateDS, 
                                    additional_metadata = list()) {
  ...
  if (is.null(DSNShortcut.old)) {
    ...
    # Add the first dataset to MetaData
    MetaData$DSs[[DSShortcut.new]] <- list(
      DSN = DSN.new,
      CodeToCreateDS = CodeToCreateDS
    )
    ...
    # Store additional metadata
    MetaData$DSs[[DSShortcut.new]]$VarNames <- names(get(DSN.new))
    MetaData$DSs[[DSShortcut.new]]$dim <- dim(get(DSN.new))
    
    # Add any additional metadata provided
    if (length(additional_metadata) > 0) {
      MetaData$DSs[[DSShortcut.new]] <- c(
        MetaData$DSs[[DSShortcut.new]],
        additional_metadata
      )
    }
    ...
    # Create the new dataset name
    DSN.new <- paste0(DSN.old, DSNShortcut.new.suffix)
