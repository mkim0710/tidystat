...
# _______________  
# @@ START) function ----  
# https://gemini.google.com/app/6ce9c0bf85531677
...
create_dataset <- function(DataSetNameShortcut.old, DataSetNameShortcut.new.suffix, CodeToCreateDataSet) {
  # Create new shortcut name
  DataSetNameShortcut.new <- paste0(DataSetNameShortcut.old, DataSetNameShortcut.new.suffix)
  print(paste("Creating dataset with shortcut:", DataSetNameShortcut.new))

  # Create new dataset entry in MetaData
  print("Accessing MetaData$DataSets...")
  print(str(MetaData$DataSets)) # Debug: Print the structure of MetaData$DataSets

  MetaData$DataSets[[DataSetNameShortcut.new]] <- list()  # Create the new list element here!
  print("Assigning DataSetName...")
  MetaData$DataSets[[DataSetNameShortcut.new]]$DataSetName <- 
    MetaData$DataSets[[DataSetNameShortcut.old]]$DataSetName |> paste0(DataSetNameShortcut.new.suffix)
  MetaData$DataSets[[DataSetNameShortcut.new]]$CodeToCreateDataSet <- CodeToCreateDataSet

  # Evaluate the code to create the dataset
  print("Evaluating CodeToCreateDataSet...")
  print(CodeToCreateDataSet) # Debug: Print the code being evaluated
  assign(MetaData$DataSets[[DataSetNameShortcut.new]]$DataSetName, 
         eval(parse(text = CodeToCreateDataSet)), envir = .GlobalEnv) # Assign in global environment

  # Store metadata about the new dataset
  print("Storing metadata...")
  MetaData$DataSets[[DataSetNameShortcut.new]]$VarNames <- 
    names(get(MetaData$DataSets[[DataSetNameShortcut.new]]$DataSetName))
  MetaData$DataSets[[DataSetNameShortcut.new]]$dim <- 
    dim(get(MetaData$DataSets[[DataSetNameShortcut.new]]$DataSetName))
}
...
if(!exists("MetaData", envir=.GlobalEnv)) { 
  assign("MetaData", list(), envir=.GlobalEnv) 
}  
if(!"CodeBook" %in% names(MetaData)) { 
  MetaData$CodeBook <- list() 
}
if(file.exists("CodeBook.xlsx")) { 
  MetaData$CodeBook <- readxl::read_xlsx("CodeBook.xlsx") 
}
if(!"DataSets" %in% names(MetaData)) { 
  MetaData$DataSets <- list() 
}

# Create the initial dataset
DataSetName0 <- "analyticDF_time2event"
DataSetNameShortcut <- "DataSet0"

MetaData$DataSets[[DataSetNameShortcut]] <- list()
MetaData$DataSets[[DataSetNameShortcut]]$DataSetName <- DataSetName0
MetaData$DataSets[[DataSetNameShortcut]]$CodeToCreateDataSet <- 
  'survival::lung |> as_tibble() |> mutate(event = as.logical(status-1), Group = c("Male", "Female")[sex] |> as.factor(), StudyPopulation = time >= 30) |> dplyr::select(-status)'

assign(MetaData$DataSets[[DataSetNameShortcut]]$DataSetName, 
       eval(parse(text = MetaData$DataSets[[DataSetNameShortcut]]$CodeToCreateDataSet)))
...
# Now use the function to create a new dataset
DataSetNameShortcut.old <- "DataSet0"
DataSetNameShortcut.new.suffix <- ".na.omit"
CodeToCreateDataSet.new <- "get(MetaData$DataSets$DataSet0$DataSetName) |> na.omit()"

create_dataset(DataSetNameShortcut.old, DataSetNameShortcut.new.suffix, CodeToCreateDataSet.new)
