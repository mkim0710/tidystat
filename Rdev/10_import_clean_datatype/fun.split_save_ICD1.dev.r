# https://github.com/mkim0710/tidystat/blob/master/Rdev/fun.split_save_ICD1.dev.r
...
fun.split_save_ICD1 <- function(dataset_name = "nsc2_m20.sas7bdat", 
  ...
  map_chr(LETTERS, function(i) {
    ...
    .t0_write <- Sys.time()
    if (is.null(write_rds_compress)) write_rds(tmp.df, here::here(.path4write, FileName))
    else write_rds(tmp.df, here::here(.path4write, FileName), compress = write_rds_compress)
...
fun.split_save_ICD1_v2 <- function(dataset_name = "nsc2_m20.sas7bdat", 
  ...
  result <- map_chr(LETTERS, function(i) {
    ...
    .t0_write <- Sys.time()
    write_rds(filtered_data, here::here(.path4write, FileName), compress = ifelse(is.null(write_rds_compress), "xz", write_rds_compress))
