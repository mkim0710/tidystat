...
eval(parse(text = filename)) %>% str #----
path4write = getwd()
write_rds( eval(parse(text = filename)), file.path(path4write, paste0(filename, ".rds", "")), compress = "xz", compression = 9 )

assign(objectname, eval(parse(text = filename)) %>% function.dta.rm_attr   )
