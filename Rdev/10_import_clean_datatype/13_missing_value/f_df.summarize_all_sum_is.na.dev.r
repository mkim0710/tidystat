...
## @ ADS.TargetTrial2v40.2.206vs373.coxphTimeOutcome_Exposure_Covariates ====  
.packagename = "survival"; if (!paste0("package:",.packagename) %in% search()) {library(.packagename, character.only = TRUE)}
# data = ADS.TargetTrial2v40.2.206vs373 %>% mutate(Exposure = ifelse(Intervention, "1 InsulinMetformin", "0 InsulinOnly") %>% as.factor) %>% mutate_if(is.logical, as.numeric) %>% 
data = ADS.TargetTrial2v40.2.206vs373 %>% mutate(Metformin = Intervention) %>% mutate_if(is.logical, as.numeric) %>% 
...
ADS.TargetTrial2v40.2.206vs373.coxphTimeOutcome_Exposure_Covariates %>% {cbind( `exp(coef(.))` = exp(coef(.)), exp(confint.default(.)), `Pr(>|z|)` = summary(.)$coefficients[,"Pr(>|z|)"] )} %>% round(2) |> as.data.frame() |> rownames_to_column() |> as_tibble() |> print(n=99) #----
ADS.TargetTrial2v40.2.206vs373.coxphTimeOutcome_Exposure_Covariates %>% cox.zph #----
ADS.TargetTrial2v40.2.206vs373.coxphTimeOutcome_Exposure_Covariates %>% cox.zph %>% plot(var = c("Metformin"))
ADS.TargetTrial2v40.2.206vs373.coxphTimeOutcome_Exposure_Covariates %>% cox.zph %>% {survminer::ggcoxzph(., var = c("Metformin"), font.main = 12, caption = "")}
