...
# @@ Rdev/10_import_clean_datatype/11_metadata/f_chrValueLabelPairs2vector.dev.Rmd  
...
# Example usage
named_vector = c(ValueLabelA = "ValueA", ValueLabelB = "ValueB")
named_vector
dput(named_vector)
...
value_options_custom <- "ValueA:ValueLabelA | ValueB:ValueLabelB"
...
value_options1 <- "ValueA = ValueLabelA, ValueB=ValueLabelB"
value_options2 <- "ValueA - ValueLabelA;  ValueB-ValueLabelB"
...
## \% chrValueLabelPairs |> str_split(ValueLabelPair_separator) ----  
## \% vec.ValueLabelPair |> str_split(Value_Label_separator) ----  
```{r, echo=FALSE, results="markup", collapse=TRUE, paged.print=FALSE, comment="", R.options=list(width=120)}
options(width=120)
ValueLabelPair_separator = ","
Value_Label_separator = "="
chrValueLabelPairs <- "ValueA = ValueLabelA, ValueB = ValueLabelB"

chrValueLabelPairs %>% str_split(paste0("\\s*", ValueLabelPair_separator, "\\s*")) %>% str
cat("    ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~    \n")
chrValueLabelPairs %>% str_split(paste0("\\s*", ValueLabelPair_separator, "\\s*")) %>% unlist() %>% str  # ***

# Step 1: Split the string into individual pairs using ValueLabelPair_separator (handling whitespaces)
vec.ValueLabelPair <- chrValueLabelPairs %>% str_split(paste0("\\s*", ValueLabelPair_separator, "\\s*")) %>% unlist()

cat("    ________________________________________________________________________    \n")
cat("    ::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::    \n")
vec.ValueLabelPair %>% map(~ .x %>% str_split(paste0("\\s*", Value_Label_separator, "\\s*"))) %>% str
cat("    ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~    \n")
vec.ValueLabelPair %>% map(~ .x %>% str_split(paste0("\\s*", Value_Label_separator, "\\s*")) %>% unlist()) %>% str
cat("    ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++    \n")
vec.ValueLabelPair %>% str_split(paste0("\\s*", Value_Label_separator, "\\s*")) %>% str  # ***

# Step 2: Separate each pair into values and labels using Value_Label_separator (handling whitespaces)
list.vec.Value_Label <- vec.ValueLabelPair %>% str_split(paste0("\\s*", Value_Label_separator, "\\s*"))

cat("    ________________________________________________________________________    \n")
cat("    ::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::    \n")
list.vec.Value_Label %>% map_chr(1) %>% dput
list.vec.Value_Label %>% map_chr(2) %>% dput
cat("    ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~    \n")
list.vec.Value_Label %>% {setNames(map_chr(., 2), map_chr(., 1))} %>% dput
cat("    ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++    \n")
list.vec.Value_Label %>% map(~ set_names(c(.x[2]), .x[1])) %>% dput
list.vec.Value_Label %>% map(~ set_names(c(.x[2]), .x[1])) %>% unlist %>% dput  # ***
cat("    ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++    \n")
list.vec.Value_Label %>% map_chr(~ set_names(c(.x[2]), .x[1])) %>% dput

cat("    ________________________________________________________________________    \n")
cat("    ::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::    \n")
# Step 3: Create the named vector where Value is the name and ValueLabel is the value
# named_vector <- list.vec.Value_Label %>% {setNames(map_chr(., 2), map_chr(., 1))}
named_vector <- list.vec.Value_Label %>% map(~ set_names(c(.x[2]), .x[1])) %>% unlist
named_vector %>% dput
```
...
## \% |> f_chrValueLabelPairs2vector() ----  
```{r, echo=FALSE, results="markup", collapse=TRUE, paged.print=FALSE, comment="", R.options=list(width=120)}
options(width=120)


f_chrValueLabelPairs2vector = function(chrValueLabelPairs, ValueLabelPair_separator = ",", Value_Label_separator = "=", vecNamed.swap = FALSE) {
    vec.ValueLabelPair = chrValueLabelPairs %>% str_split(paste0("\\s*", ValueLabelPair_separator, "\\s*")) %>% unlist() 
    list.vec.Value_Label = vec.ValueLabelPair %>% str_split(paste0("\\s*", Value_Label_separator, "\\s*"))
    vecNamed = list.vec.Value_Label %>% map(~ set_names(c(.x[2]), .x[1])) %>% unlist()
