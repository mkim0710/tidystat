...
```{r, echo=TRUE, results="markup", collapse=TRUE, paged.print=FALSE, comment="#", R.options=list(width=120)}
options(width=120)
DataSet = df_NewDMv3.CensorEND.n88.select293.NA.rmAllNA.recode %>% 
    select(matches("HEIGHT|WEIGHT")) %>% 
    rownames_to_column() 
DataSet |> str(max.level = 2, give.attr = TRUE)
```
...
DataSet %>% select(matches("HEIGHT|WEIGHT|BMI")) |> str(max.level = 2, give.attr = TRUE)
```
...
.RegEx2match = "BMI"
i = 2
DataSet %>% select(matches(.RegEx2match)) %>% select(matches(paste0("A0",i))) |> str(max.level = 2, give.attr = TRUE)
DataSet %>% select(matches(.RegEx2match)) %>% select(matches(paste0("A0",i))) %>% select_if(function(x) is.numeric(x) | is.logical(x)) %>% names %>% str_replace(paste0("^A0",i,"_"),"") %>% dput
...
DataSet %>% select(matches("BMI")) %>% select(matches("A02")) |> str(max.level = 2, give.attr = TRUE)
```
...
DataSet %>% select(matches("BMI")) %>% select(matches("A03")) |> str(max.level = 2, give.attr = TRUE)
```
...
DataSet %>% select(matches("BMI")) %>% select(matches("A07")) |> str(max.level = 2, give.attr = TRUE)
```
...
DataSet %>% select(matches(paste0(.RegEx2match,"$"))) %>% select(matches("A07")) |> str(max.level = 2, give.attr = TRUE)
DataSet %>% select(matches(paste0(.RegEx2match,"$"))) %>% select(matches("A07")) %>% select_if(function(x) is.numeric(x) | is.logical(x)) %>% names %>% str_replace("^A07_","") %>% dput
...
DataSet %>% select(matches(paste0(.RegEx2match, "\\.5bins$"))) %>% summary
```
