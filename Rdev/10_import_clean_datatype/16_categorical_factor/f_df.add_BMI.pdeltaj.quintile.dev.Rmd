...
```{r, echo=TRUE, results="markup", collapse=TRUE, paged.print=FALSE, comment="#", R.options=list(width=120)}
options(width=120)
DS = df_NewDMv3.CensorEND.n88.select293.NR.recode %>% 
    select(matches("HEIGHT|WEIGHT")) %>% 
    rownames_to_column() 
DS |> str(max.level = 2, give.attr = TRUE)
```
...
DS %>% select(matches("HEIGHT|WEIGHT|BMI")) |> str(max.level = 2, give.attr = TRUE)
```
...
.RegEx2match = "BMI"
i = 2
DS %>% select(matches(.RegEx2match)) %>% select(matches(paste0("A0",i))) |> str(max.level = 2, give.attr = TRUE)
DS %>% select(matches(.RegEx2match)) %>% select(matches(paste0("A0",i))) %>% select_if(function(x) is.numeric(x) | is.logical(x)) %>% names %>% str_replace(paste0("^A0",i,"_"),"") %>% dput
...
DS %>% select(matches("BMI")) %>% select(matches("A02")) |> str(max.level = 2, give.attr = TRUE)
```
...
DS %>% select(matches("BMI")) %>% select(matches("A03")) |> str(max.level = 2, give.attr = TRUE)
```
...
DS %>% select(matches("BMI")) %>% select(matches("A07")) |> str(max.level = 2, give.attr = TRUE)
```
...
DS %>% select(matches(paste0(.RegEx2match,"$"))) %>% select(matches("A07")) |> str(max.level = 2, give.attr = TRUE)
DS %>% select(matches(paste0(.RegEx2match,"$"))) %>% select(matches("A07")) %>% select_if(function(x) is.numeric(x) | is.logical(x)) %>% names %>% str_replace("^A07_","") %>% dput
...
DS %>% select(matches(paste0(.RegEx2match, "\\.5bins$"))) %>% summary
```
