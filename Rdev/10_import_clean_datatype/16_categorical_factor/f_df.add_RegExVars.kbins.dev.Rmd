...
#     }
# }
# DS.DNR %>% select(matches("BMI")) %>% select(matches("A03")) |> str(max.level = 2, give.attr = TRUE)
```
...
#     .names = "{.col}.5bins"
#   ))
# DS.DNR %>% select(matches(paste0(.RegEx2match, "\\.5bins$"))) %>% summary
```
...
.tmp$object = function(DS.DNR, regex_pattern = ".", breaks = c(-Inf, -0.1, -0.05, 0.05, 0.1, Inf), suffix = paste0(".cut",length(breaks)-1)) {
