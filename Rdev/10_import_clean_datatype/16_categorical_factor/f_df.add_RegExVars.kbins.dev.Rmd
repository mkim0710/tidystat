...
DataSetName = "df_NewDMv3.CensorEND.n88.select293.NR.recode.rds" |> str_replace("\\.rds$", "")
assign( DataSetName, read_rds(paste0(.path4read,"/",DataSetName,".rds")) )
...
```{r, echo=TRUE, results="markup", collapse=TRUE, paged.print=FALSE, comment="#", R.options=list(width=120)}
options(width=120)
DataSet.DNR = df_NewDMv3.CensorEND.n88.select293.NR.recode %>% 
...
##________________________________________________________________________________  
##%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%  
## @ "f_df.add_BMI.5bins" |> paste0(".source.r") |> source() ----  
...
DataSet.DNR %>% select(matches("HEIGHT|WEIGHT|BMI")) |> str(max.level = 2, give.attr = TRUE)
```
...
# .RegEx2match = "BMI"
# i = 2
# DataSet.DNR %>% select(matches(.RegEx2match)) %>% select(matches(paste0("A0",i))) |> str(max.level = 2, give.attr = TRUE)
# DataSet.DNR %>% select(matches(.RegEx2match)) %>% select(matches(paste0("A0",i))) %>% select_if(function(x) is.numeric(x) | is.logical(x)) %>% names %>% str_replace(paste0("^A0",i,"_"),"") %>% dput
# 
# for (i in 2:7) {
...
#     }
# }
# DataSet.DNR %>% select(matches("BMI")) %>% select(matches("A02")) |> str(max.level = 2, give.attr = TRUE)
```
...
#     }
# }
# DataSet.DNR %>% select(matches("BMI")) %>% select(matches("A03")) |> str(max.level = 2, give.attr = TRUE)
```
...
DataSet.DNR %>% select(matches("BMI")) %>% select(matches("A07")) |> str(max.level = 2, give.attr = TRUE)
```
...
# 
# DataSet.DNR %>% select(matches(paste0(.RegEx2match,"$"))) %>% select(matches("A07")) |> str(max.level = 2, give.attr = TRUE)
# DataSet.DNR %>% select(matches(paste0(.RegEx2match,"$"))) %>% select(matches("A07")) %>% select_if(function(x) is.numeric(x) | is.logical(x)) %>% names %>% str_replace("^A07_","") %>% dput
# 
...
#     .names = "{.col}.5bins"
#   ))
# DataSet.DNR %>% select(matches(paste0(.RegEx2match, "\\.5bins$"))) %>% summary
```
...
.tmp$object = function(DataSet.DNR, regex_pattern = ".", breaks = c(-Inf, -0.1, -0.05, 0.05, 0.1, Inf), suffix = paste0(".cut",length(breaks)-1)) {
...
DataSet.DNR <- DataSet.DNR |> env1$f$f_df.add_RegExVars.kbins(regex_pattern = paste0(.regex_pattern_part,"$"), breaks = .breaks4cut, suffix = .suffix)
DataSet.DNR %>% select(matches(paste0(.regex_pattern_part,.suffix,"$"))) %>% summary
