...
.RegEx2match = "BMI"
i = 2
DataSet %>% select(matches(.RegEx2match)) %>% select(matches(paste0("A0",i))) %>% str
...
DataSet %>% select(matches(paste0(.RegEx2match,"$"))) %>% select(matches("A07")) %>% str
...
# Define the function f_df.add_pdj.cut5
f_df.add_pdj.cut5 <- function(DataSet, regex_pattern = "\\.pd[1-6]$", suffix = ".cut5") {
  
  # Ensure that the DataSet parameter is a data frame
  ...
  DataSet <- DataSet %>%
    mutate(across(
      ...
      .fns = ~ cut(., 
                   breaks = c(-Inf, -0.1, -0.05, 0.05, 0.1, Inf), # Define breakpoints
                   include.lowest = TRUE, 
                   right = FALSE), 
...
# Assuming DataSet is already loaded and processed up to the point where pdj columns exist

# Call the function to add pdj.cut5 variables
DataSet <- f_df.add_pdj.cut5(DataSet)
