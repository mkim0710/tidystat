...
## \$ DataSet =  
### |> env1$f$f_df.add_BMI_calculated()  
### |> env1$f$f_df.add_BMI.cut4()  
```{r, echo=TRUE, results="markup", collapse=TRUE, paged.print=FALSE, comment="", R.options=list(width=120)}
options(width=120)
...
DataSet = DataSet %>% (env1$f$f_df.add_BMI_calculated)()
DataSet = DataSet %>% (env1$f$f_df.add_BMI.cut4)()
DataSet %>% select(matches("BMI"))
DataSet %>% select(matches("BMI")) %>% summary()
```
...
DataSet %>% select(matches("BMI")) %>% str
DataSet %>% select(matches("BMI")) %>% select_if(function(x) is.numeric(x) | is.logical(x)) %>% names %>% dput
...
for (i in 2:7) {
    for (.VarName.suffix in c("BMI", "BMI_lt185", "BMI_ge185lt230", "BMI_ge230lt250", "BMI_ge185lt250", "BMI_ge250lt300", "BMI_ge230", "BMI_ge250", "BMI_ge300", "BMI_ge350", "BMI_ge400")) {
        DataSet = 
            DataSet %>% 
            env1$f$f_df.add_VarName1.delta(paste0("A0",i-1,"_",.VarName.suffix), paste0("A0",i,"_",.VarName.suffix))
    }
}
