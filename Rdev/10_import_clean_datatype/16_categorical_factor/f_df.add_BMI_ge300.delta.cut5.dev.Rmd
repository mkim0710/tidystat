...
DataSet %>% select(matches(.RegEx2match)) %>% select(matches("A07")) %>% str
DataSet %>% select(matches(.RegEx2match)) %>% select(matches("A07")) %>% select_if(function(x) is.numeric(x) | is.logical(x)) %>% names %>% str_replace("^A07_","") %>% dput


DataSet %>% transmute_at(
    vars(matches(.RegEx2match)), 
    ~cut(., breaks = c(-Inf, -0.1, -0.05, 0.05, 0.1, Inf), include.lowest = T, right = F)
) %>% summary
```
