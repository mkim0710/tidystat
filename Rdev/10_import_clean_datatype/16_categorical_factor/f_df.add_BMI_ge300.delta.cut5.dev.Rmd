...
#     .names = "{.col}.quintile"
#   ))
# DataSet %>% select(matches(paste0(.RegEx2match, "\\.quintile$"))) %>% summary
```
...
f_df.add_quintile_factors <- function(data, regex_pattern) {
  data %>%
    mutate(across(
...
.RegEx2match = "\\.pd[1-6]"
DataSet <- f_df.add_quintile_factors(DataSet, paste0(.RegEx2match,"$"))
DataSet %>% select(matches(paste0(.RegEx2match, "\\.quintile$"))) %>% summary
