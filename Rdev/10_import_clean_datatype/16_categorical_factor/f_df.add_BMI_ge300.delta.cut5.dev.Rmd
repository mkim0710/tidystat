...
DataSet %>% select(matches("HEIGHT|WEIGHT|BMI")) %>% str
```
...
DataSet %>% select(matches("BMI")) %>% select(matches("A02")) %>% str
DataSet %>% select(matches("BMI")) %>% select(matches("A02")) %>% select_if(function(x) is.numeric(x) | is.logical(x)) %>% names %>% str_replace("^A02_","") %>% dput
...
for (i in 2:7) {
    ...
    for (.VarName.suffix in c("BMI_lt185", "BMI_ge185lt230", "BMI_ge230lt250", "BMI_ge185lt250", "BMI_ge250lt300", "BMI_ge230", "BMI_ge250", "BMI_ge300", "BMI_ge350", "BMI_ge400")) {
        ...
        DataSet = 
            DataSet %>% 
            env1$f$f_df.add_VarName1.delta(paste0("A0",i-1,"_",.VarName.suffix), paste0("A0",i,"_",.VarName.suffix), prefix = "", suffix = ".delta")
    }
}
DataSet %>% select(matches("BMI")) %>% select(matches("A02")) %>% str
```
...
## \$ f_df.add_VarName1.delta =  ----  
## -> included in "f_df.add_VarName1.delta.source.r"
DataSet %>% select(matches("BMI")) %>% select(matches("A03")) %>% str
DataSet %>% select(matches("BMI")) %>% select(matches("A03")) %>% select_if(function(x) is.numeric(x) | is.logical(x)) %>% names %>% str_replace("^A03_","") %>% dput
...
for (i in 3:7) {
    for (.VarName.suffix in c("BMI")) {
        ...
        DataSet = 
            DataSet %>% 
            env1$f$f_df.add_VarName1.delta(paste0("A0",i-2,"_",.VarName.suffix), paste0("A0",i,"_",.VarName.suffix), prefix = "", suffix = ".delta2")
    }
}
DataSet %>% select(matches("BMI")) %>% select(matches("A03")) %>% str
```
...
## \$ f_df.add_VarName1.delta =  ----  
## -> included in "f_df.add_VarName1.delta.source.r"
DataSet %>% select(matches("BMI")) %>% select(matches("A07")) %>% str
DataSet %>% select(matches("BMI")) %>% select(matches("A07")) %>% select_if(function(x) is.numeric(x) | is.logical(x)) %>% names %>% str_replace("^A07_","") %>% dput
...
for (j in 1:(7-1)) {
    for (i in (1+j):7) {
        for (.VarName.suffix in c("BMI")) {
            ...
            DataSet = 
                DataSet %>% 
                env1$f$f_df.add_VarName1.delta(paste0("A0",i-j,"_",.VarName.suffix), paste0("A0",i,"_",.VarName.suffix), prefix = "", suffix = paste0(".delta",j))
        }
    }
}
DataSet %>% select(matches("BMI")) %>% select(matches("A07")) %>% str
...
for (j in 1:(7-1)) {
    for (i in (1+j):7) {
        for (.VarName.suffix in c("BMI")) {
            ...
            #     DataSet %>%
            #     env1$f$f_df.add_VarName1.delta(paste0("A0",i-j,"_",.VarName.suffix), paste0("A0",i,"_",.VarName.suffix), prefix = "", suffix = paste0(".delta",j))
            DataSet[c(paste0("A0",i-j,"_",.VarName.suffix), paste0("A0",i,"_",.VarName.suffix), paste0("A0",i,"_",.VarName.suffix,".delta",j) )] |> head() |> as.data.frame() |> print()
        }
    }
}
```
