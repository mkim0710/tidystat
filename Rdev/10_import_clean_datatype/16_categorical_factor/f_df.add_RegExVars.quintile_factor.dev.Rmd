...
# DataSet.Date.NA.rmAllNA <- DataSet.Date.NA.rmAllNA |> env1$f$f_df.add_RegEx.cut(regex_pattern = "\\.pd[1-6]$", breaks = c(-Inf, -0.1, -0.05, 0.05, 0.1, Inf))
...
.RegEx2match = "\\.pd[1-6]"
DataSet.Date.NA.rmAllNA <- env1$f$f_df.add_quintile_factors(DataSet.Date.NA.rmAllNA, paste0(.RegEx2match,"$"))
DataSet.Date.NA.rmAllNA %>% select(matches(paste0(.RegEx2match, "\\.quintile$"))) %>% summary
