...
.sourcename = "f_df.add_BMI.5category" |> paste0(".source.r")
...
##________________________________________________________________________________  
##%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%  
## @ "f_df.add_BMI.5category" |> paste0(".source.r") |> source() ----  
...
for(i in 1:7) {
    DataSet.Date.NA.rmAllNA =
        ...
        env1$f$f_df.add_BMI.5category(varname_BMI = paste0("A0",i,"_","BMI"))
}
...
for (j in 1:(7-1)) {
    for (i in (1+j):7) {
        for (.VarName.suffix in c("BMI", "WEIGHT")) {
            ...
            DataSet.Date.NA.rmAllNA = 
                DataSet.Date.NA.rmAllNA |> 
                env1$f$f_df.add_VarNameT.pdeltaT0(paste0("A0",i-j,"_",.VarName.suffix), paste0("A0",i,"_",.VarName.suffix), prefix = "", suffix = paste0(".pd",j))
        }
    }
}
DataSet.Date.NA.rmAllNA %>% select(matches("BMI")) %>% select(matches("A07")) |> str(max.level = 2, give.attr = TRUE)
...
# .regex_pattern_part = "\\.pd[1-6]"
# .breaks4cut = c(-Inf, -0.1, -0.05, 0.05, 0.1, Inf)
# .suffix = paste0(".cut",length(.breaks4cut)-1)
# DataSet.Date.NA.rmAllNA <- DataSet.Date.NA.rmAllNA |> env1$f$f_df.add_RegExVars.kcategory(regex_pattern = paste0(.regex_pattern_part,"$"), breaks = .breaks4cut, suffix = .suffix)
