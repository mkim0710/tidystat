...
DataSet.Date.NA.rmAllNA %>% select(matches("HEIGHT|WEIGHT|BMI")) |> str(max.level = 2, give.attr = TRUE)
```
...
# .RegEx2match = "BMI"
# i = 2
# DataSet.Date.NA.rmAllNA %>% select(matches(.RegEx2match)) %>% select(matches(paste0("A0",i))) |> str(max.level = 2, give.attr = TRUE)
...
#     }
# }
# DataSet.Date.NA.rmAllNA %>% select(matches("BMI")) %>% select(matches("A02")) |> str(max.level = 2, give.attr = TRUE)
```
...
DataSet.Date.NA.rmAllNA %>% select(matches("BMI")) %>% select(matches("A07")) |> str(max.level = 2, give.attr = TRUE)
```
...
# DataSet.Date.NA.rmAllNA <- DataSet.Date.NA.rmAllNA |> env1$f$f_df.add_RegExVars.kbins(regex_pattern = paste0(.regex_pattern_part,"$"), breaks = .breaks4cut, suffix = .suffix)
# DataSet.Date.NA.rmAllNA %>% select(matches(paste0(.regex_pattern_part,.suffix,"$"))) %>% summary
...
# 
# DataSet.Date.NA.rmAllNA %>% select(matches(paste0(.RegEx2match,"$"))) %>% select(matches("A07")) |> str(max.level = 2, give.attr = TRUE)
...
#     .names = "{.col}.quintile"
#   ))
# DataSet.Date.NA.rmAllNA %>% select(matches(paste0(.RegEx2match, "\\.quintile$"))) %>% summary
```
...
.RegEx2match = "\\.pdelta[1-6]"
DataSet.Date.NA.rmAllNA <- env1$f$f_df.add_RegExVars.quintile_factor(DataSet.Date.NA.rmAllNA, paste0(.RegEx2match,"$"))
DataSet.Date.NA.rmAllNA %>% select(matches(paste0(.RegEx2match, "\\.quintile$"))) %>% summary
