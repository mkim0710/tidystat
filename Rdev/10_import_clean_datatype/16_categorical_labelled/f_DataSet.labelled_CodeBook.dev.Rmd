...
# _______________  
# @@ START) function ----  
## \% |> f_CodeBook_vec.labelled()  
```{r, echo=TRUE, results="markup", collapse=TRUE, paged.print=FALSE, comment="", R.options=list(width=120)}
options(width=120)

f_CodeBook_df_VarName.labelled = function(DataSet, VarName, tblVarName_VarDescription_ValueOptions, NameValuePair_separator = ",", Name_Value_separator = "=", vecNamed.swap = FALSE) {
    library(labelled)
    var_label(DataSet[[VarName]]) = tblVarName_VarDescription_ValueOptions %>% 
        dplyr::filter(WAVE_VARNAME == VarName) %>% 
        pull(VarDescription) %>% as.character
    val_labels(DataSet[[VarName]]) = tblVarName_VarDescription_ValueOptions %>% 
        filter(WAVE_VARNAME == VarName) %>% 
        pull(ValueOptions) %>% as.character |> 
        env1$f$f_chrNameValuePairs2vector(vecNamed.swap = TRUE)
}
...
DataSet = get(DataSetName)
f_CodeBook_df_VarName.labelled(DataSet, "A00_SEX", tblVarName.filter)
...
f_CodeBook_DataSet.labelled = function(DataSet) {
