...
### \% "ValueA=ValueLabelA, ValueB=ValueLabelB" |> ----  
#### https://chatgpt.com/c/670fd3f1-de80-800e-bfdc-f379288fc625
```{r, echo=FALSE, results="markup", collapse=TRUE, paged.print=FALSE, comment="", R.options=list(width=120)}
library(tidyverse)

# Sample ValueOptions string
value_options <- "ValueA=ValueLabelA, ValueB=ValueLabelB"

# Step 1: Split the string into individual pairs by comma
value_label_pairs <- str_split(value_options, ", ") %>% unlist()

# Step 2: Separate each pair into values and labels
value_label_list <- map(value_label_pairs, ~ str_split(.x, "=") %>% unlist())

# Step 3: Create the named vector where ValueLabel is the name and Value is the value
named_vector <- setNames(map_chr(value_label_list, 1), map_chr(value_label_list, 2))
...
##### ECHO \% |> string_to_named_vector() ----  
```{r, echo=TRUE, results="markup", collapse=TRUE, paged.print=FALSE, comment="", R.options=list(width=120)}
options(width=120)
library(tidyverse)

# Function to convert ValueOptions string to a named vector
string_to_named_vector <- function(value_options) {
  ...
  # Step 3: Create the named vector where Value is the name and ValueLabel is the value
...
# Example usage
named_vector = c(ValueA = "ValueLabelA", ValueB = "ValueLabelB")
named_vector
dput(named_vector)
...
# Expected output:
# ValueLabelA ValueLabelB 
#    "ValueA"    "ValueB"
```


##### ECHO \% |> string_to_named_vector() ----  
##### ECHO \% |> swap_names_values() ----  
```{r, echo=TRUE, results="markup", collapse=TRUE, paged.print=FALSE, comment="", R.options=list(width=120)}
options(width=120)
library(tidyverse)
...
### \% "ValueA=ValueLabelA, ValueB=ValueLabelB" |> ----  
```{r, echo=FALSE, results="markup", collapse=TRUE, paged.print=FALSE, comment="", R.options=list(width=120)}
i.ValueOptions = "ValueA=ValueLabelA, ValueB=ValueLabelB"
...
# VarName.suffix.selected |> deparse() |> cat("  \n", sep="")
# cat("    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%    \n")
.tblWAVE_VARNAME.join = MetaData$tblVarName$tblWAVE_VARNAME |> dplyr::filter(VarName.suffix %in% VarName.suffix.selected) %>% select(Num, VarName.suffix, VarDescription, Wave, WAVE_VARNAME, VarType, Select) %>% 
    left_join(MetaData$tblVarName$tblVarName.suffix %>% select(VarName.suffix, ValueOptions), by="VarName.suffix")
.tblWAVE_VARNAME.join
cat("    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%    \n")
.tblWAVE_VARNAME.join$VarDescription |> as.character() |> deparse() |> cat("  \n", sep="")
cat("    ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~    \n")
library(labelled)
for (i.WAVE_VARNAME in .tblWAVE_VARNAME.join$WAVE_VARNAME) {
    var_label(DataSet[[i.WAVE_VARNAME]]) = .tblWAVE_VARNAME.join %>% dplyr::filter(WAVE_VARNAME == i.WAVE_VARNAME) %>% pull(VarDescription) %>% as.character
}
cat("    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%    \n")
.tblWAVE_VARNAME.join$ValueOptions |> as.character() |> deparse() |> cat("  \n", sep="")
cat("    ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~    \n")
library(labelled)
for (i.WAVE_VARNAME in .tblWAVE_VARNAME.join$WAVE_VARNAME) {
    .CodeText = .tblWAVE_VARNAME.join %>% dplyr::filter(WAVE_VARNAME == i.WAVE_VARNAME) %>% pull(ValueOptions) %>% {paste0("c(",.,")")}
    print(.CodeText)
    # eval(parse(text = .CodeText))
}

cat("    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%    \n")
DataSet %>% {attr(., "n_distinct") = NULL; attr(., "MetaData") = NULL; .} |> select(matches(VarName.suffix.selected|>paste(collapse="|"))) |> str()
```
