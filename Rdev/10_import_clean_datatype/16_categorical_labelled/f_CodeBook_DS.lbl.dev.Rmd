...
# paste0(.path4read,"/",DSN,".rds") |> env1$f$f_DS_path_FileNameExt.read.checkEntity()
...
for (i.suffix in c(".lbl", ".lbl.fct")) {
    ...
    get(paste0("DS",i.suffix)) %>% env1$f$f_objectname.size.write_rds.git_lfs_track_add_f(.object = ., .objectname = .objectname, .path_file = .path_file, .path4write = NULL, .FileNameExt4write = NULL, createBACKUP = FALSE, .BACKUP_to_path="-BACKUP", EXECUTE = TRUE, path.size_files = TRUE, git_lfs_track = FALSE, git_add_f = FALSE, CompressionMethod = "xz", VERBOSE = FALSE)
    cat("    ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++    \n")
}
