...
### :: DataSet.DNR = get(DataSetName) |> select(matches(VarName.suffix.regex))
...
DataSet.DNR = get(DataSetName) %>% select(matches(VarName.suffix.regex), matches("A0[0-2]")) %>%
    # select(matches("A00|A01|A04|A07"))
    # select(matches("A00|A01|A07"))
    select(matches("A0[0-3]"))
DataSet.DNR = DataSet.DNR[sort(names(DataSet.DNR))]
DataSet.DNR = DataSet.DNR %>% mutate_if(function(vec) {is.numeric(vec) && n_distinct(vec) <= 10}, as.factor)
DataSet.DNR |> str(max.level = 2, give.attr = TRUE)
```
