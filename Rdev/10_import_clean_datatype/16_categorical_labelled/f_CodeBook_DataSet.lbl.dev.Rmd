...
#### \% |> env1\$f\$f_df.Table1byExposure.xlsx(VarNames4Exposure =  c("A01_BMI.5category"))  
...
DataSet.Date.NA.rmAllNA |> str(max.level = 2, give.attr = TRUE)
```
...
# @@ Rdev/10_import_clean_datatype/16_categorical_labelled/env1$f$f_CodeBook_DataSet.lbl.dev.Rmd  
...
cat("    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%    \n")
DataSet %>% {attr(., "n_distinct") = NULL; attr(., "MetaData") = NULL; .} |> select(any_of(tblVarName.filter$WAVE_VARNAME)) |> str(max.level = 2, give.attr = TRUE)
DataSet %>% {attr(., "n_distinct") = NULL; attr(., "MetaData") = NULL; .} |> select(any_of(tblVarName.filter$WAVE_VARNAME)) |> as_tibble() |> print(n=5)
cat("    ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~    \n")
DataSet %>% {attr(., "n_distinct") = NULL; attr(., "MetaData") = NULL; .} |> select(any_of(tblVarName.filter$WAVE_VARNAME)) |> mutate_all(as_factor) |> str(max.level = 2, give.attr = TRUE)
...
# cat("    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%    \n")
env1$env.internal$f_CodeBook_DataSet_VarName.lbl(DataSet, "A00_SEX", tblVarName, vecNamed.swap = TRUE, output.select = TRUE) %>% as_tibble() %>% print(n=5)
env1$env.internal$f_CodeBook_DataSet_VarName.lbl(DataSet, "A00_SEX", tblVarName, vecNamed.swap = TRUE, output.select = TRUE) %>% str(maxlevel = 2)
cat("    ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++    \n")
env1$env.internal$f_CodeBook_DataSet_VarName.lbl(DataSet, paste0("A0", 1:2, "_AGE"), tblVarName, vecNamed.swap = TRUE, output.select = TRUE) %>% as_tibble() %>% print(n=5)
env1$env.internal$f_CodeBook_DataSet_VarName.lbl(DataSet, paste0("A0", 1:2, "_AGE"), tblVarName, vecNamed.swap = TRUE, output.select = TRUE) %>% str(maxlevel = 2)
```


## :: env1$f$f_CodeBook_DataSet.lbl()  
```{r, echo=TRUE, results="markup", collapse=TRUE, paged.print=FALSE, comment="#", R.options=list(width=120)}
options(width=120)
...
# DataSet = DataSet %>% env1$f$f_CodeBook_DataSet.lbl(tblVarName, vecNamed.swap = TRUE, VERBOSE = TRUE) 
DataSet.lbl = DataSet %>% env1$f$f_CodeBook_DataSet.lbl(tblVarName, vecNamed.swap = TRUE)
DataSet.lbl %>% select(matches("^A0[0-1]")) %>% {attributes(.)$MetaData = NULL; .} %>% as_tibble() %>% print(n=5)
DataSet.lbl %>% select(matches("^A0[0-1]")) %>% {attributes(.)$MetaData = NULL; .} %>% str(maxlevel = 2)
cat("    ________________________________________________________________________    \n")
DataSet.lbl.fct = DataSet.lbl %>% mutate_if(function(vec) is.character(vec) && labelled::is.labelled(vec), as_factor)
DataSet.lbl.fct %>% select(matches("^A0[0-1]")) %>% {attributes(.)$MetaData = NULL; .} %>% str(maxlevel = 2)
```
  
  
### DataSet.fct |> env1$f$f_CodeBook_DataSet.lbl()  
```{r, echo=TRUE, results="markup", collapse=TRUE, paged.print=FALSE, comment="#", R.options=list(width=120)}
options(width=120)
...
DataSet.fct |> str(max.level = 2, give.attr = TRUE)
DataSet.fct.lbl = DataSet.fct %>% env1$f$f_CodeBook_DataSet.lbl(tblVarName, vecNamed.swap = TRUE)
DataSet.fct.lbl %>% select(matches("^A0[0-1]")) %>% {attributes(.)$MetaData = NULL; .} %>% as_tibble() %>% print(n=5)
DataSet.fct.lbl %>% select(matches("^A0[0-1]")) %>% {attributes(.)$MetaData = NULL; .} %>% str(maxlevel = 2)
cat("    ________________________________________________________________________    \n")
DataSet.fct.lbl.fct = DataSet.fct.lbl %>% mutate_if(function(vec) is.character(vec) && labelled::is.labelled(vec), as_factor) 
DataSet.fct.lbl.fct %>% select(matches("^A0[0-1]")) %>% {attributes(.)$MetaData = NULL; .} %>% str(maxlevel = 2)
```
...
if(exists("MetaData")) {
    ...
    if(!identical(.MetaData0, MetaData)) {
        # cat("    ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~    \n")
        .path_file = env1$path$LastSourceEditorContext.path_filename.MetaData.rds %>% {paste0(env1$path$path1,"/",.)}
        # cat('env1$env.internal$f_filename.ext.createBackup(backup_from_path_filename.ext = "', .path_file, '", .backup_to_path="-backup", timeFormat="%y%m%d_%H", overwrite=TRUE)', "  ;  ", sep="")
        # cat('MetaData |> write_rds("', .path_file, '", compress = "xz") |> system.time()', "  \n", sep="")
        # cat("'git lfs track ",shQuote(.path_file),"' |> system(intern=TRUE)  \n", sep=""); cat("'git add -f ",shQuote(.path_file),"' |> system(intern=TRUE)  \n", sep="")
        ...
        MetaData |> write_rds(.path_file, compress = "xz") |> system.time()
...
##%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%  
DataSetName
DataSetName %>% paste0(".lbl")
DataSetName %>% paste0(".lbl.fct")
cat("    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%    \n")
DataSet.lbl.fct %>% select(1) |> str(max.level = 2, give.attr = TRUE)
cat("    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%    \n")
for (i.suffix in c(".lbl", ".lbl.fct")) {
    ...
    get(paste0("DataSet",i.suffix)) %>% env1$f$f_objectname.size.write_rds.git_lfs_track_add_f(.object = ., .objectname = .objectname, .path_file = .path_file, .path4write = NULL, .filename.ext4write = NULL, createBackup = FALSE, .backup_to_path="-backup", Execute = TRUE, path.size_files = TRUE, git_lfs_track = FALSE, git_add_f = FALSE, CompressionMethod = "xz", VERBOSE = FALSE)
    cat("    ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++    \n")
}

```
  
    df_NewDMv3.CensorEND.n88.select293.NA.rmAllNA.recode.lbl |> write_rds('/home/rstudio/Rproject_KoGES_AA10030/df_NewDMv3.CensorEND.n88.select293.NA.rmAllNA.recode.lbl.rds.xz', compress = 'xz', compression = 9L) |> system.time()  
    env1$f$f_path.size_files(.path4read = '/home/rstudio/github_tidystat/Rdev/10_import_clean_datatype/16_categorical_labelled', regex4filename = 'df_NewDMv3.CensorEND.n88.select293.NA.rmAllNA.recode.lbl')  
        ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++    
    df_NewDMv3.CensorEND.n88.select293.NA.rmAllNA.recode.lbl.fct |> write_rds('/home/rstudio/Rproject_KoGES_AA10030/df_NewDMv3.CensorEND.n88.select293.NA.rmAllNA.recode.lbl.fct.rds.xz', compress = 'xz', compression = 9L) |> system.time()  
    env1$f$f_path.size_files(.path4read = '/home/rstudio/github_tidystat/Rdev/10_import_clean_datatype/16_categorical_labelled', regex4filename = 'df_NewDMv3.CensorEND.n88.select293.NA.rmAllNA.recode.lbl.fct')  
        ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
