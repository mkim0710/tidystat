...
DataSet.fct %>% str
DataSet.fct = DataSet.fct %>% env1$f$f_CodeBook_DataSet.labelled(tblVarName, vecNamed.swap = TRUE)
DataSet.fct %>% as_tibble() %>% print(n=5)
DataSet.fct %>% str(maxlevel = 2)
cat("    ________________________________________________________________________    \n")
DataSet.fct %>% mutate_if(function(vec) is.character(vec) && labelled::is.labelled(vec), as_factor) %>% str(maxlevel = 2)
```
