...
##~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~  
# .sourcename = "f_DataSetName.Search.read.checkEntity" |> paste0(".source.r"); .subpath=r"()"|>str_replace_all("\\\\","/"); .subpath.filename.source.r = .sourcename %>% paste0(.subpath,ifelse(.subpath=="","","/"),.); if(!.sourcename %in% names(.GlobalEnv$env1$source)) {  message('> source("',file.path(env1$path$source_base,.subpath.filename.source.r),'")'); .GlobalEnv$env1$source[[.sourcename]] = file.path(env1$path$source_base,.subpath.filename.source.r); suppressPackageStartupMessages(source(.GlobalEnv$env1$source[[.sourcename]]))  }
...
```{r}
i.VarName = "A01_AGE"
tblVarName %>% filter(VarName == i.VarName) %>% pull(ValueOptions)
tblVarName %>% filter(VarName == i.VarName) %>% pull(ValueOptions) %>% str_replace_all("[^(]*\\( *\\) *", "")
i.ValueUnit = tblVarName %>% filter(VarName == i.VarName) %>% pull(ValueOptions) %>% str_replace_all("[^(]*\\(.*\\) *", "")
```


  
## \$ MetaData\$tblVarName\$tblVarName = left_join() ----   
...
tblVarName = tblVarName %>% mutate(
    ValueUnit = ifelse(VarType == "Continuous", ValueOptions %>% str_replace_all("[^(]*\\(.*\\) *", ""), NA) %>% as.factor()
)
...
# _______________  
# @@ 321Table1-df_NewDMv3.CensorEND.n88.select293.NA.rmAllNA.recode.Rmd
...
# _______________  
# @@ 12CodeBook.recode-df_NewDMv3.CensorEND.select971.NA.rmAllNA.recode.Rmd
...
cat("    ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~    \n")
# KoGES201806vars.CLEAN_TRIM.filter |> t() %>% apply(2, function(vec) paste0(names(vec),": ",vec)) |> as.data.frame() %>% map_chr(paste, collapse="\n") %>% cat(sep="\n------------\n")
KoGES201806vars.CLEAN_TRIM.filter %>% mutate(VarName.suffix_Description = paste0(VarName.suffix, "\t| ", VarDescription, "\t| ", ValueOptions)) %>% select(VarName.suffix_Description, Questionnaire) |> t() |> as.data.frame() %>% map_chr(paste, collapse="\n") %>% cat(sep="\n    ------------------------------------------------------------------------    \n")
cat("    ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~    \n")
MetaData$tblVarName$tblWAVE_VARNAME |> dplyr::filter(VarName.suffix %in% VarName.suffix.selected) |> dplyr::select(VarName.suffix, VarDescription, Wave, WAVE_VARNAME, VarType, ValueChange) %>% spread(WAVE_VARNAME, key = "Wave")
cat("    ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~    \n")
DataSet |> select(matches(VarName.suffix.selected|>paste(collapse="|"))) |> mutate_if(is.character, as.factor) |> summary()
cat("    ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~    \n")
DataSet |> select(matches(VarName.suffix.selected|>paste(collapse="|"))) |> as_tibble()
```
  
  

## @ MetaData) Variables: exposure/mediator ----  
### \% VarCategory \%in\% c("생활습관(운동력)", "생활습관(음주력)", "생활습관(흡연력)", "식습관")  
```{r, echo=FALSE, results="markup", collapse=TRUE, paged.print=FALSE, comment="", R.options=list(width=120)}
options(width=120)
KoGES201806vars.CLEAN_TRIM |> dplyr::filter(VarCategory %in% c("생활습관(운동력)", "생활습관(음주력)", "생활습관(흡연력)", "식습관")) |> 
    dplyr::select(Num, VarCategory, VarName.suffix, VarDescription, ValueOptions, Select) |> env1$f$f_df.print_byVar(VarCategory, n=99)
```
  
  

### \% VarCategory \%in\% c("신체계측", "체성분검사")  
```{r, echo=FALSE, results="markup", collapse=TRUE, paged.print=FALSE, comment="", R.options=list(width=120)}
options(width=120)
KoGES201806vars.CLEAN_TRIM |> dplyr::filter(VarCategory %in% c("신체계측", "체성분검사")) |> 
    dplyr::select(Num, VarCategory, VarName.suffix, VarDescription, ValueOptions, Select) |> env1$f$f_df.print_byVar(VarCategory, n=99)
```
  
  
### \% VarDescription |> str_detect("활동|운동"|>regex(ignore_case=TRUE))  
```{r, echo=FALSE, results="markup", collapse=TRUE, paged.print=FALSE, comment="", R.options=list(width=120)}
options(width=120)
VarDescription.regex = "(활동)|(운동)"
KoGES201806vars.CLEAN_TRIM.filter = KoGES201806vars.CLEAN_TRIM |> dplyr::filter(VarDescription |> str_detect(VarDescription.regex|>regex(ignore_case=TRUE)))
KoGES201806vars.CLEAN_TRIM.filter$VarName.suffix |> as.character() |> deparse() |> cat("  \n", sep="")
cat("    ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~    \n")
KoGES201806vars.CLEAN_TRIM.filter |> dplyr::select(Num, VarCategory, VarName.suffix, VarDescription, ValueOptions, Select) |> env1$f$f_df.print_byVar(VarCategory, n=99)
```


#### \% VarName.suffix |> str_detect("MET|PHYS|EXER"|>regex(ignore_case=TRUE))  
##### \% VarName.suffix \%in\% c("PHYSTB", "PHYSIT", "MET", "EXERCUR", "EXER", "EXERFQ", "EXERDU" )   
##### \% VarName.suffix \%in\% c("PHYSTB", "MET")   
###### ! Check as1_7.sas7bdat  
```{r, echo=FALSE, results="markup", collapse=TRUE, paged.print=FALSE, comment="", R.options=list(width=120)}
options(width=120)
VarName.suffix.regex = "(MET)|(PHYS)|(EXER)"
KoGES201806vars.CLEAN_TRIM.filter = KoGES201806vars.CLEAN_TRIM |> dplyr::filter(VarName.suffix |> str_detect(VarName.suffix.regex|>regex(ignore_case=TRUE)))
KoGES201806vars.CLEAN_TRIM.filter$VarName.suffix |> as.character() |> deparse() |> cat("  \n", sep="")
##~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~  
VarName.suffix.selected = c("PHYSTB", "PHYSIT", "MET", "EXERCUR", "EXER", "EXERFQ", "EXERDU" ) 
KoGES201806vars.CLEAN_TRIM.filter = KoGES201806vars.CLEAN_TRIM |> dplyr::filter(VarName.suffix %in% VarName.suffix.selected)
VarName.suffix.selected |> deparse() |> cat("  \n", sep="")
cat("    ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~    \n")
# KoGES201806vars.CLEAN_TRIM.filter |> t() %>% apply(2, function(vec) paste0(names(vec),": ",vec)) |> as.data.frame() %>% map_chr(paste, collapse="\n") %>% cat(sep="\n------------\n")
...
DataSet |> select(matches(VarName.suffix.selected|>paste(collapse="|"))) |> as_tibble()
cat("    ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~    \n")
# as1_7.sas7bdat |> select(matches(VarName.suffix.selected|>paste(collapse="|"))) |> mutate_if(is.character, as.factor) |> summary()
cat("    ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~    \n")
# as1_7.sas7bdat |> select(matches(VarName.suffix.selected|>paste(collapse="|"))) |> as_tibble()
```

(55555=추적조사 미참여, 66666=조사안함, 77777=해당없음, 99999=미상/무응답/미측정)  




  
### \% VarDescription |> str_detect("식사|섭취"|>regex(ignore_case=TRUE))  
```{r, echo=FALSE, results="markup", collapse=TRUE, paged.print=FALSE, comment="", R.options=list(width=120)}
options(width=120)
VarDescription.regex = "(식사)|(섭취)"
KoGES201806vars.CLEAN_TRIM.filter = KoGES201806vars.CLEAN_TRIM |> dplyr::filter(VarDescription |> str_detect(VarDescription.regex|>regex(ignore_case=TRUE)))
KoGES201806vars.CLEAN_TRIM.filter$VarName.suffix |> as.character() |> deparse() |> cat("  \n", sep="")
cat("    ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~    \n")
KoGES201806vars.CLEAN_TRIM.filter |> dplyr::select(Num, VarCategory, VarName.suffix, VarDescription, ValueOptions, Select) |> env1$f$f_df.print_byVar(VarCategory, n=99)
...
#### \% VarName.suffix |> str_detect("B0[1-4]"|>regex(ignore_case=TRUE))  
##### \% VarName.suffix \%in\% c("BRF", "MEALRG", "AFTDNN", "B01", "B02", "B03", "B04", "B10", "B24")   
```{r, echo=FALSE, results="markup", collapse=TRUE, paged.print=FALSE, comment="", R.options=list(width=120)}
options(width=120)
VarName.suffix.regex = "(B0[1-4])"
KoGES201806vars.CLEAN_TRIM.filter = KoGES201806vars.CLEAN_TRIM |> dplyr::filter(VarName.suffix |> str_detect(VarName.suffix.regex|>regex(ignore_case=TRUE)))
KoGES201806vars.CLEAN_TRIM.filter$VarName.suffix |> as.character() |> deparse() |> cat("  \n", sep="")
##~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~  
VarName.suffix.selected = c("BRF", "MEALRG", "AFTDNN", "B01", "B02", "B03", "B04", "B10", "B24") 
KoGES201806vars.CLEAN_TRIM.filter = KoGES201806vars.CLEAN_TRIM |> dplyr::filter(VarName.suffix %in% VarName.suffix.selected)
VarName.suffix.selected |> deparse() |> cat("  \n", sep="")
cat("    ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~    \n")
# KoGES201806vars.CLEAN_TRIM.filter |> t() %>% apply(2, function(vec) paste0(names(vec),": ",vec)) |> as.data.frame() %>% map_chr(paste, collapse="\n") %>% cat(sep="\n------------\n")
...
### \% VarDescription |> str_detect("음주"|>regex(ignore_case=TRUE))  
```{r, echo=FALSE, results="markup", collapse=TRUE, paged.print=FALSE, comment="", R.options=list(width=120)}
options(width=120)
VarDescription.regex = "(음주)"
KoGES201806vars.CLEAN_TRIM.filter = KoGES201806vars.CLEAN_TRIM |> dplyr::filter(VarDescription |> str_detect(VarDescription.regex|>regex(ignore_case=TRUE)))
KoGES201806vars.CLEAN_TRIM.filter$VarName.suffix |> as.character() |> deparse() |> cat("  \n", sep="")
cat("    ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~    \n")
KoGES201806vars.CLEAN_TRIM.filter |> dplyr::select(Num, VarCategory, VarName.suffix, VarDescription, ValueOptions, Select) |> env1$f$f_df.print_byVar(VarCategory, n=99)
...
### \% VarDescription |> str_detect("흡연"|>regex(ignore_case=TRUE))  
```{r, echo=FALSE, results="markup", collapse=TRUE, paged.print=FALSE, comment="", R.options=list(width=120)}
options(width=120)
VarDescription.regex = "(흡연)"
KoGES201806vars.CLEAN_TRIM.filter = KoGES201806vars.CLEAN_TRIM |> dplyr::filter(VarDescription |> str_detect(VarDescription.regex|>regex(ignore_case=TRUE)))
KoGES201806vars.CLEAN_TRIM.filter$VarName.suffix |> as.character() |> deparse() |> cat("  \n", sep="")
cat("    ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~    \n")
KoGES201806vars.CLEAN_TRIM.filter |> dplyr::select(Num, VarCategory, VarName.suffix, VarDescription, ValueOptions, Select) |> env1$f$f_df.print_byVar(VarCategory, n=99)
...
#### \% VarName.suffix |> str_detect("Sex"|>regex(ignore_case=TRUE))  
##### \% VarName.suffix \%in\% c("SEX")  
###### :: var_label(DataSet[[i.WAVE_VARNAME]]) = ----  
###### :: val_label(DataSet[[i.WAVE_VARNAME]]) = ----  
```{r, echo=FALSE, results="markup", collapse=TRUE, paged.print=FALSE, comment="", R.options=list(width=120)}
options(width=120)
cat("    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%    \n")
# VarName.suffix.regex = "(SEX)"
...
cat("    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%    \n")
DataSet = get(DataSetName)
DataSet %>% select(any_of(tblVarName.filter$WAVE_VARNAME)) |> as_tibble() |> print(n=5)
library(labelled)
cat("    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%    \n")
tblVarName.filter$VarDescription |> as.character() |> deparse() |> cat("  \n", sep="")
cat("    ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~    \n")
for (i.WAVE_VARNAME in tblVarName.filter$WAVE_VARNAME) {
    var_label(DataSet[[i.WAVE_VARNAME]]) = tblVarName.filter %>% dplyr::filter(WAVE_VARNAME == i.WAVE_VARNAME) %>% pull(VarDescription) %>% as.character
}
cat("    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%    \n")
...
library(labelled)
for (i.WAVE_VARNAME in tblVarName.filter$WAVE_VARNAME) {
    val_labels(DataSet[[i.WAVE_VARNAME]]) = tblVarName.filter %>% 
        filter(WAVE_VARNAME == i.WAVE_VARNAME) %>% 
        pull(ValueOptions) %>% as.character |> 
        env1$f$f_chrNameValuePairs2vector(vecNamed.swap = TRUE)
}
cat("    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%    \n")
DataSet %>% {attr(., "n_distinct") = NULL; attr(., "MetaData") = NULL; .} |> select(any_of(tblVarName.filter$WAVE_VARNAME)) |> str()
DataSet %>% {attr(., "n_distinct") = NULL; attr(., "MetaData") = NULL; .} |> select(any_of(tblVarName.filter$WAVE_VARNAME)) |> as_tibble() |> print(n=5)
cat("    ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~    \n")
DataSet %>% {attr(., "n_distinct") = NULL; attr(., "MetaData") = NULL; .} |> select(any_of(tblVarName.filter$WAVE_VARNAME)) |> mutate_all(as_factor) |> str()
DataSet %>% {attr(., "n_distinct") = NULL; attr(., "MetaData") = NULL; .} |> select(any_of(tblVarName.filter$WAVE_VARNAME)) |> mutate_all(as_factor) |> as_tibble() |> print(n=5)
```
...
f_CodeBook_DataSet_VarName.labelled = function(DataSet, vec.VarName, tblVarName_VarDescription_ValueOptions, NameValuePair_separator = ",", Name_Value_separator = "=", vecNamed.swap = FALSE, VarType.numeric = "Continuous", output.select = FALSE, VERBOSE = getOption("verbose")) {
    library(labelled)
...
DataSet = get(DataSetName)
attributes(DataSet)$MetaData = NULL
# cat("    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%    \n")
f_CodeBook_DataSet_VarName.labelled(DataSet, "A00_SEX", tblVarName, vecNamed.swap = TRUE, output.select = TRUE) %>% as_tibble() %>% print(n=5)
f_CodeBook_DataSet_VarName.labelled(DataSet, "A00_SEX", tblVarName, vecNamed.swap = TRUE, output.select = TRUE) %>% str(maxlevel = 2)
cat("    ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++    \n")
f_CodeBook_DataSet_VarName.labelled(DataSet, paste0("A0", 1:2, "_AGE"), tblVarName, vecNamed.swap = TRUE, output.select = TRUE) %>% as_tibble() %>% print(n=5)
f_CodeBook_DataSet_VarName.labelled(DataSet, paste0("A0", 1:2, "_AGE"), tblVarName, vecNamed.swap = TRUE, output.select = TRUE) %>% str(maxlevel = 2)
```


## \% |> f_CodeBook_DataSet.labelled()  
```{r, echo=TRUE, results="markup", collapse=TRUE, paged.print=FALSE, comment="", R.options=list(width=120)}
options(width=120)

f_CodeBook_DataSet.labelled = function(DataSet, tblVarName_VarDescription_ValueOptions, NameValuePair_separator = ",", Name_Value_separator = "=", vecNamed.swap = FALSE, VarType.numeric = "Continuous", VERBOSE = getOption("verbose")) {
...
DataSet = get(DataSetName)
attributes(DataSet)$MetaData = NULL
# DataSet = DataSet %>% select(matches("SEX"))
# DataSet = DataSet %>% select(matches("^A00"))
# DataSet = DataSet %>% select(matches("^A0[0-1]")) %>% select(matches("AGE"))
DataSet = DataSet %>% select(matches("^A0[0-1]"))
# DataSet = DataSet %>% f_CodeBook_DataSet.labelled(tblVarName, vecNamed.swap = TRUE, VERBOSE = TRUE) 
DataSet = DataSet %>% f_CodeBook_DataSet.labelled(tblVarName, vecNamed.swap = TRUE)
DataSet %>% as_tibble() %>% print(n=5)
DataSet %>% str(maxlevel = 2)
cat("    ________________________________________________________________________    \n")
DataSet %>% mutate_if(function(vec) is.character(vec) && labelled::is.labelled(vec), as_factor) %>% str(maxlevel = 2)
```
