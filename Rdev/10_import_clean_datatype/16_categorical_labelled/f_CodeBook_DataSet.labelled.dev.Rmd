...
# @@ Rdev/10_import_clean_datatype/16_categorical_labelled/env1$f$f_CodeBook_DataSet.labelled.dev.Rmd  
...
# cat("    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%    \n")
env1$env.internal$f_CodeBook_DataSet_VarName.labelled(DataSet, "A00_SEX", tblVarName, vecNamed.swap = TRUE, output.select = TRUE) %>% as_tibble() %>% print(n=5)
env1$env.internal$f_CodeBook_DataSet_VarName.labelled(DataSet, "A00_SEX", tblVarName, vecNamed.swap = TRUE, output.select = TRUE) %>% str(maxlevel = 2)
cat("    ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++    \n")
env1$env.internal$f_CodeBook_DataSet_VarName.labelled(DataSet, paste0("A0", 1:2, "_AGE"), tblVarName, vecNamed.swap = TRUE, output.select = TRUE) %>% as_tibble() %>% print(n=5)
env1$env.internal$f_CodeBook_DataSet_VarName.labelled(DataSet, paste0("A0", 1:2, "_AGE"), tblVarName, vecNamed.swap = TRUE, output.select = TRUE) %>% str(maxlevel = 2)
```


## \% |> env1$f$f_CodeBook_DataSet.labelled()  
```{r, echo=TRUE, results="markup", collapse=TRUE, paged.print=FALSE, comment="#", R.options=list(width=120)}
options(width=120)

env1$f$f_CodeBook_DataSet.labelled = function(DataSet, tblVarName_VarDescription_ValueOptions, NameValuePair_separator = ",", Name_Value_separator = "=", vecNamed.swap = FALSE, VarType.numeric = "Continuous", VERBOSE = getOption("verbose")) {
...
# DataSet = DataSet %>% env1$f$f_CodeBook_DataSet.labelled(tblVarName, vecNamed.swap = TRUE, VERBOSE = TRUE) 
DataSet = DataSet %>% env1$f$f_CodeBook_DataSet.labelled(tblVarName, vecNamed.swap = TRUE)
